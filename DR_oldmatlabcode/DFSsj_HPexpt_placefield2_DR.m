...
savefile = [savedir 'test']; % HPa and HPb fields  %%DR
...
runepochfilter = 'isequal($environment, ''wtr1'') || isequal($environment, ''wtr2'')'; %DR
   ...
   % timefilter = { {'getlinstate', '(($state ~= -1) & (abs($linearvel) >= 3))', 6}, {'DFTFsj_getriptimes','($nripples == 0)','tetfilter',riptetfilter,'minthresh',3} }; %I don't have DFTSsj_getlinstate
    ...
    % Filter creation
    % ----------------
    psf = createfilter('animal',animals,'days',dayfilter,'epochs',runepochfilter,'cells',placecellfilter,'excludetime', timefilter,'iterator', iterator);
%     psf = createfilter('animal',animals,'epochs',epochfilter,'cells',placecellfilter,'excludetime', timefilter,'iterator', iterator);  %no day filter
...
gatherdatafile = [savedir 'HPALL_PFCfields_gather']; 
...
for anidx = 1:length(useanim)
    ...
    for i=1:length(psf(an).output{1}),
        ...
        %%%%DR commnted... can't find "modf" and this seccion isn't used
        %%%%ever 
%         % Only indexes 
