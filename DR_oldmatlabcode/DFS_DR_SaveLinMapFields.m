...
% val=1; savefile = [savedir 'HPa_allfields_DR']; % HPa fields
%val=2; savefile = [savedir 'HPb_allfields_DR']; % HPb fields
%val=3; savefile = [savedir 'HPc_allfields_DR']; % HPc fields
% val=4; savefile = [savedir 'Ndl_allfields_DR']; % Ndl fields
% val=5; savefile = [savedir 'Rtl_allfields_DR']; % Rtl fields
val=6; savefile = [savedir 'Brg_allfields_DR']; % Brg fields
...
%     runepochfilter = 'isequal($environment, ''wtr1'') || isequal($environment, ''wtr2'') || isequal($environment, ''ytr'')';
    runepochfilter = 'isequal($type, ''run'') && ~isequal($environment, ''lin'')'; %%DR... I'm using this instead.. The above doesn't capture epochs from GR's new animals for whatever reason.. maybe epoch labelling 
...
%     timefilter = { {'DFTFsj_getlinstate', '(($state ~= -1) & (abs($linearvel) >= 3))', 6}, {'DFTFsj_getriptimes','($nripples == 0)','tetfilter',riptetfilter,'minthresh',3} };
    ...
    % Filter creation
    % ----------------
    psf = createfilter('animal',animals,'days',dayfilter,'epochs',runepochfilter,'cells',cellfilter,'excludetime', timefilter,'iterator', iterator);
...
switch val
    case 1
        gatherdatafile = [savedir 'HPa_allfields_gather_DR']
    case 2
        gatherdatafile = [savedir 'HPb_allfields_gather_DR']
    case 3
        gatherdatafile = [savedir 'HPc_allfields_gather_DR']
    case 4
        gatherdatafile = [savedir 'Ndl_allfields_gather_DR']
    case 5
        gatherdatafile = [savedir 'Rtl_allfields_gather_DR']
    case 6
        gatherdatafile = [savedir 'Brg_allfields_gather_DR']
end
