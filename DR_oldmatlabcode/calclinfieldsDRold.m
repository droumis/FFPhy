function trajdata = calclinfields(spikes,statevector,lindist,linpos, index,binsize)
...
posindexfield = 7;
if ~isempty(spikes)
    maxlinposind = length(statevector);
    spikes = spikes(1:max(find(spikes(:,posindexfield)<=maxlinposind)),[1 posindexfield]); % DR - ignore any spike outside of linpos statematrix
    spikes(:,3) = statevector(spikes(1:max(find(spikes(:,2)<=maxlinposind)),2)); %add the traj state for each spike
    spikes(:,4) = lindist(spikes(1:max(find(spikes(:,2)<=maxlinposind)),2)); %add the linear distance
...
trajnum = max(statevector);
    ...
    timestep = statematrix.time(end,1) - statematrix.time(end-1,1);
