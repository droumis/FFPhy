...
% PUT MANUAL DATA
val=1;savefile = [savedir 'HPal_CA1PFC_Lags_PostSleepRip_std3_speed4_ntet2_alldata_2-22-2014']; area = 'CA1thetamodPFCripmod'; clr = 'r';
...
%     runepochfilter = 'isequal($environment, ''wtr1'') || isequal($environment, ''wtr2'') || isequal($environment, ''ytr'')';
    ...
    switch val
        case 1
            cellpairfilter = {'allcomb','(strcmp($area, ''CA1'') || strcmp($area, ''iCA1'')) && ($numspikes > 100)','strcmp($area, ''PFC'') && ($numspikes > 100)'};
%             cellpairfilter = {'allcomb','(strcmp($area, ''CA1'') || strcmp($area, ''iCA1'')) && ($numspikes > 100) && strcmp($thetamodtag, ''y'') ','strcmp($area, ''PFC'') && ($numspikes > 100) && strcmp($thetamodtag2, ''y'')'};
    % %IMP! CA1theta
    % %------------------------
        case 2
...
timefilter_rip = {{'DFTFsj_getriptimes4','($nripples > 1)','tetfilter',riptetfilter,'minstd',3}};
...
%     thetaf = runfilter(thetaf);
    ...
    if savedata == 1
        clear figopt1 runscript plotdays plotanimidx savedata cyclemaps saveDRfigs
        save(savefile,'-v7.3');
    end
...
switch val
    ...
    % ------------------------------------------------------------------------
    case 1       
        gatherdatafile = [savedir 'HPal_CA1PFC_Lags_PostSleepRip_std3_speed4_ntet2_alldata_2-22-2014_gather'], area = 'CA1ripmodPFCripmod'; kind = 'ripmodripmod'; state = '';
    case 2
...
mkdir('/mnt/data25/sjadhav/HPExpt/Figures_DR/XCorrgrams_SigPairs_HPa_postsleep');
figdir = '/mnt/data25/sjadhav/HPExpt/Figures_DR/XCorrgrams_SigPairs_HPa_postsleep/';
...
%comgr            curroverlap = Salloverlap_epcomb(i);
       ...
       %comgr     if ~isnan(curroverlap) && curroverlap>0.2 && SallNeventscorr_runtheta_epcomb(i)>=20 && SallNeventscorr_rip_epcomb(i)>=20
               ...
               %if allp(i) <0.05, str = '*'; else, str = ''; end
                ...
                figfile = [figdir,'SigCorrPairsXCorr_thetaRip_',num2str(sigcnt)];
                if saveDRfigs==1
...
%      end %if type matches
     ...
     rsthetarip = ranksum(ThetaSigPairsAllPeakLags,RipSigPairsAllPeakLags);
     figure; hold on
     bar([1], mean(ThetaSigPairsAllPeakLags),'FaceColor', 'r', 'EdgeColor', 'none');
     bar([2], mean(RipSigPairsAllPeakLags), 'FaceColor', 'b', 'EdgeColor', 'none');
     errorbar2([1 2], [mean(ThetaSigPairsAllPeakLags) mean(RipSigPairsAllPeakLags)],  [std(ThetaSigPairsAllPeakLags) std(RipSigPairsAllPeakLags)] , 0.3, 'k')
     set(gca, 'xtick', [1 2], 'xticklabel', {'Theta', 'Rip'})
     ylabel('Lag')
     title({'HPa XCorr Peak Lag SigRipPairs'; sprintf('RS (%0.5f)',rsthetarip)});
     
     figfile = [figdir,'Lag_SigCorrPairsXCorr_thetaRip'];
     if saveDRfigs==1
         print('-dpng', figfile, '-r300');
     end
     if ~cyclemaps == 0
