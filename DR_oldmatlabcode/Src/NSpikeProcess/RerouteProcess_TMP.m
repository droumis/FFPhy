...
% either kept in config folder
%configdir = '/data14/jai/Config/';

% or in the animal data folder
configdir = strcat(datadir,animalname,'/',animalname,dsz,dayt,'/');
...
if any(runepoch==epoch);
    ...
    % remove all events before start and after end of epoch according to
    % times.mat
    
    Results=Results(Results(:,1)>=epochstart,:);
    Results=Results(Results(:,1)<=epochend,:);
    ...
    if mean(Results(:,3))~=-50 % only process when a reward is triggered
        ...
        % find index of reward well triggers higher than the first
        % rewarded well
        ...
        for ii=1:(size(k,1)-1);
            crun=[];
            crun(1,1)=Results(k(ii),2); % trial start well
            crun(1,3)=Results(k(ii),5); % trial start time
            crun(1,2)=Results(k(ii+1),2); % trial end well
            crun(1,4)=Results(k(ii+1),1); % trial end time
            crun(1,5)=crun(1,4)-crun(1,3);
        ...
        drun(1,3)=Results(k(ii),5); % trial start time
...
posout=Posepochprocess(daydirect,animalname,day,epoch,'cmperpix',pixdim);
    ...
    % cmperpix for sleepbox after 2013 03 08
    posout=Posepochprocess_sleep(daydirect,animalname,day,epoch,'cmperpix',0.34);
end
