function [Epochdata]=DIOprocessReturnHome(animaldir,day,epoch,configfile,wells,odour)
% Generates data for cued cross task
...
for i =2:size(Config,2);
    ...
    curractive(1,3) = Config{1,i}(10,1); % well output at home if next
    curractive(1,4) = Config{1,i}(3,1); % well out put bit
...
while i<=size(Results,1)-1;
    ...
    % if size(m,1)~=0;
    % Get predicted outbound runs
    if crun(1,1)==homewell(1,1);
        % find departure from well
        n = find(Results(:,2)~=Results(i,2));
        o = find(n>i);
        if size(o,1)~=0;
            crun=[];
        crun(1,1)=Results(n(o(1,1),1)-1,2);
        crun(1,4)=Results(n(o(1,1),1)-1,1);
       
        % find next visited well
        l = find(Results(:,2)~=Results(i,2));
        m = find(l>i);
        [a,b]=find(Results(:,1)==crun(1,4));
        
        [c,d]=find(activewells(:,3)==Results(i,3));
            if size(c,1)~=0;
            crun(1,2)=activewells(c,2);
            crun(1,3)=Results(l(m(1,1),1),2);
            crun(1,5)=Results(l(m(1,1),1),1);
            crun(1,6)=crun(1,5)-crun(1,4);
            crun(1,8)=n(o(1,1),1)-l(m(1,1),1);
                if crun(1,2)==crun(1,3);
                    crun(1,7)=1;
                else crun(1,7)=0;
                end
            run=[run;crun];
    ...
    % Get next inbound run
        ...
        l = find(Results(:,2)~=Results(i,2));
        m = find(l>i);
        p = find(Results(:,2)==homewell(1,1));
        q = find(p>i);
            if size(m,1)~=0;
            crun(1,2)=homewell(1,1);
            crun(1,3)=Results(l(m(1,1),1),2);
            crun(1,5)=Results(l(m(1,1),1),1);
            crun(1,6)=crun(1,5)-crun(1,4);
                if size(q,1)~=0;
                crun(1,8)=p(q(1,1),1)-(n(o(1,1),1)-1);
                else crun(1,8)=NaN;
                end
...
outboundcorrect=size(find(run(find(run(:,1)==homewell(1,1)),7)==1),1)/outboundn;
inboundcorrect=size(find(run(find(run(:,1)~=homewell(1,1)),7)==1),1)/inboundn;
hometrigger=size(find(Results(:,2)==homewell(1,1)),1);
a=0;
for i=activewells(activewells(:,3)>=0,3)';
    c=size(find(Homeevents(:,3)==i),1);
    a=a+c;
end
homerewards=a;
...
Epochdata.Runtxtinfo={'Start','Goal','1st Visit','Start time','End time','Duration','Result','Errors to goal'};
Epochdata.Wellinfo.rewardedwells=[activewells(find(activewells(:,3)>=0),2),activewells(find(activewells(:,3)>=0),1)];
Epochdata.Wellinfo.homewell=homewell;
