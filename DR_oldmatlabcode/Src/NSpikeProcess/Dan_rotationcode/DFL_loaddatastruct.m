...
out = [];
datafiles = dir([animaldir, animalprefix,'_','*.mat']); % removed 'datatype' since task file in reroute task is not stored there

for i = 1:length(datafiles)
    if isempty(days)
        s = datafiles(i).name;
        if isempty(strfind(s,'task')); % do not load linpos or task files
