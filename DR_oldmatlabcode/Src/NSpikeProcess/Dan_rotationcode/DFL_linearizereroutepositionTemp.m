...
%             
%             exclude_seg = [9 10 11 12];
%             exitref_tmp = ismember(exitref(:,5),exclude_seg) & exitref(:,5);
        ...
        for ct = 2:size(exittimes,1)
            ...
            if (~isempty(tmpindex))
                for checkout = 1:size(lindist,2)
                    if ~ismember(checkout, tmptraj,'legacy')
...
if track~='reroute';
    ...
    for trajnum = 1:size(traject,1)
        ...
        %minvelocity)
        findindex = find( inDefinedTraj(:,trajnum)& ismember(segmentindex,segmentsInTraj,'legacy') & forewarddir);
        %assign the current trajectory to those indeces
        trajvector(findindex) = 2*(trajnum)-1;
        if ismember(1,includeStates,'legacy')
            statevector(findindex) = trajvector(findindex);
        end
        %then find the negative direction times
        findindex = find( inDefinedTraj(:,trajnum) & ismember(segmentindex,segmentsInTraj,'legacy') & backwarddir);
    ...
    if ismember(2,includeStates,'legacy')
        statevector(findindex) = trajvector(findindex);
    end
    ...
    trajvector(findindex) = 2*(segmenttable(rowfind(segmentindex(findindex),segmenttable(:,3)),1))-1;
    if ismember(3,includeStates,'legacy')
        statevector(findindex) = trajvector(findindex);
    end
    ...
    if ismember(4,includeStates,'legacy')
        statevector(findindex) = trajvector(findindex);
    end
    ...
    if ismember(5,includeStates,'legacy')
        statevector(findindex) = trajvector(findindex);
    end
    ...
    if ismember(6,includeStates,'legacy')
