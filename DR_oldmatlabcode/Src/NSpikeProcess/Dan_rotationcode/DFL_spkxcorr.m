function out = DFL_spkxcorr(index, excludetimes, spikes, lag_window, includestates, minV, f, varargin)
...
for ii = 1:size(index,1)
    ...
    for jj = 1:size(index,1)
        if ii ~= jj
            ...
            spike_xcorr = spikexcorr(spike_data.data(:,1)', spike_data2.data(:,1)', 0.01, lag_window);
            
            % normalise nspikes2 is the reference
            spike_xcorr.c1vsc2=spike_xcorr.c1vsc2./spike_xcorr.nspikes2;
