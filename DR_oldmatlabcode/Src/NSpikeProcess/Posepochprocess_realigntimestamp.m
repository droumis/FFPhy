...
%         max(Rpos)
        ...
        coord(inboundpix,:)=lenscorrtransf_realigntimestamp(pixind(inboundpix));
        Rpos(:,1:2)=coord;
...
%         Rpos(zerorows,2:5)=0; % values other than 0 are considered true positions and will not be correctly interpolated
      ...
      if mod(epoch,2)==0
        ...
        SKpos=SK_estimate_position_realigntimestamp(RRpos,'centimeters_per_pixel',cmperpix,'front_back_marker_weights',[1 0]);
      end
      ...
      %SKpos{1,1}.data(:,2:5)=SKpos{1,1}.data(:,2:5);
        ...
        % at each time
        if ~isempty(rawpos.data)
            pos = posinterpcorrection_realigntimestamp(rawpos, 'diode', diodepos,...
