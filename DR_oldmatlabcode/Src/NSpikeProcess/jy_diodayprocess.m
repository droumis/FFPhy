function jy_diodayprocess(daydirect, animdirect, fileprefix, daynum, varargin)

% function dioprocess(daydirect, animdirect, fileprefix, daynum, varargin)
% DIO file must be in unint32, use nspike extract -dio not -diotxt
...
filename=[daydirect,fileprefix,dsz,dayt,'.dio'];

% readDIO32 reads the DIO32 and converts it to text


[dioout]=readDIO32(filename);

diotimes = dioout(:,1);
diovals = dioout(:,2:size(dioout,2));

% Flip DIO bits since every 1-16 is actually 15-0
% Only necessary if reading the text converted version of nspike_extract
% -diotext
...
for a = 1:length(active_pins)
   ...
   % remove duplicated pulse times, happens rarely
   diopulses{pin}.pulsetimes = unique(double([cat(1,pulse.tstart) cat(1,pulse.tend)]),'rows');
...
for epoch = 1:length(times)
   ...
   cd('/data14/jai');
   %cd('/data18/jai');
   checkanimaldir = dir(animdirect);
    if (isempty(checkanimaldir))
        %an a plot folder needs to be created
        system(sprintf('mkdir %s',animdirect));
