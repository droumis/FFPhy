function position = JY_SK_estimate_position(rawpos,varargin)
...
for i = 1:length(rawpos)
  ...
  if (sum(sum(~isnan(rawpos(:,4:5)))) == 0) || (sum(sum(rawpos(:,4:5))) == 0)
    back_exists = false;
  else
    back_exists = true;
  end  
  ...
  xfront = double(rawpos(:,2));
  yfront = double(rawpos(:,3));
  xback = double(rawpos(:,4));
  yback = double(rawpos(:,5));
  ...
  position.data(:,2:3) = [x_smooth y_smooth];
  position.data(:,5) = sqrt(sum([x_vel y_vel].^2,2)); % speed
  position.data(:,6:7) = [x_vel y_vel]; % velocity
  position.data(:,8:9) = nan;
  position.data(~censor_mask,8) = x - x_smooth(~censor_mask);
  position.data(~censor_mask,9) = y - y_smooth(~censor_mask);
  % Perform LOESS smoothing to get denoised head direction
  position.data(:,4) = nan; % default head direction
  if (back_exists)
    position.data(~censor_mask,4) = loess_head_direction( ...
