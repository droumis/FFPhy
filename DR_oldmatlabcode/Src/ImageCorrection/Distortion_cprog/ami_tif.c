...
#define ami_qhc_seek_allocate_read(file,offset,type,nele,vector) { int tt;\
        ...
        tt = (type == 0) ? sizeof(short) : NEW_LONG_SIZE; tt *= nele;\
...
int read_tiff_TIFF_struct (char name[200], struct TIFF_file *myTiff)
{
  ...
  retorno = 0; entry.tam = 2*sizeof(short)+2*NEW_LONG_SIZE;
  ...
  fread(data,2*sizeof(short)+NEW_LONG_SIZE,1,fptr);
  ...
  fread(data,NEW_LONG_SIZE,1,fptr);
...
int write_tiff_TIFF_struct  (char name[200], struct TIFF_file *myTiff)
{
  ...
  /* Allocate memory: maximun data is 4*NEW_LONG_SIZE -> 2 RATIONAL */
  ami_malloc1d(data,unsigned char,4*NEW_LONG_SIZE);
  ...
  temp = offset;
  info.header.offsetIFD = offset += NEW_LONG_SIZE; hexadecimal4(offset,&data[temp]);
  ...
  offset += info.header.nDirectoryEntries*entry.tam+NEW_LONG_SIZE; /* offset + IFD's end and the offset of the next IFD */
  ...
  for (i = 0; i < info.header.nDirectoryEntries; i++, temp += entry.tam) {
    ...
    case 6: /* The offsets of the strips */
      ...
      info.stripOffset.valueOffset = entry.valueOffset = offset+4*NEW_LONG_SIZE; /* Offset + 2 rational */
      break;
    ...
    case 10:   /* X resolution */
      ...
      info.xResolution.offset = entry.valueOffset =  offset; offset += 2*NEW_LONG_SIZE;
    ...
    case 11:   /* Y resolution */
      ...
      info.yResolution.offset = entry.valueOffset =  offset; offset += 2*NEW_LONG_SIZE;
  ...
  fseek(fptr,temp,SEEK_SET);
  info.header.offsetIFD = 0; hexadecimal4(info.header.offsetIFD,data); fwrite(data,NEW_LONG_SIZE,1,fptr);
  ...
  fwrite(data,4*NEW_LONG_SIZE,1,fptr);
