...
ed points and we add a term to the polynomial also minimizing such
...
)
{
  ...
  printf("\nNote: Input image is an RGB - to output corrected coordinates, input a grey level image");
...
int width /** INPUT IMAGE WIDTH */,
int height /** INPUT IMAGE HEIGHT */,
FILE *fpcoo /** pointer to output coordinate file */
)
{
  int i,j,i2,j2;
  long m,m2,size=width*height;
  ...
  /** Modified Portion - 8/18/11 **/
  /* fprintf(fpcoo,"*********************LENS DISTORTION MODEL: OUTPUT COORDINATES*********************\n");
  fprintf(fpcoo,"\n****************************************************************************\n");
  for(k=0;k<size;k++){
     fprintf(fpcoo,"coordinate[%d]=%d\n",k,g_undistort[k]);
  }
  */
