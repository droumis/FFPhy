 ...
 * \brief function to compute the polynomial root in an interval. We use a
          combination of bisection and Newton-Raphson tecniques
...
long double ami_root_bisection(
    ...
    while ( (b2-a2)>(TOL*fabs(b2)+1e-10) &&  fabs(c-c0)>(TOL*fabs(c0)+1e-10) && (iter++)<maxiter) {
 ...
 * \brief function to estimate real polynomial roots. We use a recursive procedure
          computing the derivative polynomial roots.
...
int ami_polynomial_root(
    double *pol /** coefficient polynomial vector pol[degree]+pol[degree-1]*x+pol[degree-2]*x^2+...
                FOR HISTORICAL REASONS THE ORDER POLYNOMIAL COEFICIENT IS GIVEN IS DIFFERENT
                                THAT IN THE OTHERS POLYNOMIAL FUNCTIONS */,
    int degree /** polynomial degree */,
