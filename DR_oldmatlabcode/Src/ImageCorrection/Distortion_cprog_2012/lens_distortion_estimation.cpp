...
int main(int argc, char *argv[]) {
  ...
  /* Will prepare the output file to which the coordinates will be saved (called in undistort_image_1c) */
  if(!(fpcoo=fopen(argv[4],"w"))){
     printf("Error while opening the %s file",filename);
     exit(0);
  }
    ...
    ami::image<unsigned char> img_out3 = undistort_image_inverse_fast(img,Na,a,dc,1.,fpcoo);
    strcpy(filename,argv[2]);
    if (img_out3.write(filename)!=0) printf("Image can not be saved\n");

    fclose(fpcoo);    /* close the output coordinate file */
