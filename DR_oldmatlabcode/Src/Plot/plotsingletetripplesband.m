...
for d=day;
    for e=epoch;
        ...
        for tetind=tet;
            ...
            %open ripple filtered eeg file
            tetfile=strcat('/data14/jai/I1_/EEG/','I1ripple',dsz,num2str(d),'-',num2str(e),'-',tsz,num2str(tetind),'.mat');
            load(tetfile);
            %open ripple file
            ripfile=strcat('/data14/jai/I1_/','I1ripples',dsz,num2str(d),'.mat');
            load(ripfile);
            figure;
            
            % ripple band data structure
            fieldref=ripple{1,d}{1,e}{1,tetind};
            
            % ripple index
            ripfieldref=ripples{1,d}{1,e}{1,tetind};
...
%             rpind=round(rpind);
            ...
            dist=0;
            %nrows=round(size(ripfieldref.startind,1)/10)+1;
            ...
            for rind=1:20; %size(ripfieldref.startind,1);
                currript=fieldref.data(ripfieldref.startind(rind,1)-100:ripfieldref.endind(rind,1)+100,1)';
                %ript=[ript;currript];
                rind=rind+1;
                
                % find maxduration
                maxdur=max((ripfieldref.endind-ripfieldref.startind));
