...
out=setfilterfunction(f, 'JY_trialspikelocation', {'spikes','data','linpos'});
...
for ii=1:size(uniquecells,1);
    ...
    for jj=1:size(uniquecellindex,1)
        ...
        %barrier=sum(outdata{1,uniquecellindex(jj)}.trajectorybarrier);
        barrier=size(outdata{1,uniquecellindex(jj)}.trajectorybarrier,1);
        
        
        if sum(outdata{1,uniquecellindex(jj)}.trajectorybarrier==1)>0
            
            %barriertrialindex=find(outdata{1,uniquecellindex(jj)}.trajectorybarrier==1);
            barriertrialindex=[1:1:size(outdata{1,uniquecellindex(jj)}.trajectorybarrier,1)]';
            
            dist=0;
            nrows=ceil(barrier/2);
            columns=2;
            gap_h=0.01;
            gap_w=0.01;
            marg_h=[0.01 0.1];
            ...
            for kk=1:size(barriertrialindex,1)
                % get current axis
                mm=barriertrialindex(kk);
                axes(ha(ax));
                
                
                hold on;
                
                % divide trial duration into bins of 1s
                
                tstart=data{1,mm}.pos(1,1);
                tend=data{1,mm}.pos(end,1);
                bins=tstart:1:tend;
                
                % histogram spikes
                [spikehist, xout]=hist(data{1,mm}.spikes(:,1),bins);
                
                % calculate distance from each pos to barrier
                
                %barriercoord1=[107.1 53.98];
                %barriercoord1=[103.3 24.02];
                 barriercoord1=[67.69 61.36];
                barriercoord2=[141.2 59.74];
                
                
                distance1=[];
                %distance2=[];
                for ll=1:size(data{1,mm}.pos,1);
                    
                    
                    distance1=[distance1; pdist([data{1,mm}.pos(ll,2:3);barriercoord1])];
                    %distance2=[distance2; pdist([data{1,mm}.pos(ll,2:3);barriercoord2])];
                ...
                plot((data{1,mm}.pos(:,1)-(data{1,mm}.pos(1,1))),distance1,'Color',[0.5 0.5 0.5]);
                hold on;
                %plot((data{1,mm}.pos(:,1)-(data{1,mm}.pos(1,1))),distance2,'Color',[0.7 0.7 0.7]);
                hold on;
                
                %plot the start
                bar((xout-(data{1,mm}.pos(1,1))),spikehist.*10,'g');
                ...
                %
                set(gca,'YTickLabel',{'0','5','10'},'FontSize',10 );
                ...
                distlabel2=[0.60*(xlimval(2)-xlimval(1))+xlimval(1) 0.8*(ylimval(2)-ylimval(1))+ylimval(1)];
                text(distlabel2(1),distlabel2(2),sprintf('%s spikes',num2str(size(data{1,mm}.spikes,1))),'FontSize',10,'Color','k');
                
                
                if outdata{1,uniquecellindex(jj)}.trajectorybarrier(kk)==1
                    % barrier label
                    barrierlabel=[0.1*(xlimval(2)-xlimval(1))+xlimval(1) 0.80*(ylimval(2)-ylimval(1))+ylimval(1)];
            ...
            text(0.5, 0.98,sprintf('Trial spike count (250ms bins) vs distance to barriers for %s \n Day %s Epoch %s Tetrode %s cell %s', animals{1,1},...
            ...
            figurename = strcat(animals{1,1},'_disttobarrier_d',num2str(daytext),'_e',num2str(dayindex),'_t',num2str(uniquecells(ii,1)),'_c',num2str(uniquecells(ii,2)));
