function [output] = PlotopenfieldrateReroute(animaldir,tet,cell,day,epoch, binsize,std,spn,spr,sspr,ot,excludetime)
...
if spr == 1
    ...
    title(strcat('Occupancy normalised spike rate for ',animalname,': Day ',dayt,' epoch ',epocht, ' Tetrode ', tett,'Cell ',cellt));
    ...
    figurename = strcat(animalname,'_spr_d',dayt,'e',epocht,'t',tett,'c',cellt);
...
if sspr == 1
    ...
    % animal_day_epoch
    cd(strcat(datadir,animaldir,'/','Plot/'));
    figurename = strcat(animalname,'_sspr_d',dayt,'e',epocht,'t',tett,'c',cellt);
    saveas(gcf, figurename, 'pdf');
...
if spn == 1
    ...
    % first plot the trajectory for the epoch in grey
    
    %     nc = 1024;
    %     cmap = [1 1 1;0.8 0.8 0.8];
    %     cmap(1,:) = 1;
    %     colormap(cmap);
    
    imagedatas = flipud(output.occupancy);
    imagedatas(imagedatas>0)=1;
    
    % then plot the number of spikes on top
    hold on;
    nc = 100;
    cmap = jet(nc);
    cmap(1,:) = 1;
    ...
    % Generates a title from information provided
    title(sprintf('Spikes for %s: Day %s Epoch %s Tetrode %s cell %s \n %s spikes',...
        animalname, dayt, epocht, tett,cellt,num2str(size(goodspikes,1))));
    ...
    % animal_day_epoch
    cd(strcat(datadir,animaldir,'/','Plot/'));
    figurename = strcat(animalname,'_spn_d',dayt,'e',epocht,'t',tett,'c',cellt);
    saveas(gcf, figurename, 'pdf');
