function [imagedatas, output,pospossize, Wellpos] = PlotopenfieldrateRerouteNoSave(animaldir,tet,cell,day,epoch, binsize,std,spn,spr,sspr,ot,excludetime)
...
%         animalname, dayt, epocht, tett,cellt,num2str(size(pospos,1))));
% 
%     caxis([-1 bmax]);
% 
%     % Keeps the proportions of the data
...
if spn == 1
    ...
    % Firing rate
    imagedatas=image(imagedatasp,'CDataMapping', 'scaled');

    % Generates a title from information provided
    title(strcat('Spikes ',animalname,': Day ',dayt,' epoch ',epocht, ' Tetrode ', tett,'Cell ',cellt));
