function f = JY_setfiltertime(f, timefilter)
...
for an = 1:length(f)
    ...
    for i = 1:length(timefilter)
        ...
        elseif (length(timefilter{i}) > 2)
            ...
            error('Each time filter call must have at least two inputs: {function, searchstring/excludetimelist, option1, optionvalue1, ...}');
        end
	if (~strcmp(timefilter{i}{2}, 'excludetimelist')) 
	    filterresults{i} = JY_evaluatefilter2(dataStruct, timefilter{i}{2}); %evaluate the filter string for every structure and return results in a cell array
	    filtertype(i) = 1;
	else
