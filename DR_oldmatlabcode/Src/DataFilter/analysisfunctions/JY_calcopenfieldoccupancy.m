function [out] = JY_calcopenfieldoccupancy(index, excludetimes, spikes,data, varargin)
...
%     biny = (miny:binsize:maxy);
    ...
    if ~isempty(tmpspikes)
        ...
        output.smoothedspikerate=zeros(size(output.occupancy));
       
        if ~isempty(find(output.spikes>0))
        ...
        output.normsmoothedspikerate=output.smoothedspikerate./max(output.smoothedspikerate(:));
        ...
        output.spikes=zeros(size(output.occupancy));
        output.spikerate=zeros(size(output.occupancy));
        output.smoothedspikerate=zeros(size(output.occupancy));
        output.normsmoothedspikerate=zeros(size(output.occupancy));
    end
