function out = JY_getlinstate(animaldir,animalprefix,epochs, includeStates, varargin)
% out = getalllinstate(animaldir,animalprefix,epochs, includeStates,options)
% Produces a cell structure with the fields:
% time, traj, headdir, velocity, mindisttowell
% Head direction and velocity are linearized values.
...
linpos = JY_loaddatastructlinpos(animaldir, animalprefix, 'linpos', loaddays);
for i = 1:size(epochs,1)
    ...
    out{epochs(i,1)}{epochs(i,2)}.mindisttowell = ...
     min(linpos{epochs(i,1)}{epochs(i,2)}.statematrix.linearDistanceToWells, ...
       [], 2);
    [out{epochs(i,1)}{epochs(i,2)}.traj, out{epochs(i,1)}{epochs(i,2)}.lindist] = getlinstate_internal(linpos, epochs(i,1), epochs(i,2), includeStates, varargin{:});
    out{epochs(i,1)}{epochs(i,2)}.headdir = linpos{epochs(i,1)}{epochs(i,2)}.statematrix.segmentHeadDirection(matrixIndex);
    out{epochs(i,1)}{epochs(i,2)}.velocity = linpos{epochs(i,1)}{epochs(i,2)}.statematrix.linearVelocity(matrixIndex);
end
%--------------------------------------------------------------------------

function [traj, lindist] = getlinstate_internal(linpos, day, epoch, includeStates, varargin)
...
traj = ones(length(statematrix.time),1)*-1;
lindist = ones(length(statematrix.time),1)*-1;
traj(validPointsIndex) = statevector;
