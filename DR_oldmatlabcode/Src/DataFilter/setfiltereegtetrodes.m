...
% tetrodes that satisfy the given filter condtions in filterString.
        ...
        for e = 1:size(f(an).epochs{g},1)
             if (iscell(filterString) & (length(filterString) > 1))
                if ~isempty(f(an).data{g}{e})
                    ...
                    f(an).eegdata{g}{e} = feval(filterString{1}, adir, ...
                        aprefix, f(an).epochs{g}(e,:), filterString{2}, filterString{3:end});
                end
            else
                % this is a standard filter
                f(an).eegdata{g}{e} = evaluatefilter( ...
                tetinfo{f(an).epochs{g}(e,1)}{f(an).epochs{g}(e,2)}, ...
                filterString);
            end
        end
    end
