...
% f = singlecellanal(f)
% combination of single cell analysis looped by epoch
% output is organised by epoch
            ...
            for c = 1:size(f(an).data{g}{e},1)
                ...
                if isstruct(fout)
                    if (isempty(f(an).output) | (length(f(an).output) < g))
                        ...
                        f(an).output{g}{e}(end+1) = fout;
                    end
                elseif isnumeric(fout)
                    if ((isempty(f(an).output)) | (length(f(an).output{g}) < e))
                        f(an).output{g}{e} = [];
                    end
                    if (size(fout,1) >= 0) %edited by AS changed from (size(fout,1) == 1)
                        f(an).output{g}{e} = [f(an).output{g}{e}; fout];
                        %f(an).output{g}{e}(end+1) = fout;
