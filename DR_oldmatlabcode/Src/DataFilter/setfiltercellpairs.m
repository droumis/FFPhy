function f = setfiltercellpairs(f, filterStringArray)
% f = setfiltercellpairs(f, filterStringArray)
...
%
% This creates two lists of cells, and the first element of 
% filterStringArray specifies the combination operator to use to create the 
% final list of cells.  
%
% Valid operators are:
...
%		are included.
%	'sametet'  - same as 'allcomb' but only cells from the same tetrode
%		are included.
%
...
for an = 1:length(f)
    ...
    for i = 1:length(f(an).epochs)
        ...
        %f(an).epochs{i}(j,1), f(an).epochs{i}(j,2)
		...
		celllist = celllist(valid,:);
	    case 'sametet'
		% get rid of elements with different tetrode numbers
		valid = (celllist(:,1) == celllist(:,3));
		celllist = celllist(valid,:);
	    end
	    if (length(celllist))
