function f = JY_eegnonreferenceanal(f)
% f = singleeeganal(f)
        ...
        for e = 1:size(f(an).epochs{g},1)
            ...
            for c = 1:size(f(an).eegdata{g}{e},1)
                ...
                for i = 1:length(f(an).function.loadvariables)
                    ...
                    % added to reflect JY specific tasks
                    if strcmp(f(an).function.loadvariables{i},'pos')
                    ...
                    elseif  strcmp(f(an).function.loadvariables{i},'data')
                        ...
                        eval([f(an).function.loadvariables{i},' = loadeegnonreferencestruct(animaldir, animalprefix, f(an).function.loadvariables{i}, tmpindex(1), tmpindex(2), tmpindex(3:end));']);
                        loadstring = [loadstring, f(an).function.loadvariables{i},','];
