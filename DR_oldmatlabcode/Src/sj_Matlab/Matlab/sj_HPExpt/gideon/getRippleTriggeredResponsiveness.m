function [ripTrigResp] = getRippleTriggeredResponsiveness(prefix, day, epoch, riptet, PFCtet, PFCcell, binsize,dospeed, jitter)
%based on sj_HPexpt_ripalign_singlecell_getrip
ripTrigResp={};
ripTrigResp.ind=[day epoch PFCtet PFCcell];
...
% --------------- Parameters ---------------
pret=950; postt=950; %% Times to plot - For 2 sec
trim = 80;
...
orderby=0; %0 for chronologically, 1 for ordering by Hc, 2 for PFC, 
...
bins_resp_control=find(timeaxis>=-500 & timeaxis<=-400);
%bins_resp=bins_resp_control;
...
switch prefix
    case 'Ndl'
        rawdir = '/data15/gideon/NdlData/';
        directoryname = '/data15/gideon/Ndl';
...
for i = 3:length(dayfolders)
    if dayfolders(i).isdir
        if strcmp(daystr,dayfolders(i).name(4:5))
...
load(cellinfofile);
filterString = '$meanrate<7';%' (   strcmp($tag, ''CA1Pyr'') || strcmp($tag, ''iCA1Pyr'')    )'
...
if jitter
    rip_starttime=rip_starttime(1:end-1);
...
nstd = round(binsize*1/binsize); g1 = gaussian(nstd, 3*nstd+1);
...
% if order by Hc cells:
% if orderby==0
%     cellsortidx=1:length(rip_spkshist_cell_Hp);
...
% 
% else
% cellresp = sum(rip_spkshist_cell_PFC(:,bins_resp),2);
% [cellresp_sort,cellsortidx] = sort(cellresp);
% 
% end
...
%[cellresp_sort,cellsortidx] = sort(cellresp);
rip_spkshist_cellsort_PFC = rip_spkshist_cell_PFC.*(1000/binsize);
    
   
pfcresppre = sum(rip_spkshist_cell_PFC(:,bins_resp_control),2);
pfcresp = sum(rip_spkshist_cell_PFC(:,bins_resp),2);
[HtrigResp PtrigResp]=ttest(pfcresp,pfcresppre);
trialResps=pfcresp;
% Re-order raster as well
% rip_spks_cell_PFC=rip_spks_cell_PFC(cellsortidx);

ripTrigResp.h=HtrigResp;
ripTrigResp.p=PtrigResp;
ripTrigResp.trialResps=trialResps;
...
%         plot(xpts , ypts, 'k--','Linewidth',2);
%     end
%     title(['Responsiveness: H= ' num2str(h1) ' P= ' num2str(p1)])
%     % Ticks every 100ms
...
%     % ------
%      figure; hold on;
%      set(gcf, 'Position',[750 465 320 590]);
% %     title(['PFC CellComb Firing Z aligned to ripples-' num2str(binsize) 'ms bins'],...
% %         'FontSize',20,'Fontweight','normal');
