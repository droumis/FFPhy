function [basefr0] = sj_HPexpt_ripalign_singlecell_getrip4(prefix, day, epochs, riptets, Hptets, PFCtet, PFCcell, binsize, saveg1, dospeed)

% Oct 2013. SJ Version 4.
% ------------------------
% This is the version to convert to filter framework.
% For statistics, in Ver 3, I was shuffling entire vector in response window and a random window in background. Dist between vectors was shuffled
% Version 4 - Shuffle mean response in resp window and bck window. Similar to doing a t-test, but this is a non-parametric method. 
% Could also use max/min?. No, Determine type from whether mean resp is > or < than mean bck, and get Peak or Trough after determining significance. 


% Version3 - multiple epochs
% Cannot use raw firing rates for combining across epochs. Have to Z-score or normalize response
% Mean rate of epoch, or of the window I am plotting?
...
%  sj_HPexpt_ripalign_singlecell_getrip3('HPa', 1, [4 6], [],[], 16, 2, 10, 0);
...
cellcountthresh = 3;  % Need to reduce in Familiar Days - For hipp cells
...
if ismember(epochs,[1 3 5 7]),
    dospeed = 'low'; % Always do low speed criterion during rest sessions
    lowsp_thrs = 1; %cm/sec
...
cd(currdir);
% Now Getting Range directly from times file - This is for Cazlibration
...
% USING GETRIP to look at ripples across tetrodes
...
load(cellinfofile);
% MU file used in case MU cluster not defined in spikes file
...
for ep = 1:length(epochs)
    ...
    % Align spikes to random triggers in bck
    % ----------------------------------------
...
% Sorting by hipp resp strength
% --------------------------------------------------

% Try Raw rates instead of Z-scores. For Z-scores, see commented part in Version 1
% Sort by Fir rate in 0 to 100 ms for Hp, and apply same order to PFC/sort PFC by itself AND CONVERT TO FIRING RATES
% Keep a separate matrix for samae order for PFC to see trends over time - this is rip_spkshist_cell_PFC and rip_spkshist_cell_PFC
% % ------------------------------------------------------------------------------------------------------------------------
...
rresp = rip_spkshist_cell_PFC(:,bins_resp);
bckresp = rip_spkshist_cell_PFC(:,bins_bck);
rdm_rresp = rdm_spkshist_cell_PFC(:,bins_resp);
rdm_bckresp = rdm_spkshist_cell_PFC(:,bins_bck);

% Bck
avgbckresp_trial = mean(bckresp,2); % Mean in bck for each ripple
...
% % 1) Significance test - NEW METHOD.
% % USE SHUFFLE BETWEEN MEAN RESP AND MEAN BCK FOR EACH TRIAL
% % ---------------------------------------------------------------------------
...
% Plot the distribution of shuffled values
...
% % 1) Significance test - Old way of using pre-defined peak or trough. COMMENT
% % ---------------------------------------------------------------------------
% 
...
%     peakresp_trial = max(rresp,[],2); % Peak for each ripple. No need to use peak loc
% end
% if strcmp(type,'inh'),
...
% 2) Stats - Shuffle mean vectors in real resp window and random resp window
% --------------------------------------------------------------------------
% Rdm resp
avgrdmresp_trial = mean(rdm_rresp,2); % Mean for each ripple
avgrdmresphist = mean(rdm_rresp); % Avg resp histogram
...
ntr = size(combresp,1); % Ntr rresp + Ntr rdm_rresp. Both must be the same length
nshuffles = 5000;
for shufidx = 1:nshuffles
    order = randperm(ntr);
    ...
    shufresp = shuffle(1:ntr/2,:); shufavgresp = mean(shufresp);
    shufrdm = shuffle((ntr/2)+1:ntr,:); shufavgrdmresp = mean(shufrdm);
    %if shufidx==1, figure; hold on; end
    %plot(shufavgresp,'b'); plot(shufavgrdmresp,'g')
...
% Get the p-value of shuffle. The modulation index for shuffle will be the prctile value
pshuf_vec = length(find(Dshuf>D))/nshuffles;
modln_shuf_vec = 100 - (pshuf*100); %eg p=0.05 => prctile is 95%
...
title(sprintf('%s Day%d Tet%d Cell%d: M %g%s Prc %g%s', prefix, day, PFCtet, PFCcell, roundn(modln_peak,-1),...
...
title(sprintf('%s Day %d Tet %d Cell %d Nspkwin %d', prefix, day, PFCtet, PFCcell, sum(spkcount)),...
...
% % 3) Histogram with Random used for shuffling
% % ----------------------------------------
% figure; hold on; redimscreen_figforppt1;
% set(gcf, 'Position',[960 137 723 446]);
% 
% xaxis = -pret:binsize:postt;
% plot(xaxis,mean(rip_spkshist_cellsort_PFC),'Linewidth',3);
% plot(xaxis,mean(rip_spkshist_cellsort_PFC)+sem(rip_spkshist_cellsort_PFC),'b--','Linewidth',1);
...
% if sig_ttest ==1, str = '*'; else, str = ''; end
% if sig_shuf ==1, str_shuf = '*'; else, str_shuf = ''; end
% title(sprintf('%s Day%d Tet%d Cell%d: M %g%s Prc %g%s', prefix, day, PFCtet, PFCcell, roundn(modln,-1),...
%     str, roundn(modln_shuf,-2), str_shuf),'FontSize',tfont,'Fontweight','normal');
% 
% if saveg1==1,
%     figfile = [figdir,'RippleAlignHistWithShuf_',prefix,'_Day',num2str(day),'_Tet',num2str(PFCtet),'_Cell',num2str(PFCcell)];
%     print('-dpdf', figfile); print('-djpeg', figfile); saveas(gcf,figfile,'fig');
% end
...
% set(gcf, 'Position',[960 658 560 520]);
% % Imagesc
% %--------
...
if (savedata)
    ...
    save(savefile,'allripplemod','allripplemod_idx');
