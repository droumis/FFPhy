function [out, ripplestdout] = sj_getripples_tetinfo(index, ripples, tetinfo, varargin)
%
% SJ - Add tetinfo to input, instead of  cellinfo which is already there.
% Cellinfo method: 'cellfilter', 'isequal($area, ''CA1'') || isequal($area, ''iCA1'')'
% Tetinfo method: 'tetfilter', riptetfilter 
%
%Outputs the starttime, midtime, endtime, and peakvalue (in std) of each ripple, taken from the
%tetrode with the most cells, or given a tetfilter or cellfilter
%index is [day epoch]
%
% options are
...
try
    ...
    disp('Stopped in sj_getripples_tetinfo');
    d,e,
    keyboard;
end
