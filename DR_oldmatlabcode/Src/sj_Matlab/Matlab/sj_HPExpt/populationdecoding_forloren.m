...
%placefieldfilter = setfilterfunction(placefieldfilter, 'calcpopulationlinfields', {'spikes','linpos'},2, 0);
...
%2d place field filter
%Filter creation for place field data
%cellfilter = '(($meanrate < 7))';
%ttask{8}{2}imefilter = { {'getriptimes','($nripples == 0)', [], 'cellfilter', ...
%'((isequal($area, ''CA3''))|(isequal($area, ''CA1'')))'} };
%twodplacefieldfilter = createfilter('animal',animals,'epochs',epochfilter,'cells',cellfilter,'excludetime', timefilter);
%-----------------------------------------------------------

%iterator = 'multicellanal';
%twodplacefieldfilter = setfilteriterator(twodplacefieldfilter,iterator);
%% create a list of all of the place fields without a peak rate threshold
%twodplacefieldfilter = setfilterfunction(twodplacefieldfilter, 'calcpopulation2dfields', {'spikes','pos'},2, 0);

%twodplacefieldfilter = runfilter(twodplacefieldfilter);
...
%viewdecodeevent2([1 2], trainingfilter, decodefilter);  %For loren: this is where you view the events
...
% plot the positions on the maze for each event
load /data/mkarlsso/Bon/bonpos02;
p = pos{4}{2}.data;
o = decodefilter(1).output{1}(2);
sig = zeros(size(o.eventdata));
for i = 1:length(sig)
    if (length(o.eventdata(i).replaystat) == 3)
	sig(i) = (o.eventdata(i).replaystat(3) <= 0.05);
    end
end
sind = lookup(o.eventtime(find(sig), 1), p(:,1));
pind = lookup(o.eventtime(:,1), p(:,1));
figure
h = plot(p(:,2), p(:,3), '.', 'MarkerSize', 4, 'Color', [.7 .7 .7])
hold on
%plot(p(pind,2), p(pind,3), 'g.', 'MarkerSize', 10);
plot(p(sind,2), p(sind,3), 'b.', 'MarkerSize', 15);
axis off
