...
% Same as for run - just do sleep epochs.
% Update - Keep pre and post sleep epochs separate
...
savefile = [savedir 'HP_ripplemodsleep_PFC']; area = 'PFC'; clr = 'b'; % PFC
%savefile = [savedir 'HP_ripplemodsleep_CA1']; area = 'CA1';  clr = 'r';% CA1
%%%savefile = [savedir 'HP_ripplemodpostsleep_PFC']; area = 'PFC'; clr = 'b'; % PFC
...
if runscript == 1
    ...
    %postsleepepochfilter = 'isequal($environment, ''postsleep'')'; % Only pre and post sleep marked as sleep
    ...
    %cellfilter = 'strcmp($tag2, ''PFC'')'; % This includes all, including silent cells
    %cellfilter = 'strcmp($tag2, ''CA1Pyr'') && ($numspikes > 100)'; % This includes all.    % For more exclusive choosing, use $tag. Take care of number of spikes while gathering data
    ...
    % Filter creation
    % ----------------
    modf = createfilter('animal',animals,'days',dayfilter,'epochs',sleepepochfilter, 'cells',...
...
gatherdatafile = [savedir 'HP_ripplemodsleep_PFC_gather']; % PFC cells to Hipp ripples
%gatherdatafile = [savedir 'HP_ripplemodsleep_CA1_gather']; % CA1 cells to Hipp ripples
...
if gatherdata
    ...
    for i=1:length(alldatahist)
        ...
        while rowfind(animdaytetcell,dummyindex(:,[1 2 4 5]))~=0          % collect all rows (epochs)
            ...
            ep_ind = [ep_ind allanimindex(rowfind(animdaytetcell,dummyindex(:,[1 2 4 5])),3)];
        ...
        % Keep presleep separate
        currhistpre=[]; currrasterpre=[]; currNspkpre=0;
        currhist_rdmpre=[]; currraster_rdmpre=[];
        currtrialRespspre=[]; currtrialResps_rdmpre=[]; currtrialResps_bckpre=[];
        
        for rcnt=1:length(ind)
            r = ind(rcnt);
            if ep_ind(rcnt)~=1 % if not pre-sleep, i.e., 1st sleep of the day before run
                ...
                currNspkpre = currNspkpre + all_Nspk(r);
                currhistpre = [currhistpre; alldatahist{r}];
                currrasterpre = [currrasterpre, alldataraster{r}];
                currhist_rdmpre = [currhist_rdmpre; alldatahist_rdm{r}];
                currraster_rdmpre = [currraster_rdmpre, alldataraster_rdm{r}];
                currtrialRespspre = [currtrialRespspre, alldatatrialResps{r}];
                currtrialResps_rdmpre = [currtrialResps_rdmpre, alldatatrialResps_rdm{r}];
                currtrialResps_bckpre = [currtrialResps_bckpre, alldatatrialResps_bck{r}];
            end
        end
        ...
        if currNspk >= 50
            ...
            % Save presleep even if it is empty/ not defined, as long as postsleep is being saved
            allripplemod(cntcells).hist_pre=currhistpre*(1000/binsize); % Convert to firing rate in Hz
            allripplemod(cntcells).raster_pre=currrasterpre;
            allripplemod(cntcells).Nspk_pre=currNspkpre;
            allripplemod(cntcells).hist_rdm_pre=currhist_rdmpre*(1000/binsize); % Convert to firing rate in Hz
            allripplemod(cntcells).raster_rdm_pre=currraster_rdmpre;
            allripplemod(cntcells).trialResps_pre = currtrialRespspre';
            allripplemod(cntcells).trialResps_rdm_pre = currtrialResps_rdmpre';
        end
    end
    ...
    for i=1:cntcells
        ...
        if allripplemod(i).Nspk_pre~=0 % Defined in pre-sleep
            
            currhist = allripplemod(i).hist_pre; %currraster = allripplemod(i).raster;
            currhist_rdm = allripplemod(i).hist_rdm_pre; %currraster_rdm = allripplemod(i).raster_rdm;
            ...
            % Save
            % -----
            allripplemod(i).Dm_pre = Dm;
            allripplemod(i).pshuf_pre = pshuf; allripplemod(i).p_pre = pshuf;
            allripplemod(i).sig_shuf_pre = sig_shuf; allripplemod(i).h_pre = sig_shuf; % Sig or not
            allripplemod(i).sig_ttest_pre = sig_ttest; % Sig or not
            allripplemod(i).modln_shuf_pre = modln_shuf ; % %tile value of shuffle
            allripplemod(i).modln_peak_pre = modln_peak; % %tage peak change over baseline
            allripplemod(i).modln_pre = modln; % %tage mean change over baseline
            allripplemod(i).type_pre = type; % exc or inh
            allripplemod(i).anim_pre = allripplemod(i).index(1); allanim_pre(i) = allripplemod(i).index(1);
            allripplemod(i).days_pre = allripplemod(i).index(2); alldays_pre(i) = allripplemod(i).index(2);
            % Mean resp from histogram in respective window
            allripplemod(i).avghisttrialResps_pre = avgresp_trial;
            allripplemod(i).avghisttrialResps_rdm_pre = avgresprdm_trial;
            allripplemod(i).avghisttrialResps_bck_pre = avgrespbck_trial;
            allsig_shuf_pre(i) = sig_shuf;
            allsig_ttest_pre(i) = sig_ttest;
            
        else % if not defined in presleep
            allripplemod(i).Dm_pre = [];
            allripplemod(i).pshuf_pre = []; allripplemod(i).p_pre = [];
            allripplemod(i).sig_shuf_pre = []; allripplemod(i).h_pre = []; % Sig or not
            allripplemod(i).sig_ttest_pre = []; % Sig or not
            allripplemod(i).modln_shuf_pre = []; % %tile value of shuffle
            allripplemod(i).modln_peak_pre = []; % %tage peak change over baseline
            allripplemod(i).modln_pre = []; % %tage mean change over baseline
            allripplemod(i).type_pre = []; % exc or inh
            allripplemod(i).anim_pre = allripplemod(i).index(1); allanim_pre(i) = allripplemod(i).index(1);
            allripplemod(i).days_pre = allripplemod(i).index(2); alldays_pre(i) = allripplemod(i).index(2);
            % Mean resp from histogram in respective window
            allripplemod(i).avghisttrialResps_pre = [];
            allripplemod(i).avghisttrialResps_rdm_pre = [];
            allripplemod(i).avghisttrialResps_bck_pre = [];
            %allsig_shuf_pre(i) = [];
            %allsig_ttest_pre(i) = [];
...
length(find(allsig_shuf_pre==1))
length(find(allsig_ttest_pre==1))
...
if (figopt1)
    for i=1:cntcells
        ...
        if curridx(1)==2 && curridx(2)==2 && curridx(3)==12 && curridx(4)==1
            ...
            plot(xaxis,mean(rip_spkshist_cellsort_PFC),'k-','Linewidth',5);
...
if 1
    ...
    set(gca,'XLim',[-pret postt]);
    title(sprintf('%s Sleep: Norm histogram for ripple aligned response',area),'FontSize',tfont);
    xlabel('Time(ms)','FontSize',xfont,'Fontweight','normal');
    ...
    text(-pret+50, 0.82, sprintf('UnMod: %d',Nnosig),'FontSize',xfont,'Fontweight','normal');
    
    figfile = [figdir,area,'_Sleep_RippleAlign_PoplnHist'] %redimscreen;
    if savefig1==1,
...
if 1
    ...
    title(sprintf('%s Sleep: Fraction Ripple mod cells',area),'FontSize',tfont);
    ...
    figfile = [figdir,area,'_Sleep_RippleAlign_PertageSigDays']
    if savefig1==1,
        print('-depsc2', figfile); print('-djpeg', figfile); saveas(gcf,figfile,'fig');
    end
    ...
    figfile = [figdir,area,'_Sleep_RippleAlign_NSigDays'];
    if savefig1==1,
        print('-depsc2', figfile); print('-djpeg', figfile); saveas(gcf,figfile,'fig');
    end
