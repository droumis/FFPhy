...
% Here, I will use the filter script from DFSsj_HPexpt_getripalignspiking, and only redo the gather part. 
% Instead of using mean, peak will be used for signficance test. 
% Start by trying CA1
...
%gatherdatafile = [savedir 'HP_ripplemodpeak_PFC_gather']; % PFC cells to Hipp ripples
gatherdatafile = [savedir 'HP_ripplemodpeak_CA1_gather']; % CA1 cells to Hipp ripples
%gatherdatafile = [savedir 'HP_ripplemodpeak_PFC_speed_gather']; % PFC cells to Hipp ripples - low speed criterion
...
if gatherdata
    ...
    % For peak, I am going to redefine the response and background window.
    rwin = [0 100];
    bwin = [-500 -400];
    ...
    for i=1:cntcells
        ...
        maxbckresp_trial = max(bckresp,[],2); minbckresp_trial = min(bckresp,[],2);
        ...
        maxresp_trial = max(rresp,[],2); minresp_trial = min(rresp,[],2);
        ...
        % First step in peak is to determine which extremum to use - max or min
        % Alternative is to do a BARS fit
        % ---------------------------------------------------------------------------
        % Either use difference between mean responses, or find whether +ve or -ve peak is bigger than mean in bck. 
        % Using means here.
         Dm = mean(avgresp_trial) - mean(avgbckresp_trial); % DONT want absolute value.
         if Dm>=0,
             type = 'exc'; fnstr = 'max'; peakresp = max(avgresphist); % Peak in response histogram
         else
             type = 'inh'; fnstr = 'min'; peakresp = min(avgresphist); % Trough in response histogram
         end
  
        % 0) Simple t-test
        % -----------------
        
        cmd = sprintf('typeresp_trial = %sresp_trial;',fnstr); eval(cmd);
        cmd = sprintf('typebckresp_trial = %sbckresp_trial;',fnstr); eval(cmd);
        [sig_ttest, p] = ttest2(typebckresp_trial, typeresp_trial);
        
        
        % % 1) Significance test - USE SHUFFLE BETWEEN PEAK/TR RESP AND PEAK/TR BCK FOR EACH TRIAL
        % % ---------------------------------------------------------------------------
        % Get the actual mean peak difference between bck and resp
        Dm = mean(typeresp_trial) - mean(typebckresp_trial); % DONT want absolute value. want to shuffle.
        
        % Shuffle mean bck and mean resp 1000 times
        comb = [typeresp_trial; typebckresp_trial];
        ntr = size(comb,1);
        nshuffles = 1000;
        for shufidx = 1:nshuffles
            ...
            shufresp = shuffle(1:ntr/2,:); shuftyperesp = mean(shufresp);
            shufbck = shuffle((ntr/2)+1:ntr,:); shuftypebckresp = mean(shufbck);
            %if shufidx==1, figure; hold on; end
            %plot(shuftyperesp,'b'); plot(shuftyperdmresp,'g')
            Dshuf(shufidx) = shuftyperesp - shuftypebckresp;
        end
