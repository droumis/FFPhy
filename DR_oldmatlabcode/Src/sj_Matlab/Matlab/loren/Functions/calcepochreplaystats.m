...
for eventindex = 1:length(decodefilter(animalnum).output{1}(epochnum).eventdata)
	...
	if (length(activespiketimes) >= 5)
	    ...
	    %out = [out; [rstats decodefilter(animalnum).output{1}(epochnum).eventimmobiletime(eventindex) length(activespiketimes) ]];  %decodefilter(animalnum).output{1}(epochnum).std(eventindex)
	    ...
	    out = [out; [rstats decodefilter(animalnum).output{1}(epochnum).eventimmobiletime(eventindex) length(activespiketimes) decodefilter(animalnum).output{1}(epochnum).eventloc(eventindex)]];
	    %out = [out; [rstats decodefilter(animalnum).output{1}(epochnum).eventimmobiletime(eventindex) length(activespiketimes) decodefilter(animalnum).output{1}(epochnum).eventdist(eventindex)]];
	else
	    rstats = NaN;
	end
	decodefilter(animalnum).output{1}(epochnum).eventdata(eventindex).replaystat = rstats;
    end
end
