...
for i = 1:size(epochs,1)
    ...
    if (~isempty(tetlist))
        ...
        times = ht.timerange(1):0.01:ht.timerange(end);
        nhightheta = zeros(size(times));
        for t = 1:length(tetlist)
            ...
            try
                nhightheta = nhightheta + interp1(tlist(:,1), tlist(:,2), times, 'nearest');
            catch
                keyboard
            end
        end
        ...
        clear times;
        out{epochs(i,1)}{epochs(i,2)}.nhightheta = int8(nhightheta)';
    end
end
