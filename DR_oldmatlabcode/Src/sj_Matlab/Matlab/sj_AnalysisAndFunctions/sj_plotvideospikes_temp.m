...
colors(1,:) = [1 0.5 0];
%colorseeg = [0.5, 1, 0; 1, 0, 1]; 
colorseeg = [0.5, 1, 0; 1, 1, 1]; 
...
for frame_idx  = vid_start : vid_end 
  ...
  height = 10; 
  spacing = 15;
  % For eeg -2 or 4 plots
  height2 = 40; %height2 = 15;
  spacing2 = 45; %spacing2 = 20;
  
  % Neuron text
  for k = 1:length(Ks)
    baseline = 100 + k*spacing; 
    th = text(SCALE*5,SCALE * (baseline + height*0.5) ,sprintf('Neuron %d',k));
    set(th,'color',colors(k,:),'fontsize',60,...
      'fontname', 'trebuchet ms', 'fontweight', 'demi','verticalalignment','middle');
...
%       'verticalalignment','middle');
  ...
  for k = 1:length(eegtets)
   baseline = 15 + (k-1)*spacing2; 
    th = text(SCALE*5,SCALE * (baseline + height2*0.5) ,sprintf('EEG %d',k));
    set(th,'color',colorseeg(k,:),'fontsize',60,...
      'fontname', 'trebuchet ms', 'fontweight', 'demi', 'verticalalignment','middle');
  end
...
%     'color', [0.75 0.75 0.75]);
  ...
  line(SCALE*[104 104],SCALE*[100 + spacing - height, 100 + (length(Ks))*spacing + 2*height], ...
    'linewidth', 0.5, 'color', [0.75 0.75 0.75]);

  line(SCALE*[104 104],SCALE*[5 + spacing2 - height2, 100 + (length(eegtets))*spacing2 + height2], ...
    'linewidth', 0.5, 'color', [0.75 0.75 0.75]);
  ...
  for k = 1:length(eegtets)
      ...
      baseline = 32 + (k-1)*spacing2; 
      plot(SCALE* eet, SCALE*(baseline+eed),'color',colorseeg(k,:),'linewidth',1);
  ...
  ff2 = getframe(gca);
  imwrite(ff2.cdata(1:end-1,1:end-1,:),sprintf('video3/2_spkeeg_%03d.png',ii),'png');
...
wavwrite(bb / max(abs(1.2*bb)),44100,'video3/2_REf0704_3secwin.wav');
