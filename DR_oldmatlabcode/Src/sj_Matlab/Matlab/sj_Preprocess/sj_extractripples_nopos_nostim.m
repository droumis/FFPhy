function extractripples(directoryname, fileprefix, day, tetrode, min_suprathresh_duration, nstd, varargin)


%%%% 
%% Takes in Ripple files with Stimln artifact removed and extracts ripples

%% 03/02/2010 
%% Remove position data so dont have to processs p files for looking at
%% ripples 
%% Shantanu Jadhav
%%%%%%
...
for t = tet
    tmpflist = dir(sprintf('*ripplenostim%02d-*-%02d.mat', day, t));
    for i = 1:length(tmpflist)
        ...
        if (thresh > 0) & any(find(renv<baseline))
            ...
            %% 03/02/2010 - Remove Position Reference
...
save(sprintf('%s/%sripples%02dstd%02d.mat', directoryname, fileprefix, d, nstd), 'ripples');
popd
