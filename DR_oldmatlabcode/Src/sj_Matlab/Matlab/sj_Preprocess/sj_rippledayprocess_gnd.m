function sj_rippledayprocess_gnd(directoryname,fileprefix,days, varargin)
% Shantanu: Nov 2012. FOR EEG wrt GND
...
for day = days
    ...
    for fnum = 1:length(flist)
        ...
        ripplegnd{day}{epoch}{tet} = filtereeg2(eeggnd{day}{epoch}{tet}, ...
            ripplefilter, 'int16', 1);
        clear eegrec
        % replace the filtered invalid entries with the minimum int16 value of
        % -32768
        for i = 1:length(lo)
            if lo(i) > 1 & hi(i) < length(ripplegnd{day}{epoch}{tet}.data)
                ripplegnd{day}{epoch}{tet}.data(lo(i):hi(i)) = minint;
            end
        end
        
        % save the resulting file
        ripplefile = sprintf('%s/EEG/%sripplegnd%02d-%d-%02d.mat', ...
            directoryname, fileprefix, day, epoch, tet);
        save(ripplefile, 'ripplegnd');
        clear ripplegnd
    end
end
