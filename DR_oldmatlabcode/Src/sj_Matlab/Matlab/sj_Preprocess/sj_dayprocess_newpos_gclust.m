function toppeak = sj_dayprocess_newpos_gclust(daydirect, animdirect, fileprefix, daynum, varargin)
...
processeeg = 1;
system = 2;
% Pos sampling rate = 60/2/1.001; %29.97
%posfilt = gaussian(29.97*0.5, 60);       % For velocity: 60 points, sigma = 15
posfilt = gaussSmooth(29.97,0.5/sqrt(2),2); % points = 42, sigma = 0.5/sqrt(2), s.d.s=2
varprefix = '';
reversex = 0;
reversey = 0;
processdio = 1;
...
for epoch = 1:length(times)
    if ~isempty(times(epoch).starttime)
        ...
        ['Position data - epoch ', num2str(epoch)]
        ...
        % Instead of interpolation, use Steve's estimate position
        ...
        % Steve's position estimate
        if ~isempty(rawpos{daynum}{epoch}.data)
            position = sj_estimate_position(rawpos{daynum}{epoch},'centimeters_per_pixel',cmperpix);
            pos{daynum}{epoch} = position{1};
            clear position
            pos{daynum}{epoch}.cmperpixel = cmperpix;
            
            % now get velocity. 
            % Either Smooth the Loess estimated position with gaussian (ala Maggie), 
            pos{daynum}{epoch} = sj_addvelocity(pos{daynum}{epoch},posfilt);
            % OR, Smooth the Loess estimated velocity with gaussian (ala Caleb)
            
            % OR, fit it with a global spline with tol=length(data), and tol inversely proportional
            % to Loess velocity estimate. i.e. dont smooth high velocity areas, but do so for low-velocity areas.
            % TO BE IMPLEMENTED
        end
...
%%%%%% Modified to work with gclust files
...
for tet = 3:length(tetfolders)
    ...
    if tetfolders(tet).isdir
        ...
        gclustfile = dir('gclust_*');
        
        if ~isempty(gclustfile)
            for epoch = 1:length(times)
                if ~isempty(times(epoch).starttime)
                    starttime = times(epoch).starttime;
                    endtime = times(epoch).endtime;
                    epochname = times(epoch).name;
                    clear gclustdata
                    load(gclustfile(1).name);
                    starttime2 = timetrans({starttime},10000,2);
                    endtime2 = timetrans({endtime},10000,2);
                    if ~isempty(gclustdata.clusters)
                        for clustnum = 1:length(gclustdata.clusters)
                            ...
                            currclust = gclustdata.clusters(clustnum);
                            clustidxs = find(gclustdata.assigns==currclust);
                            
                            timepoints = gclustdata.params(clustidxs,1);
                            if (size(gclustdata.params,2) > 4)
                                amps = gclustdata.params(clustidxs,2:5);
                            else
                                amps = nan(size(gclustdata.params,1),1);
                            end
                            if (size(gclustdata.params,2) > 5)
                                spikewidth = mean(gclustdata.params(clustidxs,6));
                            else
                                spikewidth = nan(size(gclustdata.params,1),1);
                            end
