function toppeak = sj_dayprocess_cont2_cmpix(daydirect, animdirect, fileprefix, daynum, varargin)


%dr example: 
% sj_dayprocess_cont2_cmpix('/mnt/data25/sjadhav/HPExpt/HPc_day/01_120812/','/mnt/data25/sjadhav/HPExpt/HPc/','HPc',1,'cmperpix', [.1650 .6316 .1650 .6055 .1650 .6055 .1650] );
%HPc cmperpix sleep = .1650 lintrack= .6316 wtrk1= .6055 wtrk2= .4718


% Jun 2013
% Shantanu - add varuable cm/pixel for different epochs. So this expects a
% vector of cm/pixel equal to length epochs in that day
...
%       Shantanu  May 2013
%           EPOCHS_RUN -- Run epochs for feeding into sj_velocitydayprocess
%           MULTI_THRS -- Multiunit threshold
...
processspikes = 1;
processvel = 1;
epochs_run = [2 4]; % Default run epochs are 2 and 4 - Pass argument to change
multi_thrs = 60; % Multi-unit Threshold

spikesonly = 1; % If this is set to 1, eeg,multi,dio,vel is set to 0, and position is not saved.
...
for option = 1:2:length(varargin)-1
    ...
    switch varargin{option}
        ...
        case 'epochs_run'
            epochs_run = varargin{option+1};
        case 'multi_thrs'
            multi_thrs = varargin{option+1};
...
%% If spikesonly, I want to load existing files to get smooth data, but do position anyway as backup in case file does not
...
for epoch = 1:length(times)
    ...
    if ~isempty(times(epoch).starttime)
        ...
        if ~isempty(rawpos{daynum}{epoch}.data)
            ...
            epoch;
            currcmperpix = cmperpix(epoch);
            pos{daynum}{epoch}.cmperpixel = currcmperpix;
            pos{daynum}{epoch}.data(:,2:3) = pos{daynum}{epoch}.data(:,2:3)*currcmperpix;
...
%Commenting Save Postion Data on 19/06/2013 for HPa, HPb, HPc
%------------------------------------------------------------
% if spikesonly~=1    
...
%%%%%% Smoothened position and velocity 0- Takes time
% Commenting for HPa, HPb, HPc
% -----------------------------
% if (processvel)
%     disp('Doing Velocity process');
%     epochs_run
%     [pos] = sj_velocitydayprocess(animdirect,fileprefix, daynum,epochs_run,'cmperpix',cmperpix);    
%     % You just smoothened position. So "pos" returned by the function will have the required extra fields for spike data
%     % Anyway, for spike data, if posfile exists, it will be loaded
% end

cd(currdir); cd(daydirect);
%% Do EEG
if processeeg ==1 %this part filters the eeg data
    ...
    if ~isempty(eegfiles) % if there are eeg files to process, then proceed
            for eegfile = 1:length(eegfiles)
                ...
                % values cen differ for DSPs
                
                % Check if files can be opened
                contfile = eegfilename;
                fid = fopen(contfile);
                if (fid == -1)
...
if processmulti==1
            ...
            if exist(fullfile(tetfolders{tet},sprintf('%s-%02d_params.mat',dayname,tetnum)))
                ...
                for epoch = 1:length(times)
                    if ~isempty(times(epoch).starttime)
                        ...
                        % Note that multi is in units of 0.1 ms. Can convert to secs if needed.                      
                        
                        % Can add thereshold - max amplitude above some value here
                        ampmax = max(rawdata.filedata.params(find((rawdata.filedata.params(:,1) >= starttime) & (rawdata.filedata.params(:,1) <= endtime)),2:5),[],2);
                        goodpoints = find(ampmax>=multi_thrs); % 60/80/100 - Started using 60 on 6th June. Should save amplitude also
                        multi{daynum}{epoch}{tetnum} = multi{daynum}{epoch}{tetnum}(goodpoints);
                        
                        % Also save multiunit times + Max Amplitude in another structure
                        multiwithamp{daynum}{epoch}{tetnum}.data = multi{daynum}{epoch}{tetnum};
                        multiwithamp{daynum}{epoch}{tetnum}.ampmax = ampmax(goodpoints);
                ...
                for epoch = 1:length(times)
                    ...
                    multiwithamp{daynum}{epoch}{tetnum} = [];
                end
    ...
    eval([varprefix,'multiwithamp = multiwithamp;']);
    eval(['save ',fileprefix,'multiwithamp',daystring,' ',varprefix,'multiwithamp']);
end % end processmulti
...
if (processspikes==1 || spikesonly==1)
    ...
    for tet = 3:length(tetfolders)
        ...
        if tetfolders(tet).isdir
            ...
            if ~isempty(matclustfile)
                counttet = counttet + 1; % First tet with matclust file will have counttet=1;
                for epoch = 1:length(times)
                    if ~isempty(times(epoch).starttime)
                        starttime = times(epoch).starttime;
                        endtime = times(epoch).endtime;
                                    ...
                                    if (is_cluster_defined_in_epoch(clustnum,epoch+1))
                                        ...
                                        if ~isempty(pos{daynum}{epoch}.data)
                                            ...
                                            if size(pos{daynum}{epoch}.data,2)>5
                                                ...
                                                if length(intersect(epoch, epochs_run))~=0  % if a run epoch
                                                %if strcmp('run',epochname(1:end-1))
                                                    if counttet==1 && clustnum==1 % Only warn once
                                                        display(['Warning! Using only non-updated original Position Data for run epoch ', num2str(epoch) ,' for spikes']);
                                                    end
                                                end
                                                [spikepos, posindex] = lookuptime3(timepoints/10000, pos{daynum}{epoch}.data(:,1),pos{daynum}{epoch}.data(:,2:4)');
                                                spikepos_sm = [];
                                            ...
                                            if ~isempty(timepoints)
                                                %tag = clustattrib.clusters{clustnum}.tag;
                                                spikes{daynum}{epoch}{tetnum}{clustnum}.data = timepoints/10000; % Converting to seconds
                                            ...
                                            %tag = clustattrib.clusters{clustnum}.tag;
                                            spikes{daynum}{epoch}{tetnum}{clustnum}.data = timepoints/10000; % Converting to seconds
                                            % PRE-AUG2012. WAS NOT SECS here, when no position! Just like multi data. Some of my opto-stim align code assumes units
                                            % of 0.1 ms! eg. sj_multiandspike_align. For JW7
