...
c13 = []; pvalue = []; place = []; immobile = []; numcells = []; numspikes = [];
for an =1:length(valid_ripples)
    for d =1:length(valid_ripples{an})
        for e = 1:length(valid_ripples{an}{d})
            if ~isempty(valid_ripples{an}{d}{e})
                if isfield(valid_ripples{an}{d}{e},'ca1_ca3')
                    if size(valid_ripples{an}{d}{e}.pvalue,1) == size(valid_ripples{an}{d}{e}.ca1_ca3,2) 
                        if size(valid_ripples{an}{d}{e}.pvalue,1)== size(valid_ripples{an}{d}{e}.eventimmobiletime,1)
                            ...
                            numcells = [numcells; valid_ripples{an}{d}{e}.numcells];
                            numspikes = [numspikes; valid_ripples{an}{d}{e}.numspikes];
                        end
                    end
                end
            end
...
%% CONTROL FOR NUMBER OF SPIKES, CELLS ACTIVE, OR BOTH
sig_cells = numcells(pvalue<0.05);non_cells = numcells(pvalue>0.05);
sig_spikes = numspikes(pvalue<0.05);non_spikes = numspikes(pvalue>0.05);
sigc13 = c13(:,pvalue<0.05); nonc13 = c13(:,pvalue>0.05);
invalid = isnan(sig_cells) | isnan(sig_spikes);
sig_cells(invalid) = []; sig_spikes(invalid) = []; sigc13(:,invalid) = [];
invalid = isnan(non_cells) | isnan(non_spikes);
non_cells(invalid) = []; non_spikes(invalid) = []; nonc13(:,invalid) = [];
[val ind] = sort(sig_spikes);
sig_cells = sig_cells(ind); sig_spikes = sig_spikes(ind); sigc13 = sigc13(:,ind);
[val ind] = sort(non_spikes);
non_cells = non_cells(ind); non_spikes = non_spikes(ind); nonc13 = nonc13(:,ind);

count = 1;
while  mean(sig_cells)>mean(non_cells) && mean(sig_spikes)>mean(non_spikes)
    if mod(count,2)
        sig_spikes(end) = []; sig_cells(end) = []; sigc13(:,end) = [];
        count = count+1;
    else
        non_spikes(1) = []; non_cells(1) = []; nonc13(:,1) = [];
        count = count+1;
    end
end
...
Nx = length(sig_spikes); Ny = length(non_spikes);
X = sigc13; Y = nonc13;
Z = cat(2,X,Y);
q = mean(X,2)-mean(Y,2);
...
% Save figure
[y, m, d] = datevec(date);
savestring = sprintf('/home/mcarr/Figures/RipplePaper/%d_%d_%d_ca1ca3_coherence_controlspikescells.pdf', m, d, y);
...
%Significant is greater than nonsignificant for 50-180ms following SWR
%detection, p < 0.05 even when controlling for the number of spikes in each
%SWR and the number of cells participating
...
% Save figure
[y, m, d] = datevec(date);
savestring = sprintf('/home/mcarr/Figures/RipplePaper/%d_%d_%d_ca1ca3_phase_controlspikescells.pdf', m, d, y);
print('-dpdf', savestring)
