            ...
            if ~isempty(decodefilter(an).output{d}(e).index)
                ...
                if ~isempty(training_e)
                    ...
                    if ~isempty(out)
                        ...
                        decodefilter(an).output{d}(e).pvalue(eventind) = out(:,4);
                        ...
                        decodefilter(an).output{d}(e).entropy(eventind) = out(:,3);
                        decodefilter(an).output{d}(e).replaylength = NaN(size(ind));
                        decodefilter(an).output{d}(e).replaylength(eventind) = out(:,5);
                    end
                end
            end
        end
    ...
    for d = 1:length(decodefilter(an).output)
        for e = 1:length(decodefilter(an).output{d})
            if ~isempty(decodefilter(an).output{d}(e).index(1,1))
                decode_day = decodefilter(an).output{d}(e).index(1,1);
                training_e = find(decode_day==trainingfilter(an).epochs{2}(:,1));
                if ~isempty(training_e)
...
%% DETERMINE WHICH SLEEP EVENTS ARE REPLAY EVENTS
load '/data13/mcarr/RipplePaper/decodefilterB_sleep.mat'
decodefilter = decodefilterB_sleep; clear decodefilterB_sleep
for an = 1:length(decodefilter)
    for d = 1:length(decodefilter(an).output)
        for e = 1:length(decodefilter(an).output{d})
            %Determine which training index to use
            if ~isempty(decodefilter(an).output{d}(e).index)
                ...
                if ~isempty(training_e)
                    out = calcepochreplaystats([an d e], [1 training_e], trainingfilter, decodefilter);
                    if ~isempty(out)
                        ind = zeros(size(decodefilter(an).output{d}(e).eventtime,1),1);
                        eventind = out(:,6);
                        decodefilter(an).output{d}(e).pvalue = NaN(size(ind));
                        decodefilter(an).output{d}(e).pvalue(eventind) = out(:,4);
                        decodefilter(an).output{d}(e).rvalue = NaN(size(ind));
                        ...
                        decodefilter(an).output{d}(e).entropy(eventind) = out(:,3);
                        decodefilter(an).output{d}(e).replaylength = NaN(size(ind));
                        decodefilter(an).output{d}(e).replaylength(eventind) = out(:,5);                    end
                end
            end
        end
    end
end
decodefilterB_sleep = decodefilter; clear decodefilter;
save('/data13/mcarr/RipplePaper/decodefilterB_sleep.mat','decodefilterB_sleep');

load '/data13/mcarr/RipplePaper/decodefilterA_sleep.mat'
decodefilter = decodefilterA_sleep;
for an = 1:length(decodefilter)
    for d = 1:length(decodefilter(an).output)
        for e = 1:length(decodefilter(an).output{d})
            if ~isempty(decodefilter(an).output{d}(e).index);
...
decodefilterA_sleep = decodefilter; clear decodefilter

save('/data13/mcarr/RipplePaper/decodefilterA_sleep.mat','decodefilterA_sleep');
