function [out, ripplestdout] = getripples(index, ripples, cellinfo, varargin)
%
%Outputs the starttime, midtime, endtime, and peakvalue (in std) of each ripple, taken from the
%tetrode with the most cells
%index is [day epoch]
%
% options are
...
%   'tetrodes', [list of tetrodes]
%           specifies a list of tetrodes to use
%
...
elseif isempty(tetfilter)
    ...
    tetlist = evaluatefilter(cellinfo{d}{e},tetfilter);
    tetlist = unique(tetlist(:,1))';
end
ripplestdout = [];
...
if (maxcell)
    ...
    for t = 1:length(tetlist)
        ...
        if isfield(tmprip,'maxthresh') && length(tmprip.startind)>1
            ...
            rtimes(:,2) = 1;
            tmpripplestd = [rtimes(:,1) tmpripplestd(:)];
           ...
           try
                ...
                ripplestdout = ripplestdout + interp1(stdlist(:,1), stdlist(:,2), times, 'nearest');
           catch
                keyboard
           end
        end
    ...
    if ((length(startrippleind) > 1) & (length(endrippleind) > 1))
        ...
        peaktime = zeros(length(startrippleind),1);
        ripstd = ripplestdout(startrippleind);
        for i = 1:length(startripple)
            if max(nrip(startrippleind(i):endrippleind(i)))==1
                peaktime(i,1) = mean([startripple(i) endripple(i)]);
            else
                tmp = [find(max(nrip(startrippleind(i):endrippleind(i)))==nrip(startrippleind(i):endrippleind(i)),1,'first') ...
                    find(max(nrip(startrippleind(i):endrippleind(i)))==nrip(startrippleind(i):endrippleind(i)),1,'last')];
                peaktime(i,1) = mean(times([startrippleind(i)+tmp(1) endrippleind(i)-tmp(2)]));
            end
        end
        
        out = [startripple(:) endripple(:) peaktime(:)];
        ripplestdout = ripstd(((out(:,2)-out(:,1))>.050));
