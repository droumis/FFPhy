function extractslowripples(directoryname, fileprefix, day, tetrode, min_suprathresh_duration, nstd, varargin)

%function extractslowripples(directoryname, fileprefix, day, tetrode_list, %%min_suprathresh_duration, nstd, maxpeakval, options)
%
...
for j = 1:length(tet)
    t = tet(j);
    tmpflist = dir(sprintf('*slowripple%02d-*-%02d.mat', day, t));
    ...
    for i = 1:length(tmpflist)
        ...
        renv = double(slowripple{d}{e}{t}.data(:,3));
        
        % smooth the envelope:
        samprate = slowripple{d}{e}{t}.samprate;
        ...
        if (thresh > 0) & any(find(renv<baseline))
            ...
            rip.starttime = slowripple{d}{e}{t}.starttime + rip.startind / samprate;
            rip.endtime = slowripple{d}{e}{t}.starttime + rip.endind / samprate;
            rip.midtime = slowripple{d}{e}{t}.starttime + rip.midind / samprate;
            rip.peak = tmprip(:,3);
            rip.energy = tmprip(:,7);
        ...
        rip.timerange = [0 length(renv)/samprate] + slowripple{d}{e}{t}.starttime;
        rip.samprate = slowripple{d}{e}{t}.samprate;
        ...
        %ripple
        clear feeg
        slowripples{d}{e}{t} = rip;
        clear rip;
    end
end
save(sprintf('%s/%sslowripples%02d.mat', directoryname, fileprefix, d), 'slowripples');
popd
