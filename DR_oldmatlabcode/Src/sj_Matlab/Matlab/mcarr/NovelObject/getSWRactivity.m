function out = getSWRactivity(indices, excludeperiods, spikes, task, ripples, cellinfo ,varargin)
%
...
if ~isempty(evaluatefilter(cellinfo{index(1)}{index(2)},'isequal($area,''CA1'')'))
    ...
    out.totalduration = task.time(end)-task.time(1) - sum(excludeperiods(:,2)-excludeperiods(:,1));

    %Where do the ripples occur
    time = lookup(riptimes(:,1),task.time);
    out.SWRlocation = [task.quadrants(time) task.objects(task.quadrants(time))' task.novel(task.quadrants(time))'];
