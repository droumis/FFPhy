function out = filtercalclinfields(index, excludeperiods, spikes, linpos, varargin)
%trajdata = FILTERCALCLINFIELDS(index, excludeperiods, spikes, linpos)
%trajdata = FILTERCALCLINFIELDS(index, excludeperiods, spikes, linpos, options)
%
...
%   'includestate' includestates from getbehavestate. Default is 6.
%
...
for option = 1:2:length(varargin)-1   
    if isstr(varargin{option})       
        switch(varargin{option})
            ...
            case 'includestate'
                includestate = varargin{option+1};
	    otherwise
...
if iscell(statematrix.traj)
    statevector = statematrix.traj{includestate};
else
...
for i = 1:length(trajdata)
    ...
    if ~isempty(tmplinloc)
        ...
        nonfinite = ~isfinite(binvector(:,4));
        ...
        lowocc = binvector(:,6) < binsize*.1; %very low occupancy bins should be excluded
