function spikes = ssm_energy_subset(spikes, nch, Subset)
...
normsqr = sum(spikes.waveforms.^2,2);
...
for clust1 = 1:numclusts
	...
	dists = pairdist(spikes.waveforms(pts{clust1},:), 'reuse');
	interface_energy(clust1,clust1) = fast_interface_energy(dists,scale);
	k = k + 1;
	for clust2 = (clust1+1):numclusts   % now for the rest ...
		dists = pairdist(spikes.waveforms(pts{clust1},:), spikes.waveforms(pts{clust2},:), 'reuse');
