function varargout = ssg_ampfeatureselect(varargin)
% SSG_AMPFEATURESELECT M-file for ssg_ampfeatureselect.fig
%      SSG_AMPFEATURESELECT(axes_handle) creates a new SSG_AMPFEATURESELECT.  Not
...
gui_State = struct('gui_Name',       mfilename, ...
                   ...
                   'gui_OpeningFcn', @ssg_ampfeatureselect_OpeningFcn, ...
                   'gui_OutputFcn',  @ssg_ampfeatureselect_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before ssg_ampfeatureselect is made visible.
function ssg_ampfeatureselect_OpeningFcn(hObject, eventdata, handles, varargin)
...
function varargout = ssg_ampfeatureselect_OutputFcn(hObject, eventdata, handles)
...
switch (choice), 
    case 'Par'
        vector = ssg.ss_object.matclustparams(:,param1);
        
    case 'Amp', 
        vector = ssg.ss_object.ampl(:,param1);
        
    case 'Signal', 
        if ~isfield(ssg.ss_object,'matclustparams')
    ...
    case 'PC', % (for principal components, we might need to compute the first time)
        if (~isfield(ssg.ss_object, 'pcadata'))
    ...
    case 'Event Time',
        ...
        vector = ssg.ss_object.fstimes./10000;  
        
    case 'ISI Preceding',
        vector = [NaN; diff(ssg.ss_object.fstimes)];
