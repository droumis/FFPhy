...
% else
    ...
    cutoff = lambda(max(find(pdfDstat < 0.97))); %%% %% SHANTANU - THIS RAISES CUTOFF
...
%[ IGNORE
%%%% SHANTANU: THIS IS NOT THE STATISTIC AS IN THE PAPER, AS WE ARE JUST
%%%% CALCUATING FRACTION OF SPIKES BELOW TREF, AND NORMALIZING BY RELATIVE
%%%% LENGTH OF INTERVAL TMIN-TREF, AS COMPARED TO TREF-TMAX. THIS IS
%%%% (1/f)*F(TREF)

%%%% WHAT I NEED IS TO ALSO CALCULATE FRACTION BETWEEN TREF-TMAX, AND
%%%% NORMALIZE AS IN PAPER: 
%%%% R2/10 = (1/f)*(F(TREF)/F(TMAX))

% nscores =  (cdfs(:, refractory_bin)) ./ (cdfs(:, max_bin)) ;
% nscores = nscores/f;]
