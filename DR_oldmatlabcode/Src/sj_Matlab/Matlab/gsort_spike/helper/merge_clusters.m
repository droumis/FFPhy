...
%%%SHANTANU - JULY 2006
%%%SHANTANU - oct 2007 - Update
%%%THIS UPDATE IS NOT NECESSARY IF YOU USE "UPDATE ENERGY TAB" -> SSS_UPDATE_INTENERGY 
[spikes, connect_strength]= sss_update_intenergy_onmerge(spikes, to, from);
%% NEW FIELD
spikes.hierarchy.connect_strength = connect_strength;
cs=connect_strength(to,from);

%%%%%%%%%% MODIFY AGGREGATION TREE
tmin = size(spikes.waveforms,2)./Fs;  tref = max(0.002, tmin*1.5);
tmin = 0.002; tref = 0.003;
if (isfield(spikes, 'spiketimes'))   % might as well compute the isi score if we can ...
    t1 = spikes.fstimes(orig_members_to)/1000;
    t2 = spikes.fstimes(members_from)/1000;
