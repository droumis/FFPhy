function spikes = ssm_kmeans(spikes, options, nch, Subset)
...
%   Last Modified By: sbm on Fri Oct  7 01:28:36 2005
...
for rep = 1:opts.reps                                 % TOTAL # REPETITIONS
	...
	for iter = 1:opts.divisions                       % # K-MEANS SPLITS
		...
		while (~(assign_converge | mse_converge))     % convergence?
            ...
            if (progress)
                progressBar(((M - changed_assigns)/M).^10, 5, ['Calculating ' num2str(2^iter) ' means.']); % rough ...
            end
