function [spikes,aspkfile]=sss_spkcut_plexon_labviewcont1(files, ch, Fs, sdcuts, stimcode, params, consweep, sweepall, sweepstim)
...
for lp_f=1:size(files,1)
    ...
    if lp_f==1 % if the first cycle then calculate the threshold
        ...
        tempthr= [mean(mean(data(1:320000,1))) mean(std(data(1:320000,1)))];
