function [spikes,aspkfile]=sss_spkcut_plexon_allch_labviewcont1(files, ch, Fs, sdcuts, stimcode, params, consweep, sweepall, sweepstim)
...
longsp=0; bsize=1; spikestarts=10;
%% SHIFT CHANNELS %%%%%%%%%%
%ch=ch+1;
%% SHIFT CHANNELS %%%%%%%%%%
...
for lp_f=1:size(files,1)
    ...
    %%% 10min files: DIVIDE INTO 5MIN SEGMENTS FOR LAODING; INPUT HAS TO
    %%% HAVE FILE NAME REPEATED TWICE
    if (mod(lp_f,2)==1)
        t1=0; t2=300;
    else
        t1=300; t2=600;
    ...
    data=labview_loadspike4(fn,8,32000,t1,t2);  %% NEED TO READ ALL 8 channels
    ...
    if lp_f==1 % if the first cycle then calculate the threshold
        ...
        tempthr= [mean(mean(data(1:32000,1))) mean(std(data(1:32000,1)))];
...
%             thr_index(thrdiff+1)=[];
            ...
            spktime= round(thr_index * 0.3125); % 0.3125 constant converts the spiketimes from 32kHz sampling to 0.1 ms
...
%             spktime=roundn(spktime,-1); %% Resolution of 0.1 ms
        ...
        allspikes.swtimes=[allspikes.swtimes;fstimes/1000]; allspikes.fstimes=[allspikes.fstimes;fstimes];  %%% UPDATED FSTIMES
        ...
        save ([ name1 '_' [num2str(lp_f)] '_tet_thr' [num2str(round(sdcuts(lpthrs)))]], 'spikes')
...
spikes.Fs=Fs; spikes.threshT=spikestarts; spikes.threshV=[thrs Inf];
spikes.stimonset=stimonset; spikes.window=bckwindow;spikes.sweepd=sweepd;
