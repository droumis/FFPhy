function sss_amptest(spikes, assignments, show, mode, nch)
...
for clu = 1:numclusts
	...
	inds{clu} = sel;
    if (strcmp(mode, 'xyz'))
        hndl = plot3(data(sel,1), data(sel,2), data(sel,3),'.','MarkerSize',2);
    else
        hndl = plot(data(sel,1), data(sel,2), '.','MarkerSize',2);
        set(hndl, 'ButtonDownFcn', {@raise_me, hndl});
    end
	
	if (clusters(clu) == 0),                 set(hndl, 'Color', Clgy, 'Marker', '.','MarkerSize',0.5);
