function [spikes,aspkfile]=sss_spkcut_matlabcont3(files, ch, Fs, sdcuts, threshold, stimcode, params, consweep, sweepall, sweepstim)

%%% ADDED THRESHOLD FIELD: IF THRESHOLD=0, THEN +VE THRS WITH SPIKESTARTS=14, AND IF
%%% THRESHOLD=14, THEN -VE THRS WITH SPIKESTARTS = 10.
...
if (nargin < 3), Fs=30000; end % sampling frequency in Hz.
...
for lp_f=1:size(files,1)
    ...
    for lpthrs=1:length(thrs);
        ...
        %%%ORDER-OF-MAGNITUDE-HIGHER SPIKETIMES
        if lp_f==1
        ...
        save ([ name1 '_cut_thr' [num2str(round(sdcuts(lpthrs)))]], 'spikes')
        ...
        if consweep==1, addsweep =  maxsweep; addtrial = addtrial + 1; end
...
spikes.stimonset=stimonset; spikes.window=bckwindow;spikes.sweepd=sweepd;
spikes.nsweeps=floor(max(spikes.fstimes)/spikes.sweepd);

aspkfile = (['all_' recname '_thr' [num2str(round(sdcuts(lpthrs)))]]);
save ([ recname 'cut_all_thr' [num2str(round(sdcuts(lpthrs)))]], 'spikes');
