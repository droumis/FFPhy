function spikes = ssm_dejitter_tet(spikes, maxshift, nch)
%% Shantanu, for tet - dejitter on each channel separately for now
...
for ch=1:nch
    ...
    cmd=sprintf('swaveforms = spikes.waveforms_ch%d;',ch); eval(cmd);

    %%%%%%%%%% FIND FIDUCIALS
    % first, get a mask over the peaks
    isThreshLo = (swaveforms(:, spikes.threshT+1) < spikes.threshV(1));   %% SJ - change to threshT+1
    ...
    %%MY CHANGE: SHANTANU: FEB 2006**********ERROR IN -VE OR +VE THRESHOLDS
    %waves( ~isThreshLo, :) = spikes.threshV(1) - waves( ~isThreshLo, :);
    ...
    pp = spline([1:numsamples], swaveforms);
    ...
    % example = 1;
    % figure;  cla;  plot(swaveforms(example,:), 'm.-', 'LineWidth', 4);  hold on;
    ...
    pp.coefs(numsamples,:,4) = swaveforms(:,end)';
    ...
    swaveforms = resampled(:, left_valid:right_valid);
    ...
    meanlevel = mean(swaveforms(:));   % estimate of baseline
    notallowed = [1:(spikes.threshT-maxshift-1),(spikes.threshT+maxshift+1):(size(swaveforms, 2))];

    deflection = mean(abs(swaveforms - meanlevel), 1);  % mean deviation from baseline
    deflection(notallowed) = 0;
    [junk, spikes.threshT] = max(deflection);  % max deflect near old threshT marks new threshold marker
    threshValues = swaveforms(:, spikes.threshT);  % values at new marker
    ...
    store_waveforms =[store_waveforms; swaveforms];
    
end

spikes.waveforms = store_waveforms;
