...
times = spikes.fstimes/1000; % in seconds
...
tmin = size(spikes.waveforms,2)./Fs;
...
for root = 1:length(treeroots)
    ...
    line([x1 x1],[y1 y2], 'LineWidth', 2, 'Color', [0 0 0]);
    text(x1 * 0.95, y2 * 1.03, ['#=' num2str(clustsize)], 'Color', [1 0 0], 'FontSize', 8);
    text(x1 * 1.01, y2 * 0.95, ['isi=' num2str(scores(1),2)], 'Color', [0 0 1], 'FontSize', 8);
    text(x1 * 0.90, y2 * 1.06, ['Clust #' num2str(oldname)], 'Color', [0 0.5 0.5], 'FontSize', 8);
end

% Prettify the axes
hold off;
colormap(cmap);
if (length(leaflabels) <= 64)
