...
function [statematrix, segmenttable, trajwells, wellSegmentInfo, segmentInfo] = linearizeposition(directoryname,fileprefix, index, varargin)
...
for option = 1:2:length(varargin)-1
    ...
    switch varargin{option}
        ...
        case 'validtransitions'
            validtransitions = varargin{option+1};          
        case 'welldist'
...
pos = pos{index(1)}{index(2)}.data(:,1:5);
pos = pos(~isnan(pos(:,2)),:);
...
lastvalid = -1;
lastsegind = -1;
segtrans = ones(length(segmentInfo.segmentLength),1);
vtrans = ones(length(segmentInfo.segmentLength),1);
for i = 1:size(pos(~isnan(pos(:,2))),1)
	...
	warning(sprintf('zero position at index %d, animal %s, day epoch %d %d', i, fileprefix, index(1), index(2)));
	    ...
	    % figure out which segments represent valid transitions
            if (~isempty(validtransitions))
                segtrans(:,1) = tmppos(:,5);
                trans = validtransitions(validtransitions(:,1)==lastsegind,:);
                for v = 1: size(tmppos,1)
                    vtrans(v) = sum(segtrans(v)==trans(:,2));
                end
            end
            % the next point is the first point in the list with a
            % corresponding velocity less than maxv      
            newind = min(find((vel < maxv) & (segdiff <= maxsegdiff) & vtrans));
        else
