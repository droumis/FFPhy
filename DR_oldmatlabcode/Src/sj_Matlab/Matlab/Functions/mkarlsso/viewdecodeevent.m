...
b(9) = uicontrol('Tag','makeseriesbutton','Style','pushbutton','Units','pixel','Position',[57 40 50 20],'String','Series','Callback','viewdecodeevent(''makeseriesbutton_Callback'',gcbo,guidata(gcbo));');
...
p = fighandles.p;
 ...
 if isempty(currentfiguredata)
     ...
     spikeaxis = axis(figuredata(1,5));

     epoch = fighandles.decodefilter(fighandles.index(1)).epochs{1}(fighandles.index(2),:);
     animalinfo = fighandles.decodefilter(fighandles.index(1)).animal;
     ...
     loadfile = [animalinfo{2},'EEG/',animalinfo{3},'eeg',daystr,'-',num2str(epoch(2)),'-05.mat'];
     load(loadfile);
     eeg = eeg{epoch(1)}{epoch(2)}{5};
     times = 1:length(eeg.data);
     times = eeg.starttime+((times-1)*(1/eeg.samprate));
     figuredata(1,6) = axes('position',[.1 .35 .3 .1]);
     datarange = [spikeaxis(1)-1 spikeaxis(2)+1];
     goodind = find((times >= datarange(1)-1)&(times <= datarange(2)+1));
     plot(times(goodind),eeg.data(goodind));
     axis([datarange(1) datarange(2) min(eeg.data(goodind)) max(eeg.data(goodind))]);
     hold on;

     loadfile = [animalinfo{2},'EEG/',animalinfo{3},'theta',daystr,'-',num2str(epoch(2)),'-05.mat'];
     load(loadfile);
     theta = theta{epoch(1)}{epoch(2)}{5};
     times = 1:length(theta.data);
     times = theta.starttime+((times-1)*(1/theta.samprate));
     goodind = find((times >= spikeaxis(1)-1)&(times <= spikeaxis(2)+1));
     plot(times(goodind),theta.data(goodind),'r');

     loadfile = [animalinfo{2},'EEG/',animalinfo{3},'ripple',daystr,'-',num2str(epoch(2)),'-05.mat'];
     load(loadfile);
     ripple = ripple{epoch(1)}{epoch(2)}{5};
     times = 1:length(ripple.data);
     times = ripple.starttime+((times-1)*(1/ripple.samprate));
     goodind = find((times >= spikeaxis(1)-1)&(times <= spikeaxis(2)+1));
     figuredata(1,7) = axes('position',[.1 .2 .3 .1]);
     plot(times(goodind),ripple.data(goodind,1));
     axis([spikeaxis(1)-1 spikeaxis(2)+1 -150 150]);
     hold on;

     loadfile = [animalinfo{2},animalinfo{3},'pos',daystr,'.mat'];
     load(loadfile);
     pos = pos{epoch(1)}{epoch(2)};
     figuredata(1,8) = axes('position',[.1 .05 .3 .1]);
     goodind = find((pos.data(:,1) >= spikeaxis(1)-5)&(pos.data(:,1) <= spikeaxis(2)+5));
     plot(pos.data(goodind,1),pos.data(goodind,5));
     axis([spikeaxis(1)-5 spikeaxis(2)+5  0 40]);

     figuredata(1,9) = axes('position',[.1 .85 .15 .15]);
     plot(pos.data(:,2),pos.data(:,3),'.');
     hold on;
     %plot(pos.data(goodind,2),pos.data(goodind,3),'y.');
     plot(pos.data(goodind(1:round(length(goodind)/2)),2),pos.data(goodind(1:round(length(goodind)/2)),3),'r.','MarkerSize',10);
     plot(pos.data(goodind(round(length(goodind)/2:end)),2),pos.data(goodind(round(length(goodind)/2):end),3),'g.','MarkerSize',10);
     plot(pos.data(goodind(round(length(goodind)/2)),2),pos.data(goodind(round(length(goodind)/2)),3),'k.','MarkerSize',10);
     ...
     figure
     tmptickplot = [];
     load('/data19a/mkarlsso/Bon/bonspikes04');
     load('/data19a/mkarlsso/Bon/bonlinpos04');
     for i = 1:length(fighandles.eventcellsactive)
     ...
     plot(tmptickplot(:,1),tmptickplot(:,2),'+');
     hold on
