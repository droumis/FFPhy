...
for eventindex = 1:length(decodefilter(animalnum).output{1}(epochnum).eventdata)
    ...
    if ((endevent-startevent) < 2)
        ...
        if (length(activespiketimes) >= 4)
            ...
            out = [out;index(1)];
            %out = [out; [calcReplayStats(activespiketimes,activerates,timebins,trainingfilter(animalnum).output{trainingindex}(epochnum).dist) decodefilter(animalnum).output{1}(epochnum).eventimmobiletime(eventindex) length(activespiketimes) decodefilter(animalnum).output{1}(epochnum).eventdist(eventindex)]];
        end
    end
end