...
if plotxcorr == 1;
    if (sum(xc.c1vsc2) > 10) %min number coincident event
        ...
        figure(2)
        [statevector, lindist] = getbehavestate(linpos, ind(1), ind(2), 6);
        lf1 = calclinfields(spikes,statevector,lindist,linpos, ind(1:4));
        lf2 = calclinfields(spikes,statevector,lindist,linpos, [ind(1:2) ind(5:6)]);
...
if (appendindex)
    out = [index ec rms probcoa coactivez];
else
    out = [ec rms probcoa coactivez xc.nspikes1 xc.nspikes2];
end
