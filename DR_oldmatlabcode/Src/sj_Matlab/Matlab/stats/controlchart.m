...
%   $Revision: 1.1.6.3.2.1 $  $Date: 2007/01/16 03:37:56 $
...
for jplot=1:nplots
    ...
    setdatacursorinfo(handle(h(1)),ind,label,pts<lcl,pts>ucl,ruletxt);
...
if nargout>0
    ...
    function resizer(varargin)
        ...
        if ~isempty(allax) && ishandle(allax(1))
...
nestedstr = getappdata(handle(nestedobj),'ccinfo');
txt = '';
...
if ~isempty(parent)
    if ~(isscalar(parent) && ishandle(parent) && ...
