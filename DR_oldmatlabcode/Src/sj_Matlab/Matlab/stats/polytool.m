...
% $Revision: 2.31.4.5 $  $Date: 2006/12/15 19:30:15 $
...
case 'edittext',
    if isempty(newx) || isnan(newx)
      entry = get(x_field,'String');
      newx = get(x_field,'Userdata');
      set(x_field,'String',num2str(double(newx)));
      msg = sprintf('Invalid X value:  %s\nResetting to previous value',entry);
      localerrordlg(msg);
      return;
    end
...
case 'change_degree',
    ...
    if degree >= maxdf+1
       ...
       localerrordlg('Cannot fit model. Resetting to previous value.');
       return;
    end   
    if isempty(degree) || isnan(degree)
       degree = get(degreetext,'Userdata');
       set(degreetext,'String',num2str(double(degree)));
       localerrordlg('The polynomial degree must be a positive integer. Resetting to previous value.');
       return;
    end
...
warndlg(msg,'Polynomial Fitting','modal')
