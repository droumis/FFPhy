...
%   $Revision: 1.1.6.11 $  $Date: 2006/12/15 19:33:02 $
...
list2(1) = handle.listener(ax, findprop(handle(ax),'xlim'), ...
                           'PropertyPostSet', {@localupdatecurves ax});
dfgetset('axlistener',list2);


% ---------------------- function to react to axis limit changes
function localupdatecurves(prop,event,ax)
xlim = get(ax,'xlim');
fitdb = getfitdb;
ft = down(fitdb);
