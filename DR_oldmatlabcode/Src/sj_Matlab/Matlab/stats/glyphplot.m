...
%   $Revision: 1.1.6.8 $ $Date: 2006/10/02 16:34:21 $
...
elseif ~isempty(grid)
    if isnumeric(grid) && (numel(grid)==2) && all(grid>0) && all(grid==floor(grid))
        if isequal(page,'scroll')
            % Make a new inaccesible figure if they've asked for scrolling
...
h = NaN(n,2);
for i = 1:n
    h(i,:) = face(X(i,:),indices(i),ctrx(i),ctry(i),scale,axesh,plotArgs);
end
if ~holdState
    hold(axesh,'off');
