...
%   $Revision: 1.9.2.10 $  $Date: 2006/11/11 22:54:57 $
...
for j=1:nterms
   ...
   if isnested
      % First get dumm variables for this term and its nesters
...
% full represenatation for a nested term
...
for varidx = 1:length(vars)
   ...
   thispwr = pwrs(varidx);         % power of this variable
   tm(varnum) = 0;                 % term without this variable
   pwrs(varidx) = [];              % powers without this variable
   df0 = df0 * varinfo.df(varnum); % d.f. so var
