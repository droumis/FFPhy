...
%   $Revision: 1.6.2.5 $  $Date: 2006/11/11 22:55:10 $
...
if (donames && ~isempty(xnam))
   for j=1:cols
      ...
      h = text((xlimmin(1,j)+xlimmax(1,j))/2, (ylimmin(j,1)+ylimmax(j,1))/2, ...
               xnam{j}, 'HorizontalAlignment','center','VerticalAlignment','middle');
   end
else
   if ~isempty(xnam)
