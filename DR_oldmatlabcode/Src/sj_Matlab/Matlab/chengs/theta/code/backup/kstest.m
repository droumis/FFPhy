...
if (nargin >= 2) & ~isempty(CDF)
   ...
   ydiff = diff(yCDF);
   if (any(ydiff<0))
      error('Second argument (CDF) must define an increasing function of X.');
   end
   ...
   if (~isempty(dups))
      if (~all(ydiff(dups) == 0))
         error('Second argument (CDF) must not have duplicate X values.');
      end
      xCDF(dups) = [];
      yCDF(dups) = [];
   end
