function markRipples(doripples)
%  
%  Mark timesteps during which ripples occured for each epoch.
...
for ie=1:nep
    ...
    feeg= filtereeg(eegstruct, filter);

    if doripples  % find ripples
    ...
    else % find time with elevated gamma power
        gamma{d}{e}.elev= findelev(feeg, nstd, time);
        gamma{d}{e}.nstd= nstd;
        save(sprintf('%s/%s/data2/gamma_%dstd.mat', root,rat,nstd), 'gamma');
    end
end
