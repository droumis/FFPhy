...
%savefile = [savedir 'HP_xcorrmeasures_CA1PFC']; area = ''; % CA1 vs PFC
savefile = [savedir 'HP_xcorrmeasures_PFC']; area = 'PFC'; % PFC vs PFC
%savefile = [savedir 'HP_xcorrmeasures_CA1']; area = 'CA1'; % CA1 vs CA1
...
%savefile = [savedir 'HPa_xcorrmeasures_day2_CA1PFC2']; %
%savefile = [savedir 'HPa_xcorrmeasures_day2_CA1only2']; %
%savefile = [savedir 'HPa_xcorrmeasures_day8_CA1PFC2']; %
%savefile = [savedir 'HPa_xcorrmeasures_day8_CA1only2']; %
...
if runscript == 1
    ...
    % Run Corr - Correlations during theta. linvel and noripple criterion. Can also use gethighthetapower2
    ...
    runcorr_theta = setfilterfunction(runcorr_theta, 'DFAsj_calcxcorrmeasures', {'spikes'},'forripples',0);
    
    runcorr_rip = setfilterfunction(runcorr_rip, 'DFAsj_calcxcorrmeasures', {'spikes'});
...
gatherdatafile = [savedir 'HP_xcorrmeasures_CA1PFC_gather_long']; area =''; % CA1 vs PFC. Normal file is default corr. "Long" is 1sec long corr.
%gatherdatafile = [savedir 'HP_xcorrmeasures_PFC_gather_long']; area ='PFC'; % PFC vs PFC
...
if gatherdata
    ...
    ripcorrtime=[]; runcorrtime=[]; % Get timebase for ripcorr and runcorr only once
    ...
    for an = 1:length(sleepcorr)
        ...
        for i=1:length(runcorr(an).output{1}),
            ...
            if (runcorr_rip(an).output{1}(i).Neventscorr > 0) && (runcorr_theta(an).output{1}(i).Neventscorr > 0) && (runcorr(an).output{1}(i).Neventscorr > 0)
                ...
                % Time base for theta and all correlations
...
bins_run = find(abs(runcorrtime)<=4*corrwin); % Corrln window is between -400 and 400 ms
thrsev = 10;
cnt_runcorr=0;
for i=1:cntpairs
    ...
    if rip_Nevents >= thrsev
        ...
        totalcorr_theta(cnt_runcorr) = sum(runpairoutput(i).theta_corr(bins_run)); % Take total prob in 4*2*corrwin = -400 to +400ms around 0
        totalcorr_all(cnt_runcorr) = sum(runpairoutput(i).all_corr(bins_run)); % Take total prob in 4*2*corrwin = -400 to +400ms around 0
...
for i=1:cnt_runslpairs
    ...
    if (rip_Nevents >= thrsev) && (slrip_Nevents >= thrsev)
        ...
        totalrunsl_theta(cnt_runslcorr) = sum(runsleeppair(i).theta_corr(bins_run)); % Take total prob in 4*2*corrwin = -400 to +400ms around 0
...
if figopt1==1,
    for i=30:cnt_runslpairs
        ...
        plot(runcorrtime, runsleeppair(i).theta_corr,'b','LineWidth',3);
        line([0 0], [0 max([runsleeppair(i).rip_corr, runsleeppair(i).theta_corr])],'Color',[0.5 0.5 0.5],'LineWidth',2);
        %plot(runcorrtime, runsleeppair(i).all_corr,'k','LineWidth',3);
        %line([0 0], [0 max([runsleeppair(i).rip_corr, runsleeppair(i).theta_corr, runsleeppair(i).all_corr])],'Color',[0.5 0.5 0.5],'LineWidth',2);
...
%         
        ...
        plot(ripcorrtime, runsleeppair(i).rip_corr,'r','LineWidth',3);
        line([0 0], [0 max([runsleeppair(i).rip_corr])],'Color',[0.5 0.5 0.5],'LineWidth',2);

%         plot(ripcorrtime, runsleeppair(i).postsl_corr,'b','LineWidth',3);
%         plot(ripcorrtime, runsleeppair(i).presl_corr,'k','LineWidth',3);
        ...
        xlabel('Time (sec)','FontSize',20);
        ylabel('Norm corrln. - Ripple','FontSize',20);
...
%Co-active Z Rip run and full theta corr
% Scatter plot with fit
% ---------------------
figure; hold on;
if forppr==1
...
[r_thetavsrip,p_thetavsrip] = corrcoef(coactiveZ_rip, totalcorr_theta);
plot(totalcorr_theta,coactiveZ_rip,'ro','Markersize',8,'LineWidth',2); % Run on x-axis
%set(gca,'XLim',[-0.04 1.15]);
%set(gca,'YLim',[-0.4 0.7]);
xlabel(['Theta Correlation: ' num2str(4*corrwin*1000) ' ms win'],'FontSize',xfont,'Fontweight','normal');
ylabel(['Ripple Co-activeZ: ' num2str(corrwin*1000) ' ms win'],'FontSize',yfont,'Fontweight','normal');
text(5,7,['Npairs:' num2str(length(totalcorr_rip))],'FontSize',xfont,'Fontweight','normal');

% Regression
% -----------
[b00,bint00,r00,rint00,stats00] = regress(coactiveZ_rip', [ones(size(totalcorr_theta')) totalcorr_theta']);
...
[b0,bint0,r0,rint0,stats0] = regress(coactiveZ_rip_0',[ones(size(totalcorr_theta')) totalcorr_theta']);
bfit0 = b0(1)+b0(2)*xpts;
...
title(sprintf('Rip vs Theta: r=%g, p=%g; R^2=%g, preg=%g', rval, pval, rsquare, preg),...
    'FontSize',tfont,'Fontweight','normal');
if savefig1==1,
    figfile = [figdir,area,'RipCoactiveZvsThetaCorr_Scatter2'];
    print('-dpdf', figfile); print('-djpeg', figfile); saveas(gcf,figfile,'fig');
end
