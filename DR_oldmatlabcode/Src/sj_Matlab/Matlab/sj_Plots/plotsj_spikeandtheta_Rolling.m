% Shantanu Mar 2013
% From plotsj_spikeraster_Rolling
% To get example of theta phase locking.
% Originally, using it to plot theta on hipp tet, and spikes of a PFC unit
% specifically:  HPa day 2 epoch 2.  Tet 1 theta, and spikes on Tet 18 Cell 1
...
switch prefix
    case 'HPa'
        ...
        cellsp(7,:) = [day epoch 18 1]; % Want this for theta locking, or (15,1)
    ...
    case 'HPb'
        directoryname = 'D:/data25/sjadhav/HPExpt/HPb_direct';
        dire = 'D:/data25/sjadhav/HPExpt/HPb';
        riptetlist=[1,4,5,6];
        ...
        % HPb - day 1. CA1 cells 17act + 7 sil;
        % CA1
        cellsi(1,:) = [day epoch 1 1];
        cellsi(2,:) = [day epoch 1 2];
        cellsi(3,:) = [day epoch 1 3];
        cellsi(4,:) = [day epoch 1 4];
        cellsi(5,:) = [day epoch 1 5];
        cellsi(6,:) = [day epoch 2 2];
        cellsi(7,:) = [day epoch 3 1];
        cellsi(8,:) = [day epoch 4 4];
        cellsi(9,:) = [day epoch 4 5];
        cellsi(10,:) = [day epoch 4 6];
        cellsi(11,:) = [day epoch 4 8];
        cellsi(12,:) = [day epoch 4 9];
        cellsi(13,:) = [day epoch 5 1];
        cellsi(14,:) = [day epoch 6 1];
        cellsi(15,:) = [day epoch 6 4];
        cellsi(16,:) = [day epoch 18 1];
        cellsi(17,:) = [day epoch 18 2];
        cellsi(18,:) = [day epoch 18 3];
        usecellsi=1:18;
        
        % PFC
        cellsp(1,:) = [day epoch 9 1];
        cellsp(2,:) = [day epoch 9 2];
        cellsp(3,:) = [day epoch 9 3];
        cellsp(4,:) = [day epoch 9 4];
        usecellsp=1:2;
...
%--------------
% %Load ripple file
% ripfile = sprintf('%s/%sripplall%02d.mat', directoryname, prefix, day); % For HPa
% load(ripfile);
...
ripfile = sprintf('%s/%sripples%02d.mat', directoryname, prefix, day); % For HPb
load(ripfile);
%Get ripstarttime or midtime for current tetrodes in given day and epoch
rip_starttime = ripples{day}{epoch}{maineegtet}.starttime;   % in sec 
...
% EEg and Ripple cont data AND Theta
% ------------------------
...
%     ripenv = ripple{day}{epoch}{currtet}.data(:,3);
    ...
    load(thetafile);
    thetae = theta{day}{epoch}{currtet};
    thetaamp = theta{day}{epoch}{currtet}.data(:,1);
    thetaenv = theta{day}{epoch}{currtet}.data(:,3);
    
    if te==1
        ttheta = geteegtimes(thetae);
        thetaind = lookup(pt, ttheta);
        thetae.samprate=round(thetae.samprate);
        thetastart = theta{day}{epoch}{te}.starttime; % in secs - Epoch start
        thetaend = ttheta(end); % in secs - Epoch end
    end
    
    eegalltet{te} = e.data;
    thetaalltetamp{te} = thetaamp;
    thetaalltetenv{te} = thetaenv;
    
    %
    %     % Align EEG and Ripple Band to ripple time - NO Aligning In This File
    %     %--------------------------------------------------------------------
    %
...
%taxis = taxis-pret;
%lineat=200;
%for i=1:size(e_stim{1},1)0
winst = eegstart+200; winend = winst + win; % secs
while winend <= eegend
    ...
    % Get eeg times axis
    eind1 = lookup(winst, teeg); thetaind1 = lookup(winst, ttheta);
    eind2 = lookup(winend, teeg); thetaind2 = lookup(winend, ttheta);
    taxis = teeg(eind1:eind2);  thetaaxis = ttheta(thetaind1:thetaind2);
    taxis = taxis - winst; thetaaxis = thetaaxis - winst;
    ...
    % -----------------------
    % PFC - Do the one PFC neuron for now
    %currspkt = spiketimep{7}; %Cell(18,1)
    figure(i); hold on;
    ...
    cnt = 0; baseline = 0;
            ...
            for c=7
                ...
                if ~isempty(currspkt)
                    ...
                    plotraster(currspkt,(baseline+2*(cnt-1))*ones(size(currspkt)),1.8,[],'Color',[clr2],'LineWidth',4);
...
%     end
    ...
    thetatetamp = thetaalltetamp{n};
    currthetaamp = double(thetatetamp(thetaind1:thetaind2));
    
    % Plot
    % ----
    figure(i); hold on;
    eegscale = max(curreeg)-min(curreeg); thetascale = max(currthetaamp)-min(currthetaamp);
    downeeg = baseline-2; upeeg = downeeg+4;
    plotscale = 4;
    curreeg = downeeg + (plotscale/2) + curreeg.*(plotscale/eegscale);
    currthetaamp = downeeg + (plotscale/2) + currthetaamp.*(plotscale/thetascale);
    plot(taxis,curreeg,'k-','LineWidth',1);
    plot(thetaaxis,currthetaamp,'b-','LineWidth',3);
...
%     plot(taxis,curreeg,'r-','LineWidth',1);
    ...
    winsecs = [0:1:winend-winst]; %eg. 0:10:60
    secs = [winst:1:winend];
    secs = round(secs); %secs = roundn(secs,-1);
    msecs = [winst_ms:1000:winend_ms];
