...
sleep_epochs = [3];                               % need to specify sleep vs. run
run_epochs = [2];
...
for d=days
    for e=epochs                                            
        for g=tetrodes
            nowindows = floor(length(eegstruct{d}{e}{g}.data)/winsize)-1;
            for w=1:nowindows
                [spectra{d}{e}{g}(w,:),freqs,Serr] = ...
                    mtspectrumc(double(eegstruct{d}{e}{g}.data((1+(w-1)*winsize):(w+1)*winsize)),params);   %% only analyzing 1 minute / epoch
            end
        end
    end
end
