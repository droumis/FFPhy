function extractlowgamma(directoryname, fileprefix, day, tetrode, nstd)

%function extractlowgamma(directoryname, fileprefix, day, tetrode, min_suprathresh_duration, nstd)
% Reads in the low gamma files from the specified day and tetrodes and
% extracts all of the large gamma events from those tetrodes.
% 
% Low gamma events are defined as 400ms windows around low gamma amplitude
...
for j = 1:length(tet)
    ...
    for i = 1:length(tmpflist)
        ...
        temprenv = double(lowgamma{day}{e}{t}.data(:,3));
        % smooth the envelope:
        samprate = lowgamma{day}{e}{t}.samprate;
    ...
    for i = 1:length(tmpflist)
        ...
        % convert the gamma envelope field to double
        gamma = double(lowgamma{day}{e}{t}.data(:,3));
        time = 1:length(gamma);
        ...
        while tmpind(1)<win+5
            tmpind(1) = [];
        end
        while tmpind(end)>length(renv)-win-5
            tmpind(end) = [];
        end
...
%         clear eeg phase ttime validtheta valid
        ...
        if ~isempty(tmpmax)
            ...
            gamma.midind = tmpind;
            gamma.starttime = lowgamma{day}{e}{t}.starttime + gamma.startind / samprate;
            gamma.endtime = lowgamma{day}{e}{t}.starttime + gamma.endind / samprate;
            gamma.midtime = lowgamma{day}{e}{t}.starttime + gamma.midind / samprate;
            gamma.peak = tmpmax;
        ...
        gamma.timerange = [0 length(renv)/samprate] + lowgamma{day}{e}{t}.starttime;
        gamma.samprate = lowgamma{day}{e}{t}.samprate;
        ...
        %gamma
        clear feeg
        gammal{day}{e}{t} = gamma;
        clear lowgamma;
        clear gamma
    end
end
save(sprintf('%s/%sgammal%02d.mat', directoryname, fileprefix, day), 'gammal');
