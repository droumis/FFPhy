...
% [thetamod peakthetaphase] = modulation(s.data(:,5)); % OBSOLETE, use ca1plotthetamod
% [gammamod peakgammaphase] = modulation(s.gammaphase); % OBSOLETE
...
if (appendind)
    out = [ind width meanrate csi propb];
else
    out = [width meanrate csi propb];
end
