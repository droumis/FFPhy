...
for i = 1:length(linfields.trajdata)
    if ~isempty(linfields.trajdata{i})                                                             %added this error bypass code  kk 3.21.13
        rates = [rates; max(linfields.trajdata{i}(find(~isnan(linfields.trajdata{i}(:,5))),5))];
    else
        disp('empty linefields.trajdata ! -- animal did not perform a trajectory in this epoch!')                            %added this
