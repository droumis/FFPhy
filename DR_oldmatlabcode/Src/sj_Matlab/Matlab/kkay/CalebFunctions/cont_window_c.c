                 ...
                 int nrhs, const mxArray *prhs[])
{
  ...
  if ((samplingrate == 0) || (nsamplesperbuf == 0)) {
    fprintf(stderr,"Sampling rate: %d, NSamplesperbuf: %d\n",
        samplingrate, nsamplesperbuf);
  ...
  int recordDelta = ceil((nsamplesperbuf * 10000.0)/actualSamplingRate);
  ...
  for (i = 0; i < nWindows; i++) {
    ...
    err = fread(&nextTimestamp, sizeof(uint32_t), 1, eegfile);
    if (!err) mxErrMsgTxt("fread error 1");
    while ((nextTimestamp+endRecord) < window[0]) {
      err = fread(recordData, sizeof(int16_t), nsamplesperbuf, eegfile);
      if (!err) mxErrMsgTxt("fread error 2");
      err = fread(&nextTimestamp, sizeof(uint32_t), 1, eegfile);
      if (!err) fprintf(stderr,"The error is %d [feof? %d]\n",
          err, feof(eegfile));
      if (!err) mxErrMsgTxt("fread error 3");
    }
    ...
    err = fread(recordData, sizeof(int16_t), nsamplesperbuf, eegfile);
    if (!err) mxErrMsgTxt("fread error 4");
    ...
    for (j = 0; j < windowLen; j++) {
      ...
      if (odx >= nsamplesperbuf) {
        err = fread(&nextTimestamp, sizeof(uint32_t), 1, eegfile);
        if (!err) mxErrMsgTxt("data fread error");
        if (nextTimestamp > dataStartTimestamp + recordDelta) {
          fprintf(stderr,"skip length: %d",(dataStartTimestamp + recordDelta - nextTimestamp));
          fprintf(stderr,"[%d] > [%d] - [%d]\n",nextTimestamp,
              dataStartTimestamp, recordDelta);
          if (nextTimestamp < window[1]) {
            odx = 0;
          ...
          err = fread(recordData, sizeof(int16_t), nsamplesperbuf, eegfile);
          if (!err) mxErrMsgTxt("data2 fread error");
          odx = 0;
        }
        dataStartTimestamp = nextTimestamp;
      }
