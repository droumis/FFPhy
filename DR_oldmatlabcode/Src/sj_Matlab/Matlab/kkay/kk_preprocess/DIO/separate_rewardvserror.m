...
days = 3;                      %days to evaluate
epochs = [2];              %epochs to evaluate
...
for d=days
    for e=epochs
        ...
        if ~isempty(inderr)
            if ~exist('errortimesall') % first entry
                                 % rewardinfo index     % well          %INPUT (well entry) trig time
                errortimesall{d}{e}=[inderr rewardinfo{d}{e}(inderr,1) rewardinfo{d}{e}(inderr,2)];
                errortimes{d}{e}=errortimesall{d}{e}(:,3);    % INPUT trig times
            else                            % concatenate
