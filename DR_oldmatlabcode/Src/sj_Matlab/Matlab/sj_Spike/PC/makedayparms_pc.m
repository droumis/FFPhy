function makedayparms_pc(dayfolder ,thresh, maxallowedamp, varargin)
%
% Shantanu - May 2012. Alternative makedayparms to get Principal Components. 
% Needs nmakeparams_pc/nmakeparamsNoPos_pc, addpc, pcasvd_matspikes, wavepca. 
        ...
        if ~isempty(ttfile)
            ...
            if (~isempty(timestamps))
                ...
                elseif (system == 3)  % for Addinal Params and PC calc
                    if calcpos
                        nmakeparams_pc([dayfolder,'-',tetstring],str2num(tetstring),waves,timestamps,thresh,maxallowedamp, dopc, Samprange);
                    else
                        nmakeparamsNoPos_pc([dayfolder,'-',tetstring],str2num(tetstring),waves,timestamps,thresh,maxallowedamp, dopc, Samprange);
                    end % end for PC calc
                end
            end
            clear tmp1;
