function [addn_params, new_waves] = addpc(waves, triggers, dopc)

% Shantanu - May 2012
% Usually called from nmakeparams_pc.m
...
% This code is for PCs only. If 0, returns empty matrix for addn_params 
...
for i = 1:size(waves,3)
    ...
    if mod(i,100000)==0
        disp(['      PC Parameters: ' num2str(i) ' spikes done']);
        %toc;
        %tic;
    end
...
if dopc==1
    disp(['      Doing PCs']);
    %tic
    addn_params = pcasvd_matspikes(new_waves,size(new_waves,2));
    %toc
