function sj_matspikes_to_fetg(datafile,tetnum, dopc, dopos, THRESH, MAXALLOWEDAMP, dogspikefile,Samprange)
% sj_matspikes_to_fetg('08_030610-07',7, 1, 1, 0, 2500,0,[4:32]);
% Similar to sj_nmakeparams_withpc and sj_nmakeparamsNOPOS_withpc,
% which are usually called from sj_makedayparams_withpc
%
% This is more like sj_gspikes_to_fet which is a standalone code to go
% from matclust like .tt file to .fet file for Klusatakiwk and .spk file for Klusters
% and gspikefile for gsort by default

% Calculates PCs by default and Pos by default 
% Shantanu, 04/21/2010

% datafile is "dayname-tet", eg. 02_030610-05
...
if (nargin < 3)
    dopc = 1;  %threshhold: at least one spike must excede this amplitude (in micro volts)
end

if (nargin < 4)
    dopos = 1;  %threshhold: at least one spike must excede this amplitude (in micro volts)
end
...
if ~isempty(goodspikes)
    ...
    if dopos == 1
        ...
        if dopc==1 % dopos=1, dopc=1
            ...
            filedata.params = [double(timestamps) filedata.params spikepos(:,1:2)+addnoise (Fs.*(double(timestamps-timestamps(1))./UnitsPerSec))];           
            filedata.paramnames = {'Time',
                'Channel 1 Max',
                'Channel 2 Max',
                'Channel 3 Max',
        ...
        if dopc==1 %dopos=0, dopc=1
            ...
            filedata.params = [double(timestamps) filedata.params (Fs.*(double(timestamps-timestamps(1))./UnitsPerSec))];           
            filedata.paramnames = {'Time',
                'Channel 1 Max',
                'Channel 2 Max',
                'Channel 3 Max',
...
if ~isempty(goodspikes)
    ...
    if dopc==1
        ...
        % Order as 12 PCs, 4 amps, 1 Kkwik time = total 17
        if dopos==1
        ...
        nfets = 17; % its 17: only 5 used, rest are zeros
        fet_matrix = zeros(length(timestamps),17);
        fet_matrix_idxs = [13 14 15 16 17];
        fet_matrix(:,fet_matrix_idxs) = filedata.params(:,idxs);
