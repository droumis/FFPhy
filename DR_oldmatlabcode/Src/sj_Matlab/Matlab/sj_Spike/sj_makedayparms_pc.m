function sj_makedayparms_pc(daydirect, prefix, days,thresh, maxallowedamp, varargin)
%
% Shantanu - May 2012.
...
% For getting the _direct folder path
cd ..;
basedir = pwd;
                        ...
                        if ~isempty(ttfile)
                            ...
                            if (~isempty(timestamps))
                                ...
                                elseif (system == 3)  % for Addinal Params and PC calc
                                    if calcpos
                                        sj_nmakeparams_pc([dayfolders(i).name,'-',tetstring],str2num(tetstring),waves,timestamps,thresh,maxallowedamp, dopc, Samprange, donoise, dogspikefile);
                                    else
                                        sj_nmakeparamsNoPos_pc([dayfolders(i).name,'-',tetstring],str2num(tetstring),waves,timestamps,thresh,maxallowedamp, dopc, Samprange, donoise, dogspikefile);
                                    end % end for PC calc
                                end
                            end
                            clear tmp1;
