function spikes = sj_dejitter1(spikes, maxshift)


% Shantanu. 19 May 2011. Adapted from ssm_dejitter.m written by Samar 
% Use Spline to oversample instead of fft like Caleb - then get CoM for
% peak. Align at peak, and pick region around peak for saving.
% Uses thresholded peaks for mask right now, which I want to re-write. It
% is called heightwidth_fromthreshold.m in this folder. Using CoM for
% tetrode, no need for Heights/Widths which can cuase error. After
% aligning, then go in and get height/width



% After aligning everything, then go in and find width, peak, valley, PC etc
% Hint from phd_spikewidths_fwhm2.m and heightwidth_fromthreshold.m and
% from MClust - Features folder
...
isThreshLo = (spikes.waveforms(:, spikes.threshT+1) < spikes.threshV(1));
...
fiducials = (waves * [1:numsamples]') ./ sum(waves, 2);   %%% Weighted sum over waves=waves.*mask
