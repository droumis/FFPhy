function [plower, pupper, pmid, pmode, pmatrix] = pdistn(q, s, muone, background_prob, conf);
...
lowlimit  = find(sumpd > 0.5-conf/2);
if(~isempty(lowlimit) )
lowlimit  = lowlimit(1);
else
lowlimit  = 1;
...
highlimit = find(sumpd > 0.5+conf/2);
...
middlimit = length(pr);
 ...
 plower(ov)   = pr(lowlimit(1));
 pupper(ov)   = pr(highlimit(1));
