function [sp,values,rho] = spaps_L1(x,y,tol,varargin)
%SPAPS_L1 Smoothing spline with L1-norm penalty on derivative.
%
%   [SP,VALUES] = SPAPS_L1(X,Y,TOL)  returns the B-form and, if asked, the
...
%
%       F(D^M f) := integral |D^M f(t)| dt  on  min(X) < t < max(X)
%
...
%
%   [SP,VALUES,RHO] = SPAPS_L1(X,Y,...) also returns RHO.
%
...
%   |Y(:,j) - f(X(j))|^2 and  |D^M f(t)|  are sums over these prod(d) entries.
%
%   SPAPS_L1(X,Y,TOL,W)
%   SPAPS_L1(X,Y,TOL,M)
%   SPAPS_L1(X,Y,TOL,W,M)
%   SPAPS_L1(X,Y,TOL,M,W)
...
%
%                    integral lambda(t) abs(D^M f(t)) dt ,
%
...
%
%   SPAPS_L1({X1,...,Xr},Y,...) provides a smoothing spline to data values
...
%
%This is a modification of SPAPS in the MATLAB Spline Toolbox
