...
Bars_X_Grid *New_X_Grid(int nd){
  Bars_X_Grid *xg;
  xg = (Bars_X_Grid *)mxMalloc(sizeof(Bars_X_Grid));
  (*xg).n = nd;
  (*xg).x = tssdvec(nd);
  return xg;
}
...
Bars_BarsParams *New_BarsParams(){
  Bars_BarsParams *bp;
  bp = (Bars_BarsParams *)mxMalloc(sizeof(Bars_BarsParams));
...
void Free_BarsParams(Bars_BarsParams *bp){
  mxFree((*bp).iter_knots_fname);
  mxFree((*bp).iter_mu_fname);
  mxFree((*bp).iter_mufine_fname);
  mxFree((*bp).iter_params_fname);
  mxFree((*bp).summ_mu_fname);
  mxFree((*bp).summ_mufine_fname);
  mxFree((*bp).summ_params_fname);
  mxFree(bp);
}

Bars_PriorParams *New_PriorParams(){
  Bars_PriorParams *pp;
  pp = (Bars_PriorParams *)mxCalloc(1L,sizeof(Bars_PriorParams));
...
void Set_PriorParams(Bars_PriorParams *pp, Bars_BarsParams *bp, enum PriorForm pid, double *dpar, int *ipar){
  ...
  nd = (*pp).num_dparams;
  ni = (*pp).num_iparams;
  if (nd > 0) mxFree((*pp).dparams);
  if (ni > 0) mxFree((*pp).iparams);
  (*pp).prior_id = pid2;
  switch(pid2){
  case POISSON: 
    nd = 1;
...
void Free_PriorParams(Bars_PriorParams *pp){
  if ((*pp).num_dparams > 0) mxFree((*pp).dparams);
  if ((*pp).num_iparams > 0) mxFree((*pp).iparams);
  mxFree(pp);
}
  
Bars_BinnedData *New_BinnedData(int nd, int nf){
  Bars_BinnedData *bd;
  bd = (Bars_BinnedData *)mxCalloc(1L,sizeof(Bars_BinnedData));
  (*bd).n = nd;
  (*bd).nf = nf;
  (*bd).x_raw = tssdvec(nd);
  (*bd).xg = New_X_Grid(nd);
...
Bars_Model *New_Model(int nd, int nf){
  Bars_Model *m;
  m = (Bars_Model *)mxCalloc(1L,sizeof(Bars_Model));
  (*m).n = nd;
  (*m).nf = nf;
  (*m).k = 0;
  (*m).fit_info = 0;
...
void Free_Model(Bars_Model *m){
  mxFree((*m).knots_interior);
  mxFree((*m).knots_all);
  mxFree((*m).basis);
  mxFree((*m).fitted_values);
  mxFree((*m).J);
  mxFree((*m).beta);
  mxFree((*m).basis_fine);
  mxFree((*m).random_mu_fine);
  mxFree((*m).random_mu);
  mxFree(m);
}
...
Bars_WorkSpace *New_WorkSpace(int n,int nf){
  int p = MAXKNOTS + 4,nm;
  Bars_WorkSpace *ws;
  ws = (Bars_WorkSpace *)mxCalloc(1L,sizeof(Bars_WorkSpace));
  nm = imax(n,nf);
  (*ws).n_max = nm;
...
Bars_OutputStat *New_OutputStat(int np){
  Bars_OutputStat *os;
  os = (Bars_OutputStat *)mxCalloc(1L,sizeof(Bars_OutputStat));
  (*os).np = np;
...
Bars_SampStat *New_SampStat (int n, int nf, int np, int sim){
  Bars_SampStat *ss;
  ss = (Bars_SampStat *)mxCalloc(1L,sizeof(Bars_SampStat));
  (*ss).np = np;
  (*ss).sim = sim;
...
void Free_SampStat(Bars_SampStat *ss){
  int i,np;
  mxFree((*ss).random_mu_mean);
  mxFree((*ss).random_mufine_mean);
  mxFree((*ss).random_mu_mode);
  mxFree((*ss).random_mufine_mode);
  np = (*ss).np;
  for(i=0;i<np;i++){
    mxFree((*ss).params[i]);
  }
