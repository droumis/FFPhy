...
int nh;
{
  return (char *)mxCalloc((long) nh,sizeof(char));
}
...
int nrh,nch;
{
  int i;
  char **c;
  c = (char **) mxCalloc((long) nrh, sizeof(char*));
  for(i=0;i<nrh;i++){
    c[i] = (char *) mxCalloc((long) nch,sizeof(char));
  }
  return c;
}
...
int nrh,nch;
{
   int i;
   double **m;
   m=(double **) mxCalloc((long) nrh,sizeof(double*));
   for(i=0;i<nrh;i++) 
      m[i]=(double *) mxCalloc((long) nch,sizeof(double));
   return m;
}
...
int nrh,nch;
{
return( (double *) mxCalloc((long) (nrh*nch),sizeof(double)));
}
         ...
         double *y)
{
  ...
  a = (double *) mxCalloc((long) *order, sizeof(double));
  orderm1 = *order - 1L;    /* allocate difference tables */
  rdel = (double *) mxCalloc((long) orderm1, sizeof(double));
  ldel = (double *) mxCalloc((long) orderm1, sizeof(double));
  ...
  mxFree(ldel); mxFree(rdel); mxFree(a);
}
...
{               /* evaluate the non-zero B-spline basis */
  ...
  rdel = (double *) mxCalloc((long) orderm1, sizeof(double));
  ldel = (double *) mxCalloc((long) orderm1, sizeof(double));
  coeff = (double *) mxCalloc((long) *order, sizeof(double));
  ...
  mxFree(ldel); mxFree(rdel); mxFree(coeff);
}
