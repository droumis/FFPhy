...
function [perf, smooth_perf, perfvsday] = sj_smoothperf1_out (dir, days,epochs,prefix,days_ex, win, figopt, savefig)
%function [perf, smooth_perf, perfvsday] = sj_smoothperf1(prefix, win, logic, ntrajs_perday, type, figopt)
% eg [perf, smooth_perf, perfvsday] = sj_smoothperf1_out('/data25/sjadhav/RippleInterruption/REd_direct',[1:8],[2 4],[], 10, 1);
...
%clr2 = 'b'; % Confidence bounds m/b/k
...
if figopt==1
    ...
    %Title
    title([prefix ' - ' 'Outbound Trials (Moving Average-' num2str(win) 'points) Performance'],'FontSize',titlefont,'Fontweight','normal');
...
if savefig == 1    
    ...
    figfile = [figdir,prefix,'_out_raw'];
    print('-dpdf', figfile);
    print('-djpeg', figfile);
