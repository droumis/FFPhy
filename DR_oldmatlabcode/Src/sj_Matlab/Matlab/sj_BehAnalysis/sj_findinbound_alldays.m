...
function [inbound_curve, all_inbound_logic, all_inbound_wellstend, ntrajs_perday] = sj_findinbound_alldays (dir, days,epochs,prefix, doprobfit,days_ex,savedata, savefig)
...
% sj_findinbound_alldays ('/data25/sjadhav/HPExpt/HPa_direct',[1:8],[],'HPa', 1);
...
figdir = '/data25/sjadhav/HPExpt/Figures/Behavior/';
    ...
    if isempty(epochs)
        ...
        for i = 1:toteps,
            if ~isempty(linpos{day}{i})
                ...
                if ( strcmp('wtr1',task{day}{i}.environment)) 
                    epochs=[epochs, i];
                end
            end
        end
...
if doprobfit == 1
    ...
    jbfill(taxis',pc(2:end,3)',pc(2:end,2)',clr,clr,0.4,0.4); %0.4
    ...
    for i=1:length(days)
        ...
        if mod(i,2)==1 % odd days
            jbfill(currxax,yup, 0*ones(size(currxax)),'k','k',0.1,0.1); %0.1
        else % even days
            jbfill(currxax,yup, 0*ones(size(currxax)),'k','k',0.2,0.2); %0.2
        end
    end
...
if savefig == 1    
    ...
    figfile = [figdir,prefix,'_Wtr1_Inbound']
    print('-djpeg', figfile);
    print('-dpdf', figfile); saveas(gcf,figfile,'fig');
end
...
if ~isempty(days_ex)
    if ( strcmp(prefix,'REf') | strcmp(prefix,'RCd') )  % Logic is all/almost all 1s and algorithm crashes
        ninbound_curve=ex_inbound_logic;
        ex_inbound_curve=ninbound_curve;
        ex_inbound_curve_witherr = repmat(ex_inbound_curve,1,3);
        ...
        for i=1:length(days_ex),
            ex_inbound_perf_day(i) = mean(ninbound_curve(stidx:stidx+ex_ntrajs_perday(i)-1));
            stidx=stidx+ex_ntrajs_perday(i);
        end     
        disp('Not Plotting Extra Days - Algorithm crashes');
...
cd '/data25/sjadhav/RippleInterruption/Figures/01AugSep11_RippleDisFigs/Behavior/IndividualAnimals'
...
%savefile = [prefix '_inbound'];
%save(savefile,'all_inbound_logic', 'all_inbound_wellstend','ntrajs_perday','inbound_curve');
