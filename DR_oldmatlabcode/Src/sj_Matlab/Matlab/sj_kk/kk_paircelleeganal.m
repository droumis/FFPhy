function f = kk_paircelleeganal(f)

% kk modified singlecelleeganal 6.5.13

% loads corresponding local eeg for the two cells in a pair
    ...
    for g = 1:length(f(an).epochs)
        ...
        for e = 1:size(f(an).epochs{g},1)
            ...
            for c = 1:size(f(an).data{g}{e},1)
                % day epoch  tet1 cell1  tet2 cell2
                sindex = [f(an).epochs{g}(e,:) f(an).data{g}{e}(c,:)];
                % day epoch tet1 tet2
                eindex = sindex([1 2  3  5]);
                
                % load the eeg data for this cell
                for i = 1:length(f(an).function.loadvariables)
                    if (iseegvar(f(an).function.loadvariables{i}))
                          % loads both tetrodes' eeg data
                        eval([f(an).function.loadvariables{i},' = loadeegstruct(animaldir, animalprefix, f(an).function.loadvariables{i}, eindex(1), eindex(2), eindex(3:4));']);
                    end
                end
