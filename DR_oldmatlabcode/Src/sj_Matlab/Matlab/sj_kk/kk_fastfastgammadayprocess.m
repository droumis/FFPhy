function kk_fastfastgammadayprocess(directoryname,fileprefix,days, varargin)

%% simple modification of lowgammadayprocess

%fastfastgammaDAYPROCESS(directoryname,fileprefix,days, options)
%
...
%			Default 0
%       'nonreference', 0 or 1
%           specifies whether to use EEG or EEGnonreferencefastfastgammasextract.m
%           Default 0

daytetlist = [];
f = '';
defaultfilter = (['/home/kkay/matlab/Filters/basfastfastgammafilter.mat']);
assignphase = 0;
tetfilter = '';
subtractreference = 0;
...
%         keyboard
    ...
    for fnum = 1:length(flist)
        ...
        temp = filtfilt(fastfastgammafilter,1,eeg{day}{epoch}{tet}.data');
        hdata = hilbert(temp);
        env = abs(hdata);
        phase = angle(hdata);

        fastfastgamma{day}{epoch}{tet}.samprate = eeg{day}{epoch}{tet}.samprate;
        fastfastgamma{day}{epoch}{tet}.starttime = eeg{day}{epoch}{tet}.starttime;

        fastfastgamma{day}{epoch}{tet}.data(:,1) = int16(temp);
        fastfastgamma{day}{epoch}{tet}.data(:,2) = int16(phase*10000);
        fastfastgamma{day}{epoch}{tet}.data(:,3) = int16(env);
        fastfastgamma{day}{epoch}{tet}.fields = ...
        ...
        for i = 1:length(lo)
        	if lo(i) > 1 && hi(i) < length(fastfastgamma{day}{epoch}{tet}.data)
            	fastfastgamma{day}{epoch}{tet}.data(lo(i):hi(i)) = minint;
            end
        end

        % save the resulting file
        if subtractreference
            fastfastgammafile = sprintf('%sEEGnonreference/%sfastfastgamma%02d-%d-%02d.mat', ...
            	savedirectoryname, fileprefix, day, epoch, tet);
        else
            fastfastgammafile = sprintf('%sEEG/%sfastfastgamma%02d-%d-%02d.mat', ...
            	savedirectoryname, fileprefix, day, epoch, tet);
        end
        save(fastfastgammafile, 'fastfastgamma');
            ...
            if ~isempty(s)
                ...
                for c = 1:length(s)
                    ...
                    if ~isempty(data)
                        ...
                        spikes{day}{epoch}{tet}{c}.fastfastgammaphase = ...
