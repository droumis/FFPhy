...
saveg1=0; saveg2=0;
%%% Run Epochs - Response to stimulation ###
...
for d = 1:length(days)
    ...
    for i = 5:length(eind)-5;
        i;
        cnt=cnt+1;
        e_stim(cnt,:)=e.data(eind(i+5)-0.2*e.samprate:eind(i+5)+0.4*e.samprate);
        etmp_stim(cnt,:)=etmp(eind(i+5)-0.2*e.samprate:eind(i+5)+0.4*e.samprate);
        ripamp_stim(cnt,:)=double(ripamp(eind(i+5)-0.2*e.samprate:eind(i+5)+0.4*e.samprate));
        ripampfilt_stim(cnt,:)=double(ripampfilt(eind(i+5)-0.2*e.samprate:eind(i+5)+0.4*e.samprate));
        ripenv_stim(cnt,:)=double(ripenv(eind(i+5)-0.2*e.samprate:eind(i+5)+0.4*e.samprate));
        ripenvfilt_stim(cnt,:)=double(ripenvfilt(eind(i+5)-0.2*e.samprate:eind(i+5)+0.4*e.samprate));
    end
...
%     
    ...
    if saveg1==1,
        orient(gcf,'portait'); hold on; set(gcf, 'PaperPositionMode', 'auto');
        saveas(gcf,['Day' num2str(day) 'Tet' num2str(tet) '_Run1_RipplAmp'],'jpg');
    end
    ...
    ylabel('Ripple Envelope');
     ...
     if saveg1==1,
        orient(gcf,'portrait'); hold on; set(gcf, 'PaperPositionMode', 'auto');
        saveas(gcf,['Day' num2str(day) 'Tet' num2str(tet) '_Run1_RipplEnv'],'jpg');
    end
...
if saveg2==1,
    orient(gcf,'portrait'); hold on; set(gcf, 'PaperPositionMode', 'auto');
    saveas(gcf,['Days467_Tet' num2str(tet) '_Run1_RipAmp'],'jpg');
    saveas(gcf,['Days467_Tet' num2str(tet) '_Run1_RipAmp'],'fig');
...
title(['Avg over Days 4,6,7 : Ripple Env (Tet' num2str(tet) '): Pre-Filter(black) and Post-filter (red)'],...
    'FontSize',24,'Fontweight','bold');
xlabel('Time (ms)');
ylabel('Rip Power');
...
if saveg2==1,
    orient(gcf,'portrait'); hold on; set(gcf, 'PaperPositionMode', 'auto');
    saveas(gcf,['Days467_Tet' num2str(tet) '_Run1_RipEnv'],'jpg');
    saveas(gcf,['Days467_Tet' num2str(tet) '_Run1_RipEnv'],'fig');
