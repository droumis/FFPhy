function [basefr0] = sj_rippower_stimampl1(prefix, day, binsize, manytet, maintet, tets, nostim, saveg1)
%% Plot Ripple Power Calibration Curve from Data Taken Especially for this purpose
...
% eg
% sj_rippower_stimampl1('REc', 20,5,0,9,[],0);
% sj_rippower_stimampl1('REc', 18,5,0,10,[],0);
% sj_rippower_stimampl1('REc', 19,5,0,10,[],1);
% sj_rippower_stimampl1('REc', 15,5,0,10,[],0);
% sj_rippower_stimampl1('REb', 27,10,1,4,[3 4 8 10 11 12],[],1);
% sj_rippower_stimampl1('RCa', 21,5,0,4,[],1);
% sj_rippower_stimampl1('RCa', 21,5,0,4,[],0);
...
twoplot_idx=4; % for the plot comparing 0uA with 2nd stimulation amplitude, choose which index
...
for i =1:length(stim_starttime) % Need to Skip initial and final indices?
    ...
    nelements = length(1000-0.2*e.samprate:1000+0.25*e.samprate);
    if ( (currind-0.2*e.samprate <=0) | (currind+0.25*e.samprate>length(e.data)) )
        e_stim(cnteeg,:)=0*(1:nelements);
        ripamp_stim(cnteeg,:)=0*(1:nelements);
        ripenv_stim(cnteeg,:)=0*(1:nelements);
    else
        e_stim(cnteeg,:)=e.data(currind-0.2*e.samprate:currind+0.25*e.samprate);
        ripamp_stim(cnteeg,:)=double(ripamp(currind-0.2*e.samprate:currind+0.25*e.samprate));
        ripenv_stim(cnteeg,:)=double(ripenv(currind-0.2*e.samprate:currind+0.25*e.samprate));
    end
    ...
    case 'REc'
        ...
        switch day        % BY DAY WITHIN ANIMAL
            ...
            case 04
                ...
                % Nthr=2;
                nranges=1;
                %[range1] = timetrans({'00:40:58', '00:55:59'},10000,2); amp1=3000; % epoch 2
                [range1] = timetrans({'01:17:25', '01:32:26'},10000,2); amp1=3000; % epoch 4
...
for n=1:nranges
    ...
    if n==1
        ...
        basefr0 = mean(yplot(1:0.1*e.samprate));
        basefr0 = roundn(basefr0,-1); 
        std0 = std(yplot(1:0.1*e.samprate)); std0 = roundn(std0,-1);
        plot(taxis, basefr0*ones(size(taxis)),'k--','Linewidth',2 ); 
        plot(taxis, (basefr0+4*std0)*ones(size(taxis)),'c-.','Linewidth',2 ); 
    ...
    if n~=1,
        ...
        plot(taxis, (curr_basefr+4*cstd)*ones(size(taxis)),'g-.','Linewidth',2 ); 
