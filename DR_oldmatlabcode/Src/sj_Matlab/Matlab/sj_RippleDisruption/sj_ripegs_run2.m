function [pret] = sj_ripegs_run2(prefix, day, epoch,nostim, saveg1)
% Align to ripples. From sj_ripegs_run which aligns to stimulation times 
...
for te=1:length(tets)
    ...
    rip_starttime = ripples{day}{epoch}{currtet}.midtime;   % in sec  
    ripsize = ripples{day}{epoch}{currtet}.maxthresh;
    pt = rip_starttime;
    eind = lookup(pt, ti);
    if iscon==1
...
if saveg1==1
    figfile = [figdir,prefix,'_Day',num2str(day),'Ep',num2str(epoch),'Stim',num2str(i),'_RipEg1'];
    print('-dpdf', figfile);
    print('-djpeg', figfile);
