...
function [ep] = sj_epsp_day_slopesumm (processdir,prefixes,pdays,figopt1,saveg1,figopt2, saveg2, savedata)

% Uses "day" files generated by sj_epsp_slope_day
% epsp data for 1 day each to create summary plot for epsp slope
% Output: Make normalized plot with Z-scores across days and tetrodes

% Eg
% sj_epsp_day_slopesumm('/data25/sjadhav/RippleInterruption/ProcessedData/EPSP/Slope',{'REc';'REd';'REe';'REf';'REg';'REh'},[1:8],1,0,0,0,0);
...
for anim=1:length(prefixes)
    ...
    %         % Loading file gives: day, tets, all_estim, all_eslope, respZsave
    %         files(f).name;
    %         load(files(f).name);
...
[ht12m,pt12m,cit12m] = ttest2(respZep1, respZep2, 0.05);  % 0.79
[ht13m,pt13m,cit13m] = ttest2(respZep1, respZep3, 0.05);  % 0.43
...
title('EPSP Slope Z-score normalized to Pre-Sleep','FontSize',tfont,'Fontweight','normal');
...
%set(gca, 'YTickLabel',[]);
if saveg1==1,
    figfile = [figdir,grp,'_0EPSPslo_Zsumm'];
    print('-dpdf', figfile);
    print('-djpeg', figfile);
    saveas(gcf,figfile,'fig');
end

% Combine Post1 and Post2 into Post
figure; hold on;  
if forppr==1
    redimscreen_figforppr1;
else
...
respZep23 =[respZep2,respZep3];
bar(1,mean(respZep1),'k'); errorbar(1,mean(respZep1),sem(respZep1),'k','LineWidth',3);
bar(2,mean(respZep23),'r'); errorbar(2,mean(respZep23),sem(respZep23),'k','LineWidth',3);
set(gca,'XTick',[1 2],'XTickLabel',{'Pre','Post'},'FontSize',xfont,'Fontweight','normal');
title('EPSP Slope Z-score normalized to Pre','FontSize',tfont,'Fontweight','normal');
ylabel('EPSP Slope Z-score','FontSize',yfont,'Fontweight','normal');
axis([0 3 -0.1 0.1])
set(gca,'XLim',[0 3]);
set(gca,'YLim',[-0.05 0.05]);
[ht123m,pt123m,cit123m] = ttest2(respZep1, respZep23, 0.05);
if ht12m==1,
    mul = sign(mean(respZep2));
    plot(2, mean(respZep2)+1.4*mul*sem(respZep2), 'r*','MarkerSize',8);
end
...
text(1.5,0.035,['p = ',num2str(roundn(pt123m,-2))],'FontSize',16);
%set(gca, 'XTickLabel',[]);
%set(gca, 'YTickLabel',[]);
if saveg1==1,
    figfile = [figdir,grp,'_EPSPslo_Zsumm2'];
...
if figopt2==1 && ~isempty(intersect(pdays,novel))
    ...
    set(gca,'XTick',[1 2 3],'XTickLabel',{'Pre-Sleep','Post-Sleep1','Post-Sleep2'},'FontSize',xfont,'Fontweight','normal');    
    title('Novel Days: EPSP Slope Z-score normalized to Sleep1','FontSize',tfont,'Fontweight','normal');
    ylabel('EPSP Slope Z-score','FontSize',yfont,'Fontweight','normal');
    axis([0 4 -0.1 0.1])
    ...
    if saveg2==1,
        figfile = [figdir,grp,'_NovDays_EPSPslo_Zsumm'];
        print('-dpdf', figfile);
        print('-djpeg', figfile);
        saveas(gcf,figfile,'fig');
    end
...
if figopt2==1 && ~isempty(intersect(pdays,fam))
    ...
    set(gca,'XTick',[1 2 3],'XTickLabel',{'Pre-Sleep','Post-Sleep1','Post-Sleep2'},'FontSize',xfont,'Fontweight','normal');
    title('Fam Days: EPSP Slope Z-score normalized to Sleep1','FontSize',tfont,'Fontweight','normal');
    ylabel('EPSP Slope Z-score','FontSize',yfont,'Fontweight','normal');
    axis([0 4 -0.1 0.1])
    ...
    if saveg2==1,
        figfile = [figdir,grp,'_FamDays_EPSPslo_Zsumm'];
        print('-dpdf', figfile);
        print('-djpeg', figfile);
        saveas(gcf,figfile,'fig');
    end
...
if figopt1==1
    ...
    for d=1:length(pdays)
        ...
        title(['Day' num2str(pdays(d)) ' : EPSP Slope Z-score norm to Sleep1'],'FontSize',tfont,'Fontweight','normal');
        ylabel('EPSP Slope Z-score','FontSize',yfont,'Fontweight','normal');
        axis([0 4 -0.5 0.5])
        ...
        if saveg1==1,
            figfile = [figdir,grp,'_EPSPslo_Zsumm_Day',num2str(pdays(day))];
            print('-dpdf', figfile);
            print('-djpeg', figfile);
            saveas(gcf,figfile,'fig');
...
if savedata==1,
    savefile = sprintf('/data25/sjadhav/RippleInterruption/ProcessedData/EPSPSlopeSumm.mat');
    save(savefile);
end
