...
if isempty(pin)
  ...
  pin = input(strcat('Use which pin as input [ ',sprintf('%d ',find(sum(diovals))),']?'));
end
...
for ep = eps(:)'
  idx = epochs==ep;
  stimdio{ep}.pulsetimes = diopulses.pulsetimes(idx,:);
  stimdio{ep}.timesincelast = diopulses.timesincelast(idx);
  stimdio{ep}.timeuntilnext = diopulses.timeuntilnext(idx);
  stimdio{ep}.pulselength = diopulses.pulselength(idx,:);
  stimdio{ep}.pulseind = diopulses.pulseind(idx,:);
  stimdio{ep}.frequency = diopulses.frequency(idx,:);
  stimdio{ep}.frequency2 = diopulses.frequency2(idx,:);
  stimdio{ep}.index_in_sequence = diopulses.index_in_sequence(idx,:);
  stimdio{ep}.pulseStruct = pulseStruct(idx);
end
