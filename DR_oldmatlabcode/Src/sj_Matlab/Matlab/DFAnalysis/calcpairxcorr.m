...
for i = 1:size(ind,1)
    try
        ...
        gam1 = spikes{ind(1,1)}{ind(1,2)}{ind(i,3)}{ind(i,4)}.cumulative_phase;
        gam1 = gam1(find(~isExcluded(spikes1,excludetimes)));
        spikes1 = spikes1(find(~isExcluded(spikes1,excludetimes)));
        spikes2 = spikes{ind(2,1)}{ind(2,2)}{ind(i,5)}{ind(i,6)}.data(:,1);
        gam2 = spikes{ind(1,1)}{ind(1,2)}{ind(i,5)}{ind(i,6)}.cumulative_phase;
        gam2 = gam2(find(~isExcluded(spikes2,excludetimes)));
    ...
    if (~isempty(spikes1) && ~isempty(spikes2))
        ...
        out(i).time = sort(abs(out(i).time));
    ...
    if ~isempty(gam1) && ~isempty(gam2)
        tmpcorr = spikexcorr(gam1,gam2,gambin,gmax);
        maxval = max(tmpcorr.c1vsc2);
        out(i).phase = [];
        for addup = 1:maxval
        	out(i).phase = [out(i).phase tmpcorr.time(find(tmpcorr.c1vsc2 == addup))];
        end
        out(i).phase = sort(abs(out(i).phase));
        if length(out(i).phase)>= length(out(i).time)
            out(i).phase = out(i).phase(1:size(out(i).time,2));
        else
            out(i).time = out(i).time(1:size(out(i).phase,2));
        end           
