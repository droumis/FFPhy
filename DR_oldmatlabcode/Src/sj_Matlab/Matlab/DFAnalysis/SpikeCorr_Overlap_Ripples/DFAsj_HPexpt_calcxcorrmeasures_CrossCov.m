function out = DFAsj_HPexpt_calcxcorrmeasures(ind, excludetimes, spikes, varargin)
% SJ - 08/2012. For Ripple-related corrln in CA1-PFC, do not exclude PFC spikes.
...
bin = 0.01; % 0.002=2ms bins
sw1 = bin*3; % for smoothing 
%sw1 = bin*5;
...
%t2inc_forcorr = t2; % DO NOT Exclude PFC Spikes, which should be 2nd in pair

xc = spikexcorr(t1inc, t2inc_forcorr, bin, tmax);
...
% Calculate Cross-Covariance - Based on Siapas (2005) and Wierzynski (2009)
% --------------------------
% This will currently work correctly, only for all times. If you have a timefilter, eg. for ripples - 
% better to load that data structure and derive total times directly from it.

timerangest = spikes{ind(1)}{ind(2)}{ind(3)}{ind(4)}.timerange(1);
timerangeend = spikes{ind(1)}{ind(2)}{ind(3)}{ind(4)}.timerange(2);
T = (timerangeend-timerangest)./10000; % in seconds
if ~isempty(excludetimes)
    totalexcluded = sum(excludetimes(:,2)-excludetimes(:,1));
    T = T-totalexcluded;
end
spikerate1 = length(t1inc)/T; spikerate2 = length(t2inc)/T;

if (sum(xc.c1vsc2)==0 || isnan(sum(xc.c1vsc2)))
    xc.q=zeros(size(xc.time));
    xc.Q=zeros(size(xc.time));
else
    xc.q = (xc.c1vsc2/(bin*T))-(spikerate1*spikerate2);
    % Unit Normal Z scores
    xc.Q=xc.q*(sqrt((bin*T)./(spikerate1*spikerate2)));
    % Smooth Standardized Cross-Correlation
    nstd=round(0.05/bin); % 50ms/bin
    g1 = gaussian(nstd, 5*nstd+1);
    xc.q = smoothvect(xc.q, g1);
end
...
xc.Pi = spikerate1;
xc.Pj = spikerate2;
xc.alpha = 0.01/(2*tmax/bin);
xc.Z = sqrt(2)*erfinv(1-xc.alpha);
