...
void SetRJProbs(double *birth, double *death, Bars_BarsParams *bp,
  ...
  birth[0] = 0.0;
  death[0] = 0.0;
  birth[1] = (pratio[1] < DBL_EPSILON) ? 0.0 : c * __min(1.0,pratio[1]);
  death[1] = 0.0;
  birth[MAXKNOTS] = 0.0;
  if (pratio[MAXKNOTS - 1] < DBL_EPSILON){
    death[MAXKNOTS] = c;
  } else {
    death[MAXKNOTS] = c * __min(1.0,(1.0/pratio[MAXKNOTS - 1]));
  }
  for(i=2;i<MAXKNOTS;i++){
    birth[i] = (pratio[i] < DBL_EPSILON) ? 0.0 : c * __min(1.0,pratio[i]);
    death[i] = (pratio[i-1] < DBL_EPSILON) ? c :
      c * __min(1.0,(1.0/pratio[i-1]));
  }
  free(pratio);
}
...
void Bars_MCMC(Bars_Model *m1, Bars_Model *m2,
  ...
  OpenFiles(bp,1,0);    /* open parameter files */

  n = (*pd).n;          /*number of data pairs*/
  iter_info = 0;
  countup = tssivec(4);
  moves = tssivec(4);
  for(i=0;i<4;i++){
  ...
  samp_iter = (*bp).samp_iter; /* number of sample iterations */
  burn_iter = (*bp).burn_iter; /* number of burn-in iterations */
