...
% 
% Filter function that loops through all cells and calculates the
% auto correlation.Saves
...
filename=strcat(f.animal{2},f.animal{1},'autocorr',dsz,num2str(day),'.mat');
...
autocorr_map{day}{epoch} = zeros(size(index,1),size(index,1));
...
for ii = 1:size(index,1)
    ...
    for jj = 1:size(index,1)
        if ii == jj
            ...
            spike_autocorr = spikexcorr(spike_data.data(:,1)', spike_data2.data(:,1)', winbin, lag_window);
            unnorm = [unnorm;spike_autocorr.c1vsc2];
            
            % normalise nspikes2 is the reference
            spike_autocorr.c1vsc2=spike_autocorr.c1vsc2./sqrt(spike_autocorr.nspikes2*spike_autocorr.nspikes1);
            nspikesc2=[nspikesc2;sqrt(spike_autocorr.nspikes2*spike_autocorr.nspikes1)]; % normalise by the number of comparisons made
            
            %             if max(spike_autocorr.c1vsc2)>0
            %
            %                 figure;
            %                 plot(spike_autocorr.time(:)',spike_autocorr.c1vsc2(:)');
            ...
            [Max_autocorr, max_i] = max(spike_autocorr.c1vsc2);
            
            autocorr = [autocorr;spike_autocorr.c1vsc2];
            time = [time;spike_autocorr.time];
            max_autocorr = [max_autocorr;Max_autocorr];
            ...
            win=gausswin(64,1);
            win=win./sum(win);
            %convautocorr{ii,jj}=conv(spike_autocorr.c1vsc2,win);
            convautocorr=[convautocorr;conv(spike_autocorr.c1vsc2,gaussFilter,'same')];
            area=[area;trapz(spike_autocorr.time,conv(spike_autocorr.c1vsc2,gaussFilter,'same'))];
            
            %             autocorr_map{day}{epoch}(ii,jj) = max_autocorr;
...
%     dsz = '0';
% end
% filename=strcat(f.animal{2},f.animal{1},'autocorr',dsz,num2str(day));
% save(filename,'autocorr_map','lag_map');
% figure(day)
...
% set(gca,'FontSize',15);
%
% image(autocorr_map{day}{epoch}.*10)
% title(sprintf('Pairwise X-Corr (D%d E%d)',day,epoch));
%
% xlabel('neuron');
% ylabel('neuron');
%
% [s,mess,messid] = mkdir(sprintf('%sPlot/autocorr/',f.animal{2}));
% print(sprintf('%sPlot/autocorr/%s_autocorr_d%d',f.animal{2},f.animal{1},index(1)),'-depsc');

if isempty(autocorr)
    autocorr=zeros(size(index,1)*size(index,1), lag_window/winbin*2);
...
out.time = time;
out.max_autocorr=max_autocorr;
out.convautocorr = convautocorr;
out.area=area;
out.unnorm=unnorm;
