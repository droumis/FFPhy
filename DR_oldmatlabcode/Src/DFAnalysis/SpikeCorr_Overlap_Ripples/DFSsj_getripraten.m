...
if figopt1 == 1
    ...
    if epcomb==1
        ...
        % anova_rm: pgrp=0.5098; ptime=0; pint=0.0596
        [panovarm_allriprate] = anova_rm({[Conriprate_ep1(:),mean([Conriprate_ep3(:),Conriprate_ep5(:)],2)] [Expriprate_ep1(:),mean([Expriprate_ep3(:),Expriprate_ep5(:)],2)]},'on');
        ...
        %axis([0 4 0 0.5])
        if savefig1==1,
            figfile = [figdir,'RippleRateRest_PostComb'];
            print('-dpdf', figfile);
            print('-djpeg', figfile);
    ...
    if epcomb==1
        ...
        errorbar(1:length(Expday_riprate_post),mean(Expday_riprate_post,1),sem(Expday_riprate_post,1),'ro-','MarkerSize',10,'LineWidth',2);
        % Anova
        % repeated measures anova. Grp X Day Compare separately, the pre curves for con and exp; and the post curves for con and exp
    ...
    if epcomb==1
        ...
        errorbar(1:length(Expfrday_riprate_post),mean(Expfrday_riprate_post,1),sem(Expfrday_riprate_post,1),'ro-','MarkerSize',10,'LineWidth',3);
        % Anova
        % repeated measures anova. Grp X Day Compare the post/pre curves for con and exp
        % Pgrp=0.3837, ptime=0.2601, pint=0.8059,
        ...
        set(gca,'YLim',[0 2.1]);
        if savefig1==1,
            figfile = [figdir,'RippleRateRest_FracChgVsDay_PostComb'];
            print('-dpdf', figfile);
            print('-djpeg', figfile);
            saveas(gcf,figfile,'fig');
        end
