...
if runscript == 1
    ...
    if savedata == 1
        clear figopt1 runscript savedata dosepdays dostimisi doisisepdays doposdistr dopossepdays dostimlearncorr doposquant dovelbefaft
        save(savefile);
    end
...
if figopt1 == 1
    ...
    Exp_meanstimrate(1)=Exp_meanstimrate(1)-0.2;
    ...
    Con_daystimrate(1)=Con_daystimrate(1)+0.25;
    Con_daystimrate(5)=Con_daystimrate(5)-0.25;
...
if dovelstim_befaft==1,
    ...
    Conmean_velstim_diff_byanim(2)=Conmean_velstim_diff_byanim(2)+0.2;
    jit=[0,0.05,0.1,0.15];
    plot(1+jit, Expmean_velstim_diff_byanim,'ro','MarkerSize',12,'Linewidth',3);
    plot(2+jit, Conmean_velstim_diff_byanim,'bd','MarkerSize',12,'Linewidth',3);
    [p_velstim_diff,h_velstim_diff] = ranksum(Expmean_velstim_diff_byanim,Conmean_velstim_diff_byanim); %p=0.1143
    axis([0.5 2.5 -2 0.5]);
    set(gca,'XTick',[1 2],'XTickLabel',{'Exp';'Con'}','FontSize',xfont,'Fontweight','normal');
    ...
    if savefig1==1,
       figfile = [figdir,'velstim_diff_byanim_1swin'];
       print('-dpdf', figfile);
       print('-djpeg', figfile);
       saveas(gcf,figfile,'fig');
    end
