...
#define NEVENT_MEASURES    9     	/* nine fields per event:
				   start_index
				   end_index
				   peak_value
				   peak_index
...
void calcmeasures(double *eptr, double *startptr, double *endptr, double *dataptr, int mindur, double thresh);
void Usage();
    ...
    const mxArray *prhs[])         /* array of pointers to input arguments */
{
	...
	/* check to see if we are below (thresh / 2), and if so, set the
	 * lastbaseptr */
	if (*dataptr <= (thresh / 2)) {
	    lastbaseptr = dataptr;
	}
	...
	if (*dataptr >= thresh) {
		...
		/* continue until we go below half of threshold */
		while ((*dtmp > (thresh / 2) ) && (dtmp <= dataendptr)) {
		    dtmp++;
		}
		nextbaseptr = dtmp - 1;
