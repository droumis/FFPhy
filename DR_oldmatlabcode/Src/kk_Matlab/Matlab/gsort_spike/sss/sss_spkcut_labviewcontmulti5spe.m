...
%sss_spkcut_labviewcontmulti5spe('spike.dat', [1:4], 32000, 4, 1);
%VER 5: JUN 23, 2006
...
% sss_spkcut_labviewcontmulti5({'spike.dat'},[1:3],32000,4,1);
        ...
        switch length(ch),
            ...
            case 4,
                indices = find( (data(:,1)<=thr) | (data(:,2)<=thr) | (data(:,3)<=thr) | (data(:,4)<=thr));
                 %indices = find((data(:,2)<=thr) | (data(:,3)<=thr) | (data(:,4)<=thr));
                %indices = find( (data(:,1)<=thr) | (data(:,2)<=thr) | (data(:,3)<=thr) | (data(:,4)<=thr) | (data(:,1)>=-thr) ) ; 
            otherwise
                indices = find( (data(:,1)<=thr) | (data(:,2)<=thr) | (data(:,3)<=thr) | (data(:,4)<=thr));
        end
