function [spikes,aspkfile]=sss_spkcut_labviewcont5_thctx1(files, ch, Fs, sdcuts, threshold, stimcode, params, consweep, sweepall, sweepstim)

%sss_spkcut_labviewcont5_thctx1('spike.dat',[1:4],32000,4,1);
%VER 5: JUN 23, 2006
...
% sss_spkcut_labviewcont5_thctx1('spike.dat',[1:4],32000,4,1);
% sss_spkcut_labviewcont5('spike.dat',[1:3],32000,4,1);
...
addsweep=2100; addtrial=0; maxsweep=0;
allspikes.waveforms=[];
for nch=1:length(ch)
...
%             |(data(:,1)>=-thr) | (data(:,2)>=-thr) | (data(:,3)>=-thr) | (data(:,4)>=-thr) ); % values over threshold
