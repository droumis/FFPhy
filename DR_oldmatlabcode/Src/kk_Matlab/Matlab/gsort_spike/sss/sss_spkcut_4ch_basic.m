function [spikes,aspkfile]=sss_spkcut_2ch_basic(files1, files2, files3, files4, sdcuts, stimcode, params, consweep, sweepall, sweepstim)
...
if (nargin < 5), sdcuts=5; end
if (nargin < 6), stimcode=(1:size(files1,1))-1; end %% default stimcode = 0,1,2...lth(files)-1
if (nargin < 7), 
...
%     cuts = find (data == -9999);          % Cuts should be same as Channel1
    ...
    %Ch3
    name3 = strtok(files3{lp_f},'.');
    data=load ([files3{lp_f}]);
    h = waitbar(0,'Reorganizing the data into matrix...');
    data = [-9999 data' -9999];
    cuts = find (data == -9999);
    datmat3 = zeros (length(cuts)-1,(cuts(2)-cuts(1))+1); 

    for lp_c = 1: (length (cuts)-1)
        datmat3 (lp_c,:)= data (cuts(lp_c):cuts(lp_c+1)); 
        waitbar(lp_c/(length (cuts)-1));
        a=1;
    end
    clear data ndata h lp_c
    ...
    %Ch4
    name4 = strtok(files4{lp_f},'.');
    data=load ([files4{lp_f}]);
    h = waitbar(0,'Reorganizing the data: Ch1 into matrix...');
    data = [-9999 data' -9999];
%     cuts = find (data == -9999);          % Cuts should be same as Channel1
    datmat4 = zeros (length(cuts)-1,(cuts(2)-cuts(1))+1); 
    
    for lp_c = 1: (length (cuts)-1)
        datmat4 (lp_c,:)= data (cuts(lp_c):cuts(lp_c+1)); 
        waitbar(lp_c/(length (cuts)-1));
    end
    clear data ndata h lp_c
    ...
    for lpthrs=1:length(thrs);
        ...
        datmat33=[]; fstimes3=[]; shape3=[]; indct3=1; fwaves3=[];
        datmat44=[]; fstimes4=[]; shape4=[]; indct4=1; fwaves4=[];
        ...
        for lp_sw = 1: size (datmat1,1)
            ...
            %Ch3
            tempvec3 = datmat3(lp_sw,:);
            temp3 = tempvec3 (8:end-1); % clean the non A/D values in the temp
            %Ch4  
            tempvec4 = datmat4(lp_sw,:);
            temp4 = tempvec4 (8:end-1);
...
%                     indct2=indct2+1;
                ...
                % For Ch3
                for lp_sp=1:length(g2_sp);
                    shape3= temp3(g2_sp(lp_sp)-spikestarts:g2_sp(lp_sp)+(47-spikestarts));            
                    fwaves3= [fwaves3; shape3];
                end
                
                % For Ch4
                for lp_sp=1:length(g2_sp);
                    shape4= temp4(g2_sp(lp_sp)-spikestarts:g2_sp(lp_sp)+(47-spikestarts));            
                    fwaves4= [fwaves4; shape4];
                end
...
%       datmat22= [datmat22 fwaves22 datmat22(:,1)]; ftimes22= [datmat22(:,6)];
        ...
        spikes.waveforms_ch3=fwaves3; spikes.waveforms_ch4=fwaves4;
...
%         if autosweepall==1, sweepall=[sweepall; newsweep]; end
        ...
        allspikes.waveforms_ch3=[allspikes.waveforms_ch3;fwaves3];       
        allspikes.waveforms_ch4=[allspikes.waveforms_ch4;fwaves4];
        ...
        save ([ name1 '_tet_thr' [num2str(round(sdcuts(lpthrs)))]], ...
...
allspikes.waveforms=[allspikes.waveforms_ch1 allspikes.waveforms_ch2 allspikes.waveforms_ch3 allspikes.waveforms_ch4];
...
save ([ recname 'tet_all_thr' [num2str(round(sdcuts(lpthrs)))]], 'spikes');
