function varargout = sss_ampfeatureselect(varargin)
% SSS_AMPFEATURESELECT M-file for sss_ampfeatureselect.fig
%      SSS_AMPFEATURESELECT(axes_handle) creates a new SSS_AMPFEATURESELECT.  Not
%      a user GUI; intended for internal use by SS GUI applications.

%   Last Modified By: sbm on Tue Jul 19 21:32:10 2005
...
gui_State = struct('gui_Name',       mfilename, ...
                   ...
                   'gui_OpeningFcn', @sss_ampfeatureselect_OpeningFcn, ...
                   'gui_OutputFcn',  @sss_ampfeatureselect_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before sss_ampfeatureselect is made visible.
function sss_ampfeatureselect_OpeningFcn(hObject, eventdata, handles, varargin)
...
set(handles.figure_sss_ampfeatureselect, 'Name', [upper(handles.switchaxis) '-Axis Selection']);

% Move this window to the current location of the corresponding label for convenience.
set(handles.figure_sss_ampfeatureselect, 'Units', 'Pixel');
newpos = labelposition_screen_coords(handles);
currpos = get(handles.figure_sss_ampfeatureselect, 'Position');
set(handles.figure_sss_ampfeatureselect, 'Position', [(newpos-currpos(3:4)*0.75) currpos(3:4)]);
set(handles.figure_sss_ampfeatureselect, 'Color', get(handles.axes_handle, [handles.switchaxis 'Color']));

% Update guidata structures
ssg.([handles.switchaxis 'control']) = handles.figure_sss_ampfeatureselect;
guidata(handles.axes_handle, ssg);
guidata(hObject, handles);


% Outputs from this function are returned to the command line.
function varargout = sss_ampfeatureselect_OutputFcn(hObject, eventdata, handles)
...
switch (choice), 
    ...
    case 'PC', % (for principal components, we might need to compute the first time)
        if (~isfield(ssg.ss_object, 'pca'))
            ...
            set([handles.figure_sss_ampfeatureselect, parentfig], 'Pointer', 'watch');

			[pca.scores,pca.u,pca.s,pca.v] = pcasvd(ssg.ss_object.waveforms);
			ssg.ss_object.pca = pca;
			
            set(handles.figure_sss_ampfeatureselect, 'Pointer', 'arrow');
    ...
    case 'Event Time',
        ...
        vector = ssg.ss_object.swtimes;  %%% SHANTANU, MAR28, 2006
...
guidata(handles.axes_handle, ssg);
guidata(handles.figure_sss_ampfeatureselect, handles);
figure(handles.figure_sss_ampfeatureselect);
