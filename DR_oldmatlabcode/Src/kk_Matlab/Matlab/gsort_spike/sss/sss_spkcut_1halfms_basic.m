function [spikes,aspkfile]=sss_spkcut_1halfms_basic(files, sdcuts, stimcode, params, consweep, sweepall, sweepstim)
    ...
    for lpthrs=1:length(thrs);
        ...
        for lp_sw = 1: size (datmat2,1)
            ...
            if length(find(temp>=thr))>=1
                ...
                g2_sp=g2_sp(find ((g2_sp>spikestarts+1) & (g2_sp<size(temp2,2)-(56-spikestarts))));
                ...
                for lp_sp=1:length(g2_sp);
                    temps= temp(g2_sp(lp_sp)-spikestarts:g2_sp(lp_sp)+(47-spikestarts));
