function [spikes,aspkfile]=sss_spkcut_labviewcontmulti4(files, ch, Fs, sdcuts, threshold, stimcode, params, consweep, sweepall, sweepstim)
...
% sss_spkcut_labviewcontmulti4({'spike.dat';'spike.dat';'spike.dat';'spike.dat';'spike.dat';'spike.dat';'spike.dat';'spike.dat';'spike.dat';'spike.dat'},[1:4],32000,4,1);
...
while stop==0
    ...
    data=labview_loadspike4(fn,4,Fs,t1,t2);  %% NEED TO READ ALL 4 channels
    data=data(ch,:); data=data';
   
    
    %%%%% REMOVE HIGH NOISE EPOCHS: CHECK FEASIBILITY %%%%%%
    %    
            x=find((data(:,2)<-0.4)|(data(:,2)>0.4));
    ...
    for lpthrs=1:length(thrs);
        ...
        %save spktimes allspikes.fstimes    
        if consweep==1, 
