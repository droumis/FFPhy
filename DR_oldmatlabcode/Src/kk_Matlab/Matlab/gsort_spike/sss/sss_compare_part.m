function sss_compare_part (spikes,clustnum,arfp,Coloropt, parameters)
...
store_times=[];
    ...
    clr = ['w' 'y' 'm' 'b' 'r' 'c' 'w' 'y' 'm' 'b' 'c' 'r' 'w' 'y' 'm' 'b' 'c' 'r'];
    ...
    for lp=1:size(clustnum,1)
        ...
        if (Coloropt==0); % if units combined 
            ...
            subplot (4,6,[1:18])
            if (size (tempwaves,1) > 5000)
                unit_sm = tempwaves(randperm (size (tempwaves,1)),:);
                unit_sm = unit_sm (1:5000, :);
