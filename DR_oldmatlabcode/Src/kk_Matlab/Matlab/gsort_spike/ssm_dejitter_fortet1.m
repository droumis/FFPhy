function spikes = ssm_dejitter_fortet1(spikes, maxshift, nch)

%% SHANTANU - Oct 12, 2007 
%% REDO SSM_DEJITTER FOR TETRODE: METHOD1 - DEJITTER ON THE BIGGEST
%% WAVEFORM OF THE 4 CHANNELS. ALTERNATIVE IS TO DEJITTER EACH CHANNEL
%% SEPARATELY ONLY FOR WAVEFORMS THAT EXCEED THREHSOLD ON THAT CHANNEL

%% USING NEGATIVE THRESHOLD
...
numsamplesperchan =numsamples/nch;

maxchunk = floor(((numspikes - 1) / spline_chunk));

%%%% FIND THE WAVEFORM TO DEJITTER

%% Method A - Find Minima among 4 waves in peak region
% for ch=1:nch
%     cmd = sprintf ( 'minwave(:,ch)=min(spikes.waveforms_ch%d(:,spikes.threshT:spikes.threshT+5),[],2);',ch); 
%     eval(cmd);
% end

%% Method B - Find Minima among 4 waves at spikes.threshT+1
for ch=1:nch
    cmd = sprintf ( 'thrsval(:,ch)=spikes.waveforms_ch%d(:,spikes.threshT+1);',ch); 
    eval(cmd);
end

[peakvalue,channel_idx]=min(thrsval,[],2);

waveforms=zeros(numspikes,numsamples/nch);
for i=1:numspikes
    cmd=sprintf('waveforms(i,:) = spikes.waveforms_ch%d(i,:);',channel_idx(i)); eval(cmd);
end

%%%%%%%%%% FIND FIDUCIALS using these waveforms%%%%%%%%%%%%%
% first, get a mask over the peaks
isThreshLo = (waveforms(:, spikes.threshT+1) <= spikes.threshV(1));
[h, w, pl, mask] = thresholded_peaks_fortet(spikes, waveforms);
...
fiducials = (waves * [1:numsamplesperchan]') ./ sum(waves, 2);   %%% Weighted sum over waves=waves.*mask
...
resample_indsperchan = repmat([1:numsamplesperchan], [numspikes, 1]) + repmat(shifts, [1, numsamplesperchan]);

% Which regions are invalid?
left_valid = max(find(any(resample_indsperchan < 1))) + 1;            if (isempty(left_valid)), left_valid = 1;  end;
right_valid = min(find(any(resample_indsperchan > numsamplesperchan))) - 1;  if (isempty(right_valid)), right_valid = numsamplesperchan; end;
...
spikes.waveforms=[];
for ch=1:nch
    cmd = sprintf ( 'spikes.waveforms_ch%d = resampled(:, (numsamplesperchan*(ch-1))+left_valid:(numsamplesperchan*(ch-1))+right_valid);',ch);
    eval(cmd);
    cmd = sprintf ( 'spikes.waveforms = [spikes.waveforms, spikes.waveforms_ch%d];',ch);
    eval(cmd);
end

waveforms = waveforms(:, left_valid:right_valid);
...
meanlevel = mean(waveforms(:));   % estimate of baseline
notallowed = [1:(spikes.threshT-maxshift-1),(spikes.threshT+maxshift+1):(size(waveforms, 2))];

deflection = mean(abs(waveforms - meanlevel), 1);  % mean deviation from baseline
deflection(notallowed) = 0;

%%%%%% SHANTANU - DONT NEED THIS AS THRESHT ALREADY UPDATED, AND I AM USING THRESHT+1
[junk, spikes.threshT] = max(deflection);  % max deflect near old threshT marks new threshold marker
%%%%%%%%%%%%%
threshValues = waveforms(:, spikes.threshT);  % values at new marker
