function [spikes,draw] = sss_spksort(spikes, parameters, spkfile, Spiketype)
...
if ((nargin < 2) || isempty(parameters))
    parameters.outliers_a =1e-4;
    parameters.kmeans_options.divisions=6; % 2^6=64;
    parameters.reint_out=1; parameters.tmin=0.001; parameters.tref=0.002; parameters.cutoff = 0.01;
end

if (nargin < 3), Spiketype = 1; end

nch = 1; % For single channel data only
spkfile = strtok(spkfile,'.');
...
maxshift = 3;
spikes = ssm_dejitter(spikes, maxshift);
...
spikes = ssm_outliers(spikes, parameters.outliers_a, nch);
...
Subset=1:size(spikes.waveforms,2);
spikes = sss_kmeans(spikes, parameters.kmeans_options, nch, Subset);
    % NOTE - 04/23/10, CHANGINF FROM SSM_KMEANS HERE BECAUSE DOESNT WORK ON 64-BIT LINUX MACHINE
...
spikes = ssm_energy(spikes, nch, Subset);
...
% spikes.stimonset=200; window=50; 
