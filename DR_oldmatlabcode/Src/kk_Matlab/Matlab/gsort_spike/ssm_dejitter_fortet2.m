function spikes = ssm_dejitter_fortet2(spikes, maxshift, nch)

%% Method: Add all channels to get one representative channel as the
% waveform: figure out the shift based on that, and then de-jitter all
% channels based on that shift.

% Shantanu, 04/26/2010 

%% 
% SSM_DEJITTER  Aligns waveform peaks.
...
% Old Method - Find biggest among 4 waves at spikes.threshT+1
% valid_ch = find(spikes.threshV(:,2)<1000); 
% for vch=1:length(valid_ch)
%     ch = valid_ch(vch);
%     cmd = sprintf ( 'thrsval(:,ch)=spikes.waveforms_ch%d(:,spikes.threshT+1);',ch); 
%     eval(cmd);
% end
% [peakvalue,channel_idx]=max(abs(thrsval),[],2);
% waves=zeros(size(spikes.waveforms_ch1));
% for i=1:numspikes
%     cmd=sprintf('waves(i,:) = spikes.waveforms_ch%d(i,:);',channel_idx(i)); eval(cmd);
% end
% lothrs = min(spikes.threshV(valid_ch,1));
% hithrs = min(spikes.threshV(valid_ch,2));
% 


% Current Method: Add waveforms on all valid channels
%Checking validity - for +ve thresholds for now - can incorporate -ve also
valid_ch = find(spikes.threshV(:,2)<1000); 
waves=zeros(size(spikes.waveforms_ch1));
for vch=1:length(valid_ch)
    ch=valid_ch(vch);
    cmd=sprintf('waves = waves + spikes.waveforms_ch%d;',ch); eval(cmd);
end
...
if ~any(spikes.threshV(:,1) ~= -Inf)
    ...
    isThreshLo = (waves(:, spikes.threshT+2) <= lothrs);
end
[h, w, pl, mask] = thresholded_peaks_fortet(spikes, waves);
...
waves(~isThreshLo, :) = waves(~isThreshLo, :) - hithrs;
...
target = round(nanmean(fiducials));
