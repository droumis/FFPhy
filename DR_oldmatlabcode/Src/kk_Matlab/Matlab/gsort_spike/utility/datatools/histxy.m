...
x = x(:);  M = length(x);
y = y(:);  N = length(y);
if (~isnumeric(x) || ~isnumeric(y) || (M~=N))
	error('First 2 input arguments must be numeric matrices of equal size.');
end
...
[x,min1,max1] = rescale_data(x,1,D(1));   x = round(x);
[y,min2,max2] = rescale_data(y,1,D(2));   y = round(y);
