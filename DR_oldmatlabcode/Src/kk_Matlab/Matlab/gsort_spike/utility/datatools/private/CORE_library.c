                 ...
                 double *minval, int *minind, double *maxval, int *maxind)
{
  ...
  *minval =  input[0];     *minind = 0;   // init min/max to first element
  *maxval =  input[0];     *maxind = 0;

  for (i = 1; i < M; i++) {               // go through keeping track of min/max till now
    if (input[i] < *minval) {*minval = input[i];  *minind = i;}
    if (input[i] > *maxval) {*maxval = input[i];  *maxind = i;}
  }
}
// ------------------------------------------------------------------------
...
void norm_squared_rows(double *matrix, int M, int N, double *normsqr)
{
  ...
  for (i = 0; i < M; i++) {
    normsqr[i] = 0;
    for (j = 0; j < N; j++) {
      temp = matrix[i + j*M];   // use temp to avoid matrix access 2x in next line
      normsqr[i] += temp*temp;
    }
  }
}
                                ...
                                int M, int N, int D)
{
  ...
  // Distances are computed as follows:
  //     dist(x,y)^2 = (x-y)'(x-y)
  // Note that this handles dist(x,x)^2 == 0 w/o
  // the roundoff error of the previous function.
  for (n = 0; n < N; n++) {
    for (m = 0; m < N; m++) {
      dists[m + n*M] = 0;
      for (d = 0; d < D; d++) {
