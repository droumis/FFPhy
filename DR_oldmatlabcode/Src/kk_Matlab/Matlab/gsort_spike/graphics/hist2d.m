...
elseif (dims(2) == 2)  % scatter plot density
    ...
    %data1 = data_mtx(:,1); data2=data_mtx(:,2);
    [data1,min1,max1] = rescale(data_mtx(:,1),1,bins); % SJ 05/31/10
    [data2,min2,max2] = rescale(data_mtx(:,2),1,bins); % SJ 05/31/10
    
    % The bin centers are built based on range of the unscaled data.
    
    x_inds = linspace(min1,max1,bins); % SJ 05/31/10
    y_inds = linspace(min2,max2,bins); % SJ 05/31/10
    ...
    [data_mtx,oldmin,oldmax] = rescale_data(data_mtx,1,bins);
