function spikes = ssm_aggregate(spikes, parameters)
...
%   Last Modified By: sbm on Sun Oct  9 22:53:16 2005
...
while (any(any(triu(untested,1)==3)))   % only the upper triangular half is meaningful
    ...
    %%% Shantanu OCT 2007- to make Aggregation visible
    pause(0.05);
    ...
    if (isfield(spikes, 'fstimes'))  % if we were given spike times, use them ...
	    ...
	    %disp('Where is spikes.fstimes? Use correct format for spktimes') 
        end        
