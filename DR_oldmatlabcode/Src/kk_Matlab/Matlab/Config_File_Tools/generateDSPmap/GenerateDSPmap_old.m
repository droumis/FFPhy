function GenerateDSPmap(tetrodemap, varargin)
%GenerateDSPmap(tetrodemap, options)
%
%This function creates and writes an nspike config file in the current working
%directory called 'dsplist.txt'.  
...
%
%__Options__
%'spectialtetrodes' - give a vector of tetrode numbers that do not have 4
%channels.  These tetrodes can't be viewed in the spike machines.
%
%'master' - give the name of the master machine- required
%
%'slave1' - required
%
%'slave2' - if you are using more than 12 full tetrodes, this needs to be declared. (default '')
%
%'rt' - if you are collecting position data, the name of the rt machine needs to be declared. (default '')
%
...
if ~(sum(rowcount(fulltetrodes,tetrodemap(:)) == 4) == length(fulltetrodes))
    problemtetrodes = fulltetrodes(find(rowcount(fulltetrodes,tetrodemap(:)) ~= 4))';
    error(['Any tetrodes not specified in ''specialtetrodes'' must have four channels defined. This is not the case. Problem tetrodes: ',num2str(problemtetrodes),'.']);
end
...
for j = 1:length(values)
    tmpval = values(j);
    count = rowcount(tmpval,tetrodemap(:));
    channels(find(tetrodemap == tmpval),2) = [0:count-1]';
    [rowval, colval] = find(tetrodemap == tmpval);
    eegchannels = [eegchannels; [tmpval dspmap(rowval(1),colval(1))]];
end

tmpdspmap = dspmap(:);
channels = [channels tmpdspmap(1:size(channels,1))];
...
fprintf(FID,['datatype	',mastermachine,'	CONTINUOUS	POSITION	DIGITALIO\n', ...	
...
'dioport	2	output\n', ...
'dioport	3	output\n', ...
'eegtracelength	4.000000\n', ...
'calibrationfile	/home/lorenlab/NSpike/nspikecalfile\n\n']);
%***********************************************
