...
%       be processed
%      #(s) indicate the set of tetrodes to process
...
if (nargin < 6) || isempty(min_suprathresh_duration)
   min_suprathresh_duration = 0.015;
end

if (nargin < 7) || isempty(nstd)
   nstd = 2;
end

if (nargin < 8) || isempty(maxpeak)
   maxpeak = 1000;
end
...
load(fullfile(directoryname,sprintf('%spos%02d.mat',fileprefix,d)));
...
for t = tet
    tmpflist = dir(fullfile(directoryname,'EEG',sprintf('%sripple%02d-*-%02d.mat', fileprefix, day, t)));
    for i = 1:length(tmpflist)
        % load the ripple file
        load(fullfile(directoryname,'EEG',tmpflist(i).name));
        % get the epoch number
        dash = find(tmpflist(i).name == '-');
        e = str2num(tmpflist(i).name((dash(1)+1):(dash(2)-1)));

        % convert the ripple envelope field to double
        if size(ripple{d}{e}{t}.data,2) == 3
           renv = double(ripple{d}{e}{t}.data(:,3));
        else
           renv = double(ripple{d}{e}{t}.data(:,2));
        end
...
save(fullfile(directoryname,sprintf('%sripples%02d.mat', fileprefix, d)), 'ripples');
