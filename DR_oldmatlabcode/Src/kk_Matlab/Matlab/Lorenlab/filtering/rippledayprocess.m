...
defaultfilter = 'rippledayprocess_filter.mat';
...
if isempty(f) 
    ...
    eval(['load ', f]);
    fprintf('Using non default filter');
    if isfield(ripplefilter,'descript');
      fprintf(':\n%s\n',ripplefilter.descript);
    else
      fprintf('\n');
    end
end
...
% 			ripplefilter, 'int16', 1); 
  ...
  if ~isempty(varprefix)
    varname = sprintf('%sripple',varprefix);
    eval(sprintf('%sripple = ripple;',varprefix));
    clear ripple
  else
    varname = 'ripple';
  end
	...
	ripplefile = sprintf('%s/EEG/%s%sripple%02d-%d-%02d.mat', ...
			    directoryname, fileprefix, varprefix, day, epoch, tet);
        save(ripplefile, varname);
	clear(varname);
    end
