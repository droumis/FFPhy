...
%  2. 'excludetimelist' which indicates that the function will return a cell
...
for an = 1:length(f)
    ...
    for i = 1:length(timefilter)
        ...
        if (~strcmp(timefilter{i}{2}, 'excludetimelist'))
            %evaluate the filter string for every structure and return results in a cell array
            filterresults{i} = evaluatefilter2(dataStruct, timefilter{i}{2}); 
            filtertype(i) = 1;
        else
            % if this is an exclude time list, save it and note the type in filtertype
    ...
    for i = 1:length(f(an).epochs)
        ...
        for j = 1:size(f(an).epochs{i},1)
            ...
            for k = 1:length(timefilter)
                if (filtertype(k) == 1)
                    f(an).excludetime_vector{k} = filterresults{k};
