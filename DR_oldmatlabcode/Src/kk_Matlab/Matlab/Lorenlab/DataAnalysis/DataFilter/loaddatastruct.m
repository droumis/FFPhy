...
if ~exist(animaldir,'dir')
   error([animaldir,' not found']);
end

datafile = dir(fullfile(animaldir, [animalprefix, datatype,'.mat']));
if ~isempty(datafile)
  load(fullfile(animaldir,datafile.name));
  eval(['out = ',datatype,';']);
  return;
end


datafiles = dir(fullfile(animaldir, [animalprefix, datatype,'*','.mat']));
