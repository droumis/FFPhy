    ...
    for i = 1:length(f(an).epochs)
        ...
        for j = 1:size(f(an).epochs{i},1)
          if iscell(filterString)
            if iscell(filterString{an})
              f(an).data{i}{j} = filterString{an}{i}{j};
            else
              f(an).data{i}{j} = filterString{an};
            end
          else
            % f(an).data{i}{j} = evaluatefilter(tetrodeinfo{f(an).epochs{i}(j,1)}, filterString);
