...
for an = 1:length(f)
   ...
   for i = 1:length(f(an).function.loadvariables)
      if sum(strcmp(f(an).function.loadvariables{i}, {'eeg','theta','ripple','wideripple','gamma','delta','hp','tdr'}))
         eval([f(an).function.loadvariables{i},' = loadeegstruct(animaldir, animalprefix, f(an).function.loadvariables{i}, totaldays);']);
%          eval([f(an).function.loadvariables{i},' = loadeegstruct(animaldir, animalprefix, f(an).function.loadvariables{i}, totaldays, totalepochs, totaltetrodes);']);
      else
   ...
   for g = 1:length(f(an).epochs)
      ...
      for e = 1:size(f(an).epochs{g},1)
         ...
         if isstruct(fout)
           if length(fout) == length(f(an).data{g}{e})
            f(an).output{g}(e,ismember(totaltetrodes,f(an).data{g}{e})) = fout;
           elseif prod(size(fout)) == 1
            f(an).output{g}(e) = fout;
          else
            error('In singletetrodeanal, analysis function must return struct array of size tetrodes or size 1.');
          end
