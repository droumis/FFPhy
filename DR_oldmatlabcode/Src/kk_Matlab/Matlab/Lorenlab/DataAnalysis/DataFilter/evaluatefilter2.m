function newstruct = evaluatefilter2(cellvar, filterString, varargin)
%
...
for i = 1:size(indexstruct.index,1) 
    ...
    for j = 1:length(variables)
        if isfield(structVar,variables{j})
            if (size(getfield(structVar,variables{j}),1) ~= size(structVar.(activeField),1))
    ...
    % if the data are row vectors, transpose them
    if (size(tmpresult,1) ~= size(structVar.(activeField),1))
        tmpresult = tmpresult';
    end
    eval(['newstruct', createcellindex(indexstruct.index(i,:)),'= [structVar.(activeField) tmpresult];']);
end
end
