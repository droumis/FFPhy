...
if ((nargin == 0)|(~ischar(varargin{1})))  %launch matclust
    global figattrib;
    ...
    if isempty(figattrib)
        ...
        'Tag','figure1','NumberTitle','off','Name','MATCLUST','Renderer','OpenGL','Visible','off');
    else
...
set(h,'KeyPressFcn','matclust(''figure1_KeyPressFcn'',gcbo,guidata(gcbo))');
...
acontext1 = uimenu(axeslistcontext,'Tag','paramaddContext1','Label','Add parameter(s)','Callback','matclust(''paramadd'',gcbo,guidata(gcbo))');
acontext2 = uimenu(axeslistcontext,'Tag','paramdeleteContext1','Label','Delete parameter','Callback','matclust(''paramdelete'',gcbo,guidata(gcbo))','UserData',1);
...
acontext1 = uimenu(axeslistcontext,'Tag','paramaddContext2','Label','Add parameter(s)','Callback','matclust(''paramadd'',gcbo,guidata(gcbo))');
acontext2 = uimenu(axeslistcontext,'Tag','paramdeleteContext2','Label','Delete parameter','Callback','matclust(''paramdelete'',gcbo,guidata(gcbo))','UserData',2);
...
if ((M(1)>figloc(1))&&(M(1)<(figloc(1)+figloc(3)))&&(M(2)>figloc(2))&&(M(2)<(figloc(2)+figloc(4)))&&(clustattrib.nodata == 0))
    ...
    switch figattrib.toolselect
        case 1 %polygon drawing tool
            if (graphattrib.polydraw == 0) %this is the first point
                ...
                set(graphattrib.polyg(a1,a2,findex).lines{clustattrib.currclust},'Color',figattrib.mixcolor(clustattrib.currclust,:));
        ...
        case 2 %square drawing tool
            ...
            set(graphattrib.polyg(a1,a2,findex).lines{clustattrib.currclust},'Color',figattrib.mixcolor(clustattrib.currclust,:));
            set(graphattrib.polyg(a1,a2,findex).lines{clustattrib.currclust},'UserData',[a1 a2 findex clustattrib.currclust]);
            graphattrib.polyg(a1,a2,findex).type{clustattrib.currclust} = 2;
            clustattrib.clusters{clustattrib.currclust}.box{a1,a2,findex} = graphattrib.polyg(a1,a2,findex).relativevertices{clustattrib.currclust};    
...
if ((M(1)>figloc(1))&&(M(1)<(figloc(1)+figloc(3)))&&(M(2)>figloc(2))&&(M(2)<(figloc(2)+figloc(4)))&&(clustattrib.nodata == 0)) %is the motion within the plot window?
	...
	currviewbox = graphattrib.viewbox(a1,:,a2);
	xrange = ((abs(diff(clustdata.datarange(:,a1))))*[currviewbox(1);currviewbox(2)])+ (clustdata.datarange(1,a1));
	yrange = ((abs(diff(clustdata.datarange(:,a2))))*[currviewbox(3);currviewbox(4)])+ (clustdata.datarange(1,a2));
...
if (graphattrib.magdrawsquare) %a magnifying sqare was being drawn
...
if (graphattrib.handdown)
	...
	XTick = newXTick;
	YTick = newYTick;
	xrange = ((abs(diff(clustdata.datarange(:,currx))))*[currviewbox(1);currviewbox(2)])+ (clustdata.datarange(1,currx));
	yrange = ((abs(diff(clustdata.datarange(:,curry))))*[currviewbox(3);currviewbox(4)])+ (clustdata.datarange(1,curry));
...
if ((graphattrib.polydraw == 0)&&(figattrib.toolselect == 3)) %arrow tool is selected
...
if ~isempty(clustdata.params)
    ...
    try    
        ...
        plotspace(end+1,1:2) = [clustdata.datarange(2,curry) clustdata.datarange(2,currx)];
         
        %make the minimum value fall on (1,1) because negative values are not
        %allowed when making an image
        
        plotspace(:,1) = plotspace(:,1) - (clustdata.datarange(1,curry));
        plotspace(:,2) = plotspace(:,2) - (clustdata.datarange(1,currx));
        ...
        graphscale = diag([(1/(currviewbox(4)-currviewbox(3)))*resolutiony/((clustdata.datarange(2,curry)-clustdata.datarange(1,curry))) ; (1/(currviewbox(2)-currviewbox(1)))*resolutionx/((clustdata.datarange(2,currx)-clustdata.datarange(1,currx)))]);
        ...
        try
            for i = 1:length(graphattrib.polyg(currx,curry,findex(fNum)).vertices)
                try
                    ...
                    set(graphattrib.polyg(currx,curry,findex(fNum)).lines{i},'Color',figattrib.mixcolor(i,:));    
...
while errormade
    ...
    a = inputdlg('Enter time filter to copy into.','Copy polygons');
...
if ~isempty(def)
    ...
    for i = 1:size(def,1)
        ...
        set(graphattrib.polyg(a1,a2,findex).lines{dest},'Color',figattrib.mixcolor(dest,:));
        graphattrib.polyg(a1,a2,findex).type{dest} = graphattrib.polyg(a1,a2,findex).type{clustnum};
...
graphattrib.polyg(a1,a2,linepick).type{clustnum} = graphattrib.polyg(a1,a2,findex).type{clustnum};
try
...
if (filename)
    ...
    drawnow;
	save(filename,'clustdata','graphattrib','clustattrib'); 
