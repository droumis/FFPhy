   ...
   const mxArray *prhs[])        /* array of pointers to input arguments */
{
   ...
   tmpcontrec.timestamp = 0;
   /* find the offsets of the first record to be included. */
   while ((tmpcontrec.timestamp < (tstart - reclen)) && (!feof(efile))) {
      readcontrec(efile, &tmpcontrec);
   /* mexPrintf("nsamp = %d, sampfreq = %f\n", tmpcontrec.numsamples, tmpcontrec.sampfreq); */
   } 
   if ((feof(efile)) | (tmpcontrec.timestamp > tend)) {
      sprintf(tmpstring, "Time range not found in file.\n");
      mexWarnMsgTxt(tmpstring);
      datatmp = mxCreateDoubleMatrix(1, 0, mxREAL);
      plhs[0] = datatmp;
      mxFree(eegfile);
      mxFree(timestring1);
      mxFree(timestring2);  
   ...
   *tmpdata = (double) tmpcontrec.timestamp / TIMESCALE;
   ...
   while(!feof(efile)) {
      ...
      if ((tmpcontrec.timestamp - lasttime) > (1.5 * reclen)) {
         sprintf(tmpstring, "Error: excessive difference between current and last timestamps: %ld vs %ld. Filling in difference with SHRT_MIN\n", tmpcontrec.timestamp, lasttime);
         mexPrintf(tmpstring);
         ...
         numinterp = round(((double)(tmpcontrec.timestamp - lasttime) - reclen) / (double) TIMESCALE * (double) tmpcontrec.sampfreq);
         sprintf(tmpstring, "Adding %d samples\n", numinterp);
         mexPrintf(tmpstring);
         for (k = 0; k < numinterp; k++) {
            eegdata[currenti++] = (double) SHRT_MIN;
         }
         lasttime == tmpcontrec.timestamp;
      }
      /* copy the samples to tmpdata */
