...
%     samples: a double vector of voltages, expressed in microvolts
...
second). */
const int32_t MAX_NUM_SAMPLES = 1e7;
...
void mexFunction(int nlhs, mxArray* plhs[], int nrhs, const mxArray* prhs[]) {
  ...
  if (nrhs == 1) {
    ...
    if (!mxIsClass(prhs[1], "uint32") || (mxGetM(prhs[1]) != 1) ||
        (mxGetN(prhs[1]) != 1) || (mxGetNumberOfDimensions(prhs[1]) > 2)) {
      mexErrMsgTxt("Second argument must be uint32 timestamp\n");
    } else {
      start_timestamp = (uint32_t) mxGetScalar(prhs[1]); 
    }
    if (!mxIsClass(prhs[2], "uint32") || (mxGetM(prhs[2]) != 1) ||
        (mxGetN(prhs[2]) != 1) || (mxGetNumberOfDimensions(prhs[2]) > 2)) {
      mexErrMsgTxt("Third argument must be uint32 timestamp\n");
    } else {
  ...
  if (nrhs == 3) {
    ...
    while (current_record.timestamp + (uint32_t) ceil(TS_PER_SEC *
        (current_record.num_samples - 1) / nominal_sampling_rate) <
        start_timestamp) {
      ...
      timestamps */
      if (abs(next_record.sampling_rate - nominal_sampling_rate) >
          SAMP_RATE_PRECISION) {
        sprintf(tmp_string, "Sampling rate of CONTINUOUS record (%f) does not match "
            "nominal sampling rate (%f)", current_record.sampling_rate, 
  ...
  fprintf(stdout,"Current record timestamp: %d\n",
      current_record.timestamp);

  /* Dynamically allocated arrays of data */
  uint32_t* num_samples;
  num_samples = mxMalloc((mwSize) sizeof(uint32_t));
  double** samples; /* jagged array of arrays */
  samples = mxMalloc((mwSize) sizeof(double*));
  ...
  num_samples[0] = 0;
  samples[0] = (double*) mxMalloc(alloc_sz);
  ...
  while (current_record.timestamp - (uint32_t) 
    ...
    for (i = 0; i < current_record.num_samples; i++) {
      samples[j-1][num_samples[j-1]] = (double) current_record.samples[i];
    ...
    if (read_continuous_record(file_handle, &next_record) == 0) {
      ...
      inplausible timestamp */
      if (abs(next_record.sampling_rate - nominal_sampling_rate) >
          SAMP_RATE_PRECISION) {
        sprintf(tmp_string, "Sampling rate of this record (%f) does not match "
      ...
      if (time_gap < -TS_PRECISION) {
        ...
        if (nrhs == 3) {
          ...
          finished chunk */
          samples[j-1] = (double*) mxRealloc(samples[j-1], (mwSize) 
              (num_samples[j-1]*sizeof(double)));
       ...
       /* LOOK!!!! j is now incremented to the next chunk
          ...
          samples = (double**) mxRealloc(samples, (mwSize) 
              (j*sizeof(double*)));
          ...
          num_samples[j-1] = 0;
          samples[j-1] = (double*) mxMalloc(alloc_sz);
  ...
  for (i = 0; i < j; i++) {
    ...
    memcpy((double*) mxGetData(out_samples), samples[i], 
        num_samples[i]*sizeof(double));
