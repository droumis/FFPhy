...
if ~isempty(posdata.data)
    ...
    for i = [1 2]
        ...
        % check first non-zero point to see if it makes sense
        firstvel = dist(tmppos(indvalid(1),:),tmppos(indvalid(2))) / ...
           time(indvalid(2)) - time(indvalid(1));
        if (abs(firstvel) > maxv)
          tmppos(indvalid(1),:) = 0;
          fprintf('!');
        end
        ...
        % =====================================================================
        % INTERPOLATION
        % Values = zero are considered missing. Interpolate, but
        % not for gaps larger than _maxdinterp_.
        % =====================================================================
        if (pinterp)
            ...
            if (sum(isnan(tmp)) > 0)
              indvalid = find(~isnan(tmp));
              indzeros = find(isnan(tmp));
            else
              indvalid = find(tmp);
              indzeros = find(~tmp);
            end
