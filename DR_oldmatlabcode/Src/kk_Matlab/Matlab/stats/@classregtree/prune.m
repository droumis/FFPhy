...
%      t1 = classregtree(meas,species,'splitmin',5,'names',varnames);
%      view(t1);
%      t2 = prune(t1,'level',1);
%      view(t2);
%
...
%   $Revision: 1.1.6.2.2.1 $  $Date: 2007/01/30 02:22:36 $
...
elseif nargin>2
   okargs =   {'level' 'nodes'};
   defaults = {[]      []};
   [eid,emsg,level,nodes] = dfswitchyard('statgetargs',okargs,defaults,varargin{:});
   if ~isempty(emsg)
