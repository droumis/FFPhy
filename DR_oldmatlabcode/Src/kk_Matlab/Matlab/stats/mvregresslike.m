...
%   NLL=MVREGRESSLIKE(X,Y,BETA,SIGMA,ALG) computes the negative log-likelihood
%   function NLL for a multivariate regression of the multivariate
...
%
%   [NLL,VARPARAM]=MVREGRESSLIKE(...) also returns an estimated covariance
%   matrix of the parameter estimates BETA.
%
%   [NLL,VARPARAM]=MVREGRESSLIKE(...,VARTYPE,VARFORMAT) specifies the type
...
%    $Revision: 1.1.6.1.2.1 $ $Date: 2007/01/16 03:37:58 $
...
[NumSamples, NumSeries, NumParams, Y, X, goodrows] = ...
          statcheckmvnr(Y, X, Param, Covar, isequal(EstMethod,3));
      
if EstMethod==3                   % mvn method
    nll = -statmvnrobj(Y,X,Param,Covar);
else                              % cwls or ecm method
    nll = -statecmobj(X,Y,Param,Covar);
end

if nargout>=2
    Info = statecmmvnrfish(Y,X,Covar,VarType,VarFormat);
    VarParam = inv(Info);
end
