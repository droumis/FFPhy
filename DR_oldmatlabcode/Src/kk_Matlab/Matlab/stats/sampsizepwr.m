...
%   N=SAMPSIZEPWR('TESTTYPE',P0,P1) returns the sample size N required for a
%   two-sided test of the specified type to have a power (probability of
...
%   the null hypothesis is true) is 0.05.  P0 specifies the parameter values
%   under the null hypothesis.  P1 specifies the value of the single parameter
%   being tested under the alternative hypothesis.
%
...
%
%      Compute the sample size N required to distinguish p=.23 from p=.2
%      with a binomial test.  The result is approximate, so make a plot to
%      see if any smaller N values also have the required power of 0.5.
...
%   $Revision: 1.1.6.3 $  $Date: 2006/10/02 16:35:09 $
...
%POWERFUNN Normal power calculation
    ...
    if tail==0
        ...
        power = normcdf(critL,mu1,S) + 1-normcdf(critU,mu1,S);
    elseif tail==1
        crit = mu0 + (mu0-norminv(alpha,mu0,S));
        power = 1-normcdf(crit,mu1,S);
    else % tail==-1
...
%POWERFUNT T power calculation
    ...
    if tail==0
        ...
        critU = -critL;
        power = nctcdf(critL,n-1,ncp) + 1-nctcdf(critU,n-1,ncp);
    elseif tail==1
        crit = tinv(1-alpha,n-1);
        power = 1-nctcdf(crit,n-1,ncp);
    else % tail==-1
        crit = tinv(alpha,n-1);
...
[errid,errmsg,alpha,tail] = statgetargs(paramNames, paramDflts, varargin{:});
if ~isempty(errid)
