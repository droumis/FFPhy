...
%   correlation matrix.  If its diagonal elements are not 1, MVTCDF scales
...
%   References:
%      [1] Genz, A. (2004) "Numerical Computation of Recutangular Bivariate
...
%   $Revision: 1.1.6.5.2.1 $  $Date: 2007/01/08 20:32:33 $
...
if abs(rho) < 1 % possibly == 0
    ...
    for i = 1:n
        ...
        if isfinite(b1) && isfinite(b2)
            p2(i) = quadl(@bvtIntegrand,loLimit,hiLimit,tol);
        else
...
if abs(rho_32) < 1 % possibly == 0
    ...
    for i = 1:n
        ...
        if isfinite(b2) && isfinite(b3) && ~isnan(b1)
            v = nu(i);
            p2(i) = quadl(@tvtIntegrand1,loLimit,hiLimit,tol/4);
        else
            % This piece is zero if either limit is +/- infinity.  If
            % either is NaN, p1 will already be NaN.
        end
...
if abs(rho_21) > 0
    ...
    for i = 1:n
        ...
        if isfinite(b1) && isfinite(bj) && ~isnan(bk)
            v = nu(i);
            p3(i) = quadl(@tvtIntegrand2,loLimit,hiLimit,tol/4);
        else
            % This piece is zero if either limit is +/- infinity.  If
            % either is NaN, p1 will already be NaN.
        end
...
if abs(rho_31) > 0
    ...
    for i = 1:n
        ...
        if isfinite(b1) && isfinite(bj) && ~isnan(bk)
            v = nu(i);
            p4(i) = quadl(@tvtIntegrand2,loLimit,hiLimit,tol/4);
        else
            % This piece is zero if either limit is +/- infinity.  If
            % either is NaN, p1 will already be NaN.
        end
...
reflect = (x > 0); % Reflect if necessary.
