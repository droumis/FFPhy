...
%   P=LINYPTEST(B,SIGMA,C,H,DFE) returns the p-value P of a hypothesis test
%   on a vector of parameters.  B is a vector of K parameter estimates.
%   SIGMA is the K-by-K estimated covariance matrix of the parameter
...
%   freedom for the SIGMA estimate, or Inf if SIGMA is known rather than
%   estimated.
%
...
%   of the hypothesis matrix H.  If DFE is Inf or is not given, T is a
...
%   $Revision: 1.1.6.1.2.1 $  $Date: 2007/01/16 03:37:57 $
...
if ~ok
    error('stats:linhyptest:BadH',...
          'H is not full rank and hypothesis are not consistent.')
end
