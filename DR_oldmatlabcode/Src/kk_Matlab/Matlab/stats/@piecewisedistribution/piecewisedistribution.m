...
%   $Revision: 1.1.6.2.2.1 $  $Date: 2007/01/16 03:38:17 $
...
st = dbstack;
name = strread(st(2).name,'piecewisedistribution.%s');
error('stats:piecewisedistribution:NoCatAllowed', ...
      'Concatenation of PIECEWISEDISTRIBUTION objects not allowed.\nUse a cell array to contain multiple objects.');
end

function throwUndefinedError
st = dbstack;
name = strread(st(2).name,'piecewisedistribution.%s');
error('stats:piecewisedistribution:UndefinedFunction', ...
      'Undefined function or method ''%s'' for input arguments of type ''piecewisedistribution''.',name{1});
end