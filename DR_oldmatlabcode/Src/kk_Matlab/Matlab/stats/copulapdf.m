...
%   $Revision: 1.1.6.3 $  $Date: 2006/11/11 22:55:04 $
...
if ischar(family)
    ...
    error('stats:copulapdf:BadFamily', ...
          'The FAMILY argument must be must be a copula family name.');
end
...
case {'clayton' 'frank' 'gumbel'}
    ...
    case 'frank'
        ...
        if alpha == 0
            ...
            y = -alpha.*expm1(-alpha) .* prod(exp(alpha.*u), 2) ...
                   ./ (1 + exp(-alpha).*prod(exp(alpha.*u), 2) - sum(exp(alpha.*u), 2)).^2;
        end
