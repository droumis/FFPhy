...
for day = days
   ...
   for i = 1:length(task{day})   
      if ((~isempty(task{day}{i})) && (strcmp(task{day}{i}.type,'run')) )
            ...
            % Get and save behavestate file data since getbehavestate has been run in
            % sj_linearizeposition1
   ...
   statefile = sprintf('%s/%sbehave%02d.mat', directoryname, fileprefix, day);
