function out = DFsj_filtercalclinfields(index, excludeperiods, spikes, linpos, pos, varargin)

% Equivalent to sj_calclinfields
%trajdata = FILTERCALCLINFIELDS(index, excludeperiods, spikes, linpos, pos, behavestate)
%trajdata = FILTERCALCLINFIELDS(index, excludeperiods, spikes, linpos, pos, behavestate, options)
%
...
%pos - pos data structure for velocity
...
% Velocity
defaultfilter = 'velocitydayprocess_filter.mat'; % Use Calebs speed filter if necessary
...
% Keep Exclude Periods in addition to speed condition imposed And
% statevector=-1 that already exists based on behavestate
statevector(find(isExcluded(statematrix.time, excludeperiods))) = -1; % Based on exclude time
...
if ~isempty(spikes)
    ...
    if (phasedist)
	spikes(:,6) = tmpspikes(:,6); % theta phase
    end
else
    spikes = [0 0 -1 0 0];
end
