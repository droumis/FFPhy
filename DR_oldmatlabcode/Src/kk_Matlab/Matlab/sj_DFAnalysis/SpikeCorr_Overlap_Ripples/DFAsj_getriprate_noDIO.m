function [out] = DFAsj_getriprate_noDIO(index, excludeperiods, ripples, DIO, varargin)
% Alt: DFAsj_getriprate_noDIO(index, excludeperiods, ripples, DIO, tetlist, varargin)
% Or specification of tetfilter or cellfilter / or cellinfo or tetinfo
% out = getriptimes(index, excludeperiods, ripples, options)
%
%   index [day epoch tetrode]
%   No Longer Used: Tetrodes is in index - tetlist is a list of tetrodes to include in the analysis
%
%   options are
%	'minenergy', E
...
for t = 1:length(tetlist)
    ...
    nrtimes = [r.timerange(1) ; reshape(nrtimes', ...
        length(nrtimes(:)), 1) ; r.timerange(2)]; % Added timerange start and end to nrtimes
    nrtimes(:,2) = 0;
    nrtimes(:,3) = 0;
...
clear times;
rip.nripples = nrip; %number of ripples on each tetrode per ms
rip.ripsize = nrip_size; %ripsize per ms: same size is reported from start to end of ripple


%calculate ripple rate
if appendindex == 0
    out = sj_calcriprate(rip, excludeperiods, numtetrodes, 'stimtime',stim_starttime);
elseif appendindex ==1
    %tmpout = sj_calcriprate(rip, excludeperiods, numtetrodes, 'stimtime',stim_starttime);
    %out = [index tmpout];
    out = sj_calcriprate(rip, excludeperiods, numtetrodes, 'stimtime',stim_starttime);
    out.index = index;
