...
function [overlap peakcomb trajpeakcomb peak1 peak2 trajpeak1 trajpeak2] = sj_calcoverlap(trajdata1,trajdata2, varargin)
...
%
% Shantanu - From calcoverlap
% I want to return more parameters, including peak rate
%
% trajdata1{traj}
...
[peakcomb, trajpeakcomb] = max(peaks);
[peak1, trajpeak1] = max(peaks1); [peak2, trajpeak2] = max(peaks2);
overlap = [];
if (peak1 >= thresh) && (peak2 >=thresh)
%if (peakcomb >= thresh)
    for traj = 1:length(trajdata1)
        if length(trajdata1)>=traj && length(trajdata2)>=traj
            if ~isempty(trajdata1{traj}) && ~isempty(trajdata2{traj})
