...
% invoking the appropriate methods of data, model, and gui.
  ...
  methods (Access = public)
    ...
    function load_assignments(self)
      ...
      % sometimes rawpos may have been generated from a larger time range
      if isempty(setdiff(self.data.timestamps(index_range),rawpos.timestamp))
        [c,ia,ib] = intersect(self.data.timestamps(index_range),rawpos.timestamp);
        rawpos.timestamp = rawpos.timestamp(ib);
        rawpos.xfront = rawpos.xfront(ib);
        rawpos.yfront = rawpos.yfront(ib);
        rawpos.xback = rawpos.xback(ib);
        rawpos.yback = rawpos.yback(ib);
      end

      if ~isequal(rawpos.timestamp, ...
