...
%     Default is a scalar, but can also be a cell array of the
%     same size as the rawpos struct.
%
...
if ~iscell(centimeters_per_pixel)
  cpp = centimeters_per_pixel;
  centimeters_per_pixel = cell(size(rawpos));
  [centimeters_per_pixel{:}] = deal(cpp);
end
for i = 1:numel(rawpos)
    position{i}.params.centimeters_per_pixel = centimeters_per_pixel{i};
end



for i = 1:numel(rawpos)
  if isempty(rawpos{i}) || isempty(rawpos{i}.data)
    continue;
  end
  ...
  if (back_exists & front_back_marker_weights~= 0)
  ...
  if (back_exists)
    back_censor_mask = censor_mask | ...
      isnan(xback) | isnan(yback) | ((xback == 0) & (yback == 0));
    t_headangle = t(~back_censor_mask);
    xfront_ha = xfront(~back_censor_mask);;
    yfront_ha = yfront(~back_censor_mask);;
    xback_ha = xback(~back_censor_mask);;
    yback_ha = yback(~back_censor_mask);;
  end
  ...
  n = numel(t);
  xfront = xfront * centimeters_per_pixel{i};
  yfront = yfront * centimeters_per_pixel{i};
  if (back_exists)
    xback = xback * centimeters_per_pixel{i};
    yback = yback * centimeters_per_pixel{i};
  end
  ...
  if (back_exists)
    ...
    x = nansum(bsxfun(@times,[xfront xback], ...
        front_back_marker_weights),2) ./ ...
        sum(front_back_marker_weights);
    y = nansum(bsxfun(@times,[yfront yback], ...
        front_back_marker_weights),2) ./ ...
        sum(front_back_marker_weights);
  else
    x = xfront;
  ...
  position{i}.data(censor_mask,2:3) = position{i}.data(censor_mask,2:3) + sqrt(-1);
  ...
  if (back_exists)
    position{i}.data(~back_censor_mask,4) = loess_head_direction( ...
      t_headangle,xfront_ha,yfront_ha,xback_ha,yback_ha, ...
      loess_halfwidth,num_loess_iterations, ...
