...
%
%%% created 3.13.12 by KK, modified simply from
%%% /usr/local/filtering/rippledayprocess.m
%
%Applies a fastfastgamma filter to all epochs for each day and saves the data in
...
%			specifies, for each day, the tetrodes for which fastfastgamma
%			extraction should be done
%
%		'f', matfilename
%			specifies the name of the mat file containing the
%			fastfastgamma filter to use 
%			(default /home/kkay/matlab/fastfastgammafilter.mat).  
%			Note that the filter must be called 'fastfastgammafilter'.
...
for day = days
	...
	fastfastgamma{day}{epoch}{tet} = filtereeg2(eeg{day}{epoch}{tet}, ...
			fastfastgammafilter, 'int16', 1); 
	clear eegrec
	...
	fastfastgammafile = sprintf('%s/EEG/%sfastfastgamma%02d-%d-%02d.mat', ...
