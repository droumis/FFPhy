function kk_spindledayprocess(directoryname,fileprefix,days, varargin)
%spindleDAYPROCESS(directoryname,fileprefix,days, options)
%
...
daytetlist = [];
f = '';
defaultfilter = (['/home/kkay/Src/Matlab/Filters/spindlefilter.mat']);
assignphase = 0;
tetfilter = '';
subtractreference = 0;
...
for day = days
    ...
    for fnum = 1:length(flist)
        ...
        temp = filtfilt(spindlefilter,1,eeg{day}{epoch}{tet}.data');
        hdata = hilbert(temp);
        env = abs(hdata);
        phase = angle(hdata);

        spindle{day}{epoch}{tet}.samprate = eeg{day}{epoch}{tet}.samprate;
        spindle{day}{epoch}{tet}.starttime = eeg{day}{epoch}{tet}.starttime;

        spindle{day}{epoch}{tet}.data(:,1) = temp;
        spindle{day}{epoch}{tet}.data(:,2) = phase*10000;
        spindle{day}{epoch}{tet}.data(:,3) = env;
        spindle{day}{epoch}{tet}.fields = ...
            'filtered_amplitude instantaneous_phase*10000 envelope_magnitude';

        if (downsample ~= 1)
        spindle{day}{epoch}{tet}.data = spindle{day}{epoch}{tet}.data(1:downsample:end, :);
        spindle{day}{epoch}{tet}.samprate = spindle{day}{epoch}{tet}.samprate / downsample;
        end
        ...
        for i = 1:length(lo)
        	if lo(i) > 1 && hi(i) < length(spindle{day}{epoch}{tet}.data)
            	spindle{day}{epoch}{tet}.data(lo(i):hi(i)) = minint;
            end
        end

        % save the resulting file
        if subtractreference
            spindlefile = sprintf('%sEEGnonreference/%sspindle%02d-%d-%02d.mat', ...
            	savedirectoryname, fileprefix, day, epoch, tet);
        else
            spindlefile = sprintf('%sEEG/%sspindle%02d-%d-%02d.mat', ...
            	savedirectoryname, fileprefix, day, epoch, tet);
        end
        save(spindlefile, 'spindle');
            ...
            if ~isempty(s)
                ...
                for c = 1:length(s)
                    ...
                    if ~isempty(data)
                        ...
                        spikes{day}{epoch}{tet}{c}.spindlephase = ...
