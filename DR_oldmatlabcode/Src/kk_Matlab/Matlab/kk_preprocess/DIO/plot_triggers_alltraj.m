...
% load your rewardinfo struct manually

day=5;
epochs=2;
pos = loaddatastruct('/data12/kkay/Cha/','cha','pos',day);
...
for e=epochs        % run over all epochs
    ...
    for r=1:size(data,1)
        figure
        % type 1 corresponds to input-to-output traj         (horizontal zig)
        % type 2 corresponds to output-to-next-input traj    (diagonal zag)
        for ii=1:2
            ...
            string1 = sprintf('begin (red) %d',starttime);
            string2 = sprintf('end (blue) %d',endtime);
            title(char(string1,string2,num2str(diff/10000)),'FontSize',17,'FontWeight','bold')
            % if no delay, then print in red
            if diff==0
                title(char(string1,string2,'0 -- should see red error trial'),'FontSize',17,'FontWeight','bold','Color','r')
            end
                
            % if type 1 traj, label reward vs. error (and which well)
            if ii==1
                if data(r,3)==1
                    set(gca,'Color',[1 1 0.3]);   % rewarded is yellow background
                    text(70,65,['well ' num2str(data(r,1))],'FontSize',18,'FontWeight','bold')
                elseif data(r,3)==0
                    set(gca,'Color',[1 .85 .85])  % error is red background
                    text(70,65,['well ' num2str(data(r,1))],'FontSize',18,'FontWeight','bold')
                end
            end
          
            % if type 2 traj, label inbound vs. outbound
            if ii==2
                if data(r+1,4)==10
