% use this script to visually check GAMMAL epochs extracted by
% getgammaltimes -- also use to empirically determine good values for
% powerratio
...
if 1
    ...
    gammal = loaddatastruct(animaldir, animalprefix,'gammal',epochs(1));
    gammah = loaddatastruct(animaldir, animalprefix,'gammah',epochs(1));
    ...
    for tet=1:length(tetlist)       
        ...
        if validtet(tet)
            ...
            % ripple + gammal + gammah times

            rippletrace{tet} = nan(size(eegtimes))';
            gammaltrace{tet} = nan(size(eegtimes))';
            gammahtrace{tet} = nan(size(eegtimes))';
            
            noripples = length(ripples{epochs(1)}{epochs(2)}{tetlist(tet)}.startind);
            nogammal = length(gammal{epochs(1)}{epochs(2)}{tetlist(tet)}.startind);
            nogammah = length(gammah{epochs(1)}{epochs(2)}{tetlist(tet)}.startind);
            
            disp(sprintf('tetrode %d : %d cells, %d ripples, %d gammal %d gammah',tetlist(tet),nocellsontet,noripples,nogammal,nogammah))
            ...
            for j=1:nogammal
                startind(j) = gammal{epochs(1)}{epochs(2)}{tetlist(tet)}.startind(j);
                endind(j) = gammal{epochs(1)}{epochs(2)}{tetlist(tet)}.endind(j);
                gammaltrace{tet}(startind(j):endind(j)) = 1;
            end
            
            for j=1:nogammah
                startind(j) = gammah{epochs(1)}{epochs(2)}{tetlist(tet)}.startind(j);
                endind(j) = gammah{epochs(1)}{epochs(2)}{tetlist(tet)}.endind(j);
                gammahtrace{tet}(startind(j):endind(j)) = 1;
            end
...
for w = 1:nowindows
    ...
    for r=1:5     % iterate through regions
        region=regions{r};
        for tet=[find(validtet==1)]
            if strcmp(tetlist_region{tet},region)
                ...
                %rippletrace   
                plot(timevec,rippletrace{tet}(eegstartindex:(eegstartindex+windowsize_eegsamp))-plotcount*1000+500,'k','LineWidth',5)
                %gammaltrace   
                plot(timevec,gammaltrace{tet}(eegstartindex:(eegstartindex+windowsize_eegsamp))-plotcount*1000+450,'r','LineWidth',5)
                %gammahtrace   
                plot(timevec,gammahtrace{tet}(eegstartindex:(eegstartindex+windowsize_eegsamp))-plotcount*1000+475,'b','LineWidth',5)
