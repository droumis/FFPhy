function toppeak = kk_dayprocess_2(daydirect, animdirect, fileprefix, daynum, varargin)

%%% modifications from the common dayprocess:

%%% (6.14.11) 1. ** made CMPERPIX a vector instead of a single number, to specify
%%% individual epochs  
%%%%  so need to specify all epochs, e.g.:  cmperpix = [1 0.5127 0.2437 1 1] if there are 5 epochs

%% 3.12.13
%% added code after shantanu's sj_dayprocess_cont2
    %% makes it easier to process individual components
    %% incorporates smoothed velocity within script
...
spikesonly = 0;  % If this is set to 1, eeg,multi,dio,vel is set to 0, and position is not saved.
...
for option = 1:2:length(varargin)-1
    ...
    switch varargin{option}
        ...
        case 'processmulti'
            processmulti = varargin{option+1};
        case 'processdio'
            processdio = varargin{option+1};
        case 'processspikes'
            processspikes = varargin{option+1};
        case 'processvel'
...
for epoch = 1:length(times)
    if ~isempty(times(epoch).starttime)
        ...
        if ~isempty(rawpos{daynum}{epoch}.data)
            ...
            pos{daynum}{epoch}.cmperpixel = cmperpix(epoch);
            pos{daynum}{epoch}.data(:,2:3) = pos{daynum}{epoch}.data(:,2:3)*cmperpix(epoch);
...
%%%%%% Smoothened position and velocity 0- Takes times
for epoch=1:length(times)
if (processvel)
    disp('Velocity process');
    sj_velocitydayprocess(animdirect,fileprefix, daynum,epoch,'cmperpix',cmperpix(epoch));
end
...
if processmulti==1
    ...
    for tet = 1:length(tetfolders)
        if ~isempty(tetfolderinfo{tet})
            ...
            if exist(fullfile(tetfolders{tet},sprintf('%s-%02d_params_pc.mat',dayname,tetnum)))
                rawdata = load(fullfile(tetfolders{tet},sprintf('%s-%02d_params_pc.mat',dayname,tetnum)));
                if isempty(rawdata)
                                        ...
                                        if ~isempty(pos{daynum}{epoch}.data)
                                            ...
                                            if size(pos{daynum}{epoch}.data,2)>5
                                                ...
                                                if strcmp('run',epochname(1:end-1))
                                                    display('Warning! Using only non-updated original Position Data for run epoch for spikes');
                                                end
                                                [spikepos, posindex] = lookuptime3(timepoints/10000, pos{daynum}{epoch}.data(:,1),pos{daynum}{epoch}.data(:,2:4)');
                                                spikepos_sm = [];
                                                flag_sm=0;
