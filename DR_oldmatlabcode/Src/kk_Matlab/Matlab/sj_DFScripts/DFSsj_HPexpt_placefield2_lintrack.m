...
savefile = [savedir 'HPb_PlaceFieldsTrajMaps_Day1Lin']; %Day2
...
if runscript == 1
    ...
    epochfilter = 'isequal($type, ''runlin'')';
            ...
            for neuron = 1:length(cells)
                ...
                for ep = 1:length(allepochs)
                    ...
                    if figopt1==1,
                        ...
                        %redimscreen;
                        set(gcf,'Position',[20 290 920 770]);
                        hold on;
...
%            all2dcorr_day{an}{day}(nc)=corr2d;
        ...
        if savelinfields==1
            savefile = sprintf('%s/%slinfields%02d_Lin.mat', animdirect, prefix, day);
            save(savefile,'linfields');
            savefile = sprintf('%s/%smapfields%02d_Lin.mat', animdirect, prefix, day);
