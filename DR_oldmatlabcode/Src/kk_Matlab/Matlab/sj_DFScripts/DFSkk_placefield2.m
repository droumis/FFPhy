...
savefile = [savedir 'PlaceFieldsTrajMaps_placefield2_Egypt'];

minabsvel = 3;  % cm/sec - Most conservative for runs and place fields
minlinvel = 5;

plotdays = 3:8; % If you only load data when runscript=0 and savedata=0, then this field will supplant days
...
if runscript == 1
    ...
    ca1cellfilter = '(isequal($area, ''CA1'') && ($numspikes > 100) && ($meanrate < 100))';
    ca2cellfilter = '(isequal($area, ''CA2'') && ($numspikes > 100) && ($meanrate < 100))';
    ca3cellfilter = '(isequal($area, ''CA3'') && ($numspikes > 100) && ($meanrate < 100))';
    ...
    tetlist = [2,13,15,16];    % Egypt "pure" CA1 tetrode
    ...
    timefilter = { {'DFTFsj_getlinstate', '(($state ~= -1) & (abs($linearvel) >= 5))', 6},
                    {'DFTFsj_getriptimes','($nripples == 0)','tetrodes',tetlist,'minthresh',2} };
    ...
    ca3lf = setfilterfunction(ca3f, 'DFAsj_filtercalclinfields_tf', {'spikes', 'linpos'}, 'binsize', 2);
    ca3mf = setfilterfunction(ca3f, 'DFAsj_twodoccupancy', {'spikes', 'linpos','pos'}, 'binsize', 1, 'std', 3);

    
    % Run analysis
    % ------------
    ca1lf = runfilter(ca1lf);  % Place Field Stability     (filtercalclinfields)
    ca1mf = runfilter(ca1mf);  % Place Field Map           (twodoccupancy)
    ca2lf = runfilter(ca2lf);  % Place Field Stability     (filtercalclinfields)
    ca2mf = runfilter(ca2mf);  % Place Field Map           (twodoccupancy)
    ca3lf = runfilter(ca3lf);  % Place Field Stability     (filtercalclinfields)
    ca3mf = runfilter(ca3mf);  % Place Field Map           (twodoccupancy)
...
if strcmp(area,'CA1')
    calf=ca1lf;
    camf=ca1mf;
elseif strcmp(area,'CA2')
    calf=ca2lf;
    camf=ca2mf;
else strcmp(area,'CA3')
    calf=ca3lf;
    camf=ca3mf;
end
...
for anidx = 1:length(useanim)
    an = useanim(anidx);
    for i=1:length(calf(an).output{1}),
        
        index{an}(i,:)=calf(an).output{1}(i).index;
        alltrajdata{an}{i}=calf(an).output{1}(i).trajdata;
        allmapdata{an}{i}=camf(an).output{1}(i);
...
for anidx = 1:length(useanim)
    ...
    for j=1:size(index{an},1)  %iterate over day-epoch-tetrode-cell
        
%         % Determine whether cell is in multiple epochs for a given day
%         tetrode=index{an}(j,3);
%         neuron=index{an}(j,4);
%         index{an}(j,3:4)
%         for b=1:             % "block" of epochs for a cell
%         
        
        day=index{an}(j,1);
        epoch=index{an}(j,2);
            ep=epoch;
        currtet=index{an}(j,3);
        currcell=index{an}(j,4);
        
        disp(sprintf('%d %d %d %d',day,epoch,currtet,currcell))
        
        trajdata = alltrajdata{an}{j};        %% added {an} after alltrajdata -- not sure why it was otherwise
        mapdata = allmapdata{an}{j}.smoothedspikerate;
        maxrate=[];
        
        % Get trajectory with maximum peak firing rate
        for i=1:length(trajdata),
            maxrate(i) = max(trajdata{i}(5:end-5,5));
        end
        if max(maxrate)<1
            disp(sprintf('skip, maxrate %d',max(maxrate)));
            continue
        end
        ...
        if figopt1==1,
                        ...
                        % Linearized traj
                        if 0
                        figure(str2num([num2str(day) num2str(ep) num2str(currtet) num2str(currcell)]));
                       ...
                       %subplot(1,length(allepochs),ep); hold on;
                        ...
                        % Maps -
                        % 1) Each traj separately - 1 fig for each epoch
                        if 1
                                            ...
                                            for tr = 1:length(mapdata)
                                                subplot(2,2,tr); hold on;
                                                hold on
                                                imagesc(flipud(mapdata{tr}),[0 max(maxrate)]); colorbar
                                                axis tight;
...
%                     end
                    ...
                    mapplace{an}{day}{ep}{currtet}{currcell}=allmapdata{an}{j};
