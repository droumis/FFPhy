function out = sj_plotgammamod(sind, tind, excludetimes, spikes, gamma, varargin)
% out = plotgammamod(spike_index, gamma_index, excludeperiods, spikes, gamma, options)
% plots the gamma moduation histogram for the specified cell based on the
% specified index into the gamma structure.
...
t = geteegtimes(gamma{tind(1)}{tind(2)}{tind(3)});

tph = gamma{tind(1)}{tind(2)}{tind(3)}.data(:,1);
sph = tph(lookup(s, t));
...
title(sprintf('cell %d %d %d %d, eegtet %d gamma, mod %f, peakph %g, pval %f', sind, tind(3), m, phdeg, stats.p));
...
title(sprintf('cell %d %d %d %d, eegtet %d gamma, Kappa %f, prefang %g, pval %f', sind, tind(3), kappa, thetahat_deg, prayl));
