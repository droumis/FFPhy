function traj = parsetrajectories_Wtrack(smoothedpos,min_trip_duration,min_excursion_distance)
%
%   traj = parsetrajectories_Wtrack(smoothedpos,min_trip_duration,min_excursion_distance)
%
%       .descript
%       .subject
%       .day
...
traj = smoothedpos;
traj.descript = 'trajectories on W track';
...
for j = 2:size(smoothedpos.data,1)
...
% determine whether the trajectory passes through the choicepoint zone
load('choicepoint')
traj.choicepointzone = choicepoint;
for k = 1:length(traj.smoothedpos_snippets)
    snippet = traj.smoothedpos_snippets(k);
    traj.choicepoint(k) = any(inpolygon( ...
        snippet.data(:,2),snippet.data(:,3), ...
        choicepoint.vertices(:,1),choicepoint.vertices(:,2) ));
end
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% now score the trajectories as correct or error
% keep track of the last-visited side arm
if (traj.startzone(1) == 1) | (traj.startzone(1) == 3)
    last_side_arm = traj.startzone(1);
else
    last_side_arm = NaN;
end
for i = 1:numel(traj.startzone)
    if ~isnan(traj.startzone(i)) & ~isnan(traj.endzone(i))
        ...
        if traj.endzone(i) == traj.startzone(i)
            % perseverative re-visit; note that we do *not* update last_side_arm
            % in this case
            traj.correct(i) = 0;
        else
            if (traj.startzone(i) == 1) | (traj.startzone(i) == 3)
                % test against the return-to-center rule
                if traj.endzone(i) == 2
                    traj.correct(i) = 1;
                    last_side_arm = traj.startzone(i);
                else % the rat went from side arm to side arm
                    traj.correct(i) = 0;
                    last_side_arm = traj.endzone(i);
                end
            elseif traj.startzone(i) == 2
                if isnan(last_side_arm)
                    traj.correct(i) = NaN;
                elseif traj.endzone(i) == last_side_arm
                    traj.correct(i) = 0;
                else
                    traj.correct(i) = 1;
                end
