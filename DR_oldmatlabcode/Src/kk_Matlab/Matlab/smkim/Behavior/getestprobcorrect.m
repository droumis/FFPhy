% [probcorrect] = getestprobcorrect(behavperform, backgroundprob, initialcond, conf)
...
function [pc, lt] = getestprobcorrect(bp, background_prob, startflag, conf)
...
[blower, bupper, bmid, bmode, pmatrix] = pdistn(qnew, signewsq, muone, background_prob, conf);

%-------------------------------------------------------------------------------------
%find the last point where the confidence interval crosses chance
%for the backward filter (lt)

lt = find(blower < background_prob);
...
title(['IO(' num2str(conf) ') Learning Trial = ' num2str(lt) ' RW variance = ' num2str(sige^2) ]);
xlabel('Trial Number')
