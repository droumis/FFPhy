function out = dfakk_calcxcorrmeasures(ind,eind,excludetimes, spikes, linpos, thetagnd, varargin)

% use with kk_paircelleeganal iterator
...
for option = 1:2:length(varargin)-1   
    if isstr(varargin{option})       
        switch(varargin{option})
            ...
            case 'thetaphase'       % collect theta phase (same tetrode) for later checking sig theta modulation kk 6.3.13
                thetaphase = varargin{option+1}; 
            case 'bin'
            ...
            case 'calclinfields'
                calclinfields = varargin{option+1};
            case 'calctrajxcorr'
                calctrajxcorr = varargin{option+1};
            otherwise
...
% if thetaphase specified, retrieve theta phase of spikes and output
if thetaphase
    
    % tetrode of cell 1
    try
    thetaph1 = thetagnd{eind(1)}{eind(2)}{eind(3)}.data(:,2);
    thetatime1 = geteegtimes(thetagnd{eind(1)}{eind(2)}{eind(3)});
    out.sph1 = thetaph1(lookup(t1inc, thetatime1));   
    catch
        out.sph1=[];
    end
    ...
    %tetrode of cell 2
    try
    thetaph2 = thetagnd{eind(1)}{eind(2)}{eind(4)}.data(:,2);
    thetatime2 = geteegtimes(thetagnd{eind(1)}{eind(2)}{eind(4)});
    sph2 = thetaph2(lookup(t2inc, thetatime2));
    out.sph2 = thetaph2(lookup(t2inc, thetatime2));
    catch
        out.sph2=[];
    end
end
