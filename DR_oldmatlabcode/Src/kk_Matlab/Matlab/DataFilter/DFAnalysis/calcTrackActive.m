function out = calcTrackActive(index, excludeperiods, spikes, linpos, varargin)
...
for option = 1:2:length(varargin)-1   
    if isstr(varargin{option})       
        switch(varargin{option})
            ...
            case 'activeRateDef'
                activeRateDef = varargin{option+1};    
            otherwise
...
if (meanrate >= thresh)
    ...
    numofbins = length(find(rates >= activeRateDef));
    out = numofbins/length(rates);
else
    out = nan;
end