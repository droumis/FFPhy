...
% pre parameters
animal_list = {'Chapati'}
minthresh = 1;     % how big the spindle events are, in SD (usually 2)
nbins = 12;
eventreg = 'ctx';
phasereg = 'ctx';
reftet = [];            % manually set ctx reference tetrode -- or leave blank to let analysis function to choose one w/ highest variance

timefilt_flag = 'norip';  % first digit is ripple exclusion (1) or not (0)
...
ca1f = setfilterfunction(ca1f, 'dfakk_getspindletrigspiking', {'spindle','spindles','spikes','tetinfo'},'minthresh',minthresh,'spindletetfilter',['isequal($area, ''' phasereg ''')'],'reftet',reftet);
ca2f = setfilterfunction(ca2f, 'dfakk_getspindletrigspiking', {'spindle','spindles','spikes','tetinfo'},'minthresh',minthresh,'spindletetfilter',['isequal($area, ''' phasereg ''')'],'reftet',reftet);
ca3f = setfilterfunction(ca3f, 'dfakk_getspindletrigspiking', {'spindle','spindles','spikes','tetinfo'},'minthresh',minthresh,'spindletetfilter',['isequal($area, ''' phasereg ''')'],'reftet',reftet);
