...
runscript = 1
region1 = 'CA1'    % reference region -- match it with first filter in cellpairfilter
region2 = 'CA3'
...
%epochfilter{1} = '(isequal($type, ''run''))'; % || isequal($type,''run''))';


cellpairfilter = {'allcomb', '(isequal($area, ''CA1'') && ($numspikes > 100) && isequal($type, ''principal''))', ...
                             '(isequal($area, ''CA3'') && ($numspikes > 100) && isequal($type, ''principal''))'};

%cellpairfilter = {'allcomb', '(isequal($area, ''CA2'') && ($numspikes > 100) && ($meanrate < 7))', ...
%                             '(isequal($area, ''CA3'') && ($numspikes > 100) && ($meanrate < 7))'};
...
if 1
% consolidate across epochs to obtain .pairoutput  
dummyindex=[];
...
if 1
    ...
    % title the graph
    titlestr = {[animals{1} ' ' region1 ' ' region2 ' z-scored cross-correlograms (Mizuseki specs)']};
    for k=1:length(timefilter)
        titlestr = [titlestr,timefilter{k}{2}];
    end
