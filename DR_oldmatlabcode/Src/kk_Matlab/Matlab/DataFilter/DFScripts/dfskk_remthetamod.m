...
animals = {'Egypt'};
tdthresh = .6;
statictet = 11;   % Choose reference tetrode! -- which isn't extracted into thetagnd, but is "theta"
     % theta eeg reference for all specified days
...
timefilter_ca1 = {{'kk_getriptimes', '($nripples == 0)', [], 'cellfilter', '(isequal($area, ''CA1''))'} ...
                  {'kk_getremtimes', '($nrem >= 2)', [], 'cellfilter', '(isequal($area, ''CA1''))','tdthresh',tdthresh}};
...
ca1f = setfilterfunction(ca1f, 'dfakk_thetaphase', {'spikes', 'theta'},'nbins',24);
ca2f = setfilterfunction(ca2f, 'dfakk_thetaphase', {'spikes', 'theta'},'nbins',24);
ca3f = setfilterfunction(ca3f, 'dfakk_thetaphase', {'spikes', 'theta'},'nbins',24);
%ca2f = setfilterfunction(ca2f, 'dfakk_thetaphase', {'spikes', 'theta'},'nbins',24);
...
dummy=[]; sigcount=0;
for k=1:length(caf.celloutput)
    if (caf.celloutput(k).rayleighp < 0.05) && (length(caf.celloutput(k).spikephases) >= min_spikes)     % check if significant
