function extractfastfastgamma(directoryname, fileprefix, day, tetrode, min_suprathresh_duration, nstd, varargin)

%% simple modification of extractslowripples.m, KK 6.19.12


%function extractfastfastgamma(directoryname, fileprefix, day, tetrode_list, %%min_suprathresh_duration, nstd, maxpeakval, options)
%
%	Reads in the slow ripple files from the specified day and tetrodes and
...
for j = 1:length(tet)
    t = tet(j);
    tmpflist = dir(sprintf('*fastfastgamma%02d-*-%02d.mat', day, t));
    ...
    for i = 1:length(tmpflist)
        ...
        renv = double(fastfastgamma{d}{e}{t}.data(:,3));
        
        % smooth the envelope:
        samprate = fastfastgamma{d}{e}{t}.samprate;
        ...
        if (thresh > 0) & any(find(renv<baseline))
            ...
            rip.starttime = fastfastgamma{d}{e}{t}.starttime + rip.startind / samprate;
            rip.endtime = fastfastgamma{d}{e}{t}.starttime + rip.endind / samprate;
            rip.midtime = fastfastgamma{d}{e}{t}.starttime + rip.midind / samprate;
            rip.peak = tmprip(:,3);
            rip.energy = tmprip(:,7);
        ...
        rip.timerange = [0 length(renv)/samprate] + fastfastgamma{d}{e}{t}.starttime;
        rip.samprate = fastfastgamma{d}{e}{t}.samprate;
...
save(sprintf('%s/%sgammaff%02d.mat', directoryname, fileprefix, d), 'gammaff');
popd
