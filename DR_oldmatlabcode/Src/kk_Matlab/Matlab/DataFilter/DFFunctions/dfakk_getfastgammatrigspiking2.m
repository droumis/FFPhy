function [out] = dfakk_getfastgammatrigspiking2(index, excludeperiods, fastgamma, gammaf, spikes, tetinfo,varargin)
% [out] = dfakk_getgammatrigspiking(index, excludeperiods, fastgamma, gammaf, spikes, tetinfo, varargin)
...
%                   of the gammaf for each row in out.out
...
%% First find de novo here valid fastgamma periods for each gammatetfilter tetrode.
    % Then take intersection of valid fastgamma periods with includetimes.
...
if isempty(tetlist)
    ...
    out.spikephases = [];
    out.numspikes_total = [];
    out.numspikes_gammaf = [];
    return
end


g = gammaf{day}{epoch}{tetlist(1)};
...
for tet = tetlist
    if ~strcmp(ripple_flag,'rip')
        eventperiods = [gammaf{day}{epoch}{tet}.starttime gammaf{day}{epoch}{tet}.endtime];
...
for i = 1:size(index,1)
        ...
        % if some other region (say CA3), identify CA1 tetrode that is most coherent to get events    
            ...
            % to the cell's tetrode
                ...
                gamma1 = fastgamma{index(i,1)}{index(i,2)}{index(i,3)};
                coherences = [];
                for kk = 1:length(tetlist)
                    gamma2 = fastgamma{index(i,1)}{index(i,2)}{tetlist(kk)};
    ...
    if ~isempty(spikes{index(i,1)}{index(i,2)}{index(i,3)}{index(i,4)})
        ...
        filteegtimes=geteegtimes(fastgamma{index(i,1)}{index(i,2)}{reftet});
        includedspike_indices = lookup(includedspike_times,filteegtimes);
        reftetphases = fastgamma{index(i,1)}{index(i,2)}{reftet}.data(:,2);
    
        spikephases{i} = reftetphases(includedspike_indices);                            % list of phase times of spikes
        numspikes_gammaf(i,:) = length(includedspike_indices);             % # spikes firing during gammaf period
...
if exist('spikephases')
    ...
    out.numspikes_gammaf = numspikes_gammaf;
else
    out.indices_coherence = [];
    out.fields_indices_coherence = '[<cellindex> , <reftet num> , <coherence b/t cell tetrode and reftet> ]';
    out.spikephases = [];
