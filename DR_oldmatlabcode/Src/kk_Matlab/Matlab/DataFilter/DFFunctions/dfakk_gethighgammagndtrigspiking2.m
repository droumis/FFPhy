function [out] = dfakk_gethighgammagndtrigspiking2(index, excludeperiods, highgammagnd, gammah, spikes, tetinfo,varargin)
% [out] = dfakk_getgammatrigspiking(index, excludeperiods, highgamma, gammah, spikes, tetinfo, varargin)
...
%                   of the gammah for each row in out.out
...
%% First find de novo here valid highgamma periods for each gammatetfilter tetrode.
    % Then take intersection of valid highgamma periods with includetimes.
...
if isempty(tetlist)
    ...
    out.spikephases = [];
    out.numspikes_total = [];
    out.numspikes_gammah = [];
    return
end


g = gammah{day}{epoch}{tetlist(1)};
...
for tet = tetlist
    if ~strcmp(ripple_flag,'rip')
        eventperiods = [gammah{day}{epoch}{tet}.starttime gammah{day}{epoch}{tet}.endtime];
...
for i = 1:size(index,1)
        ...
        % if some other region (say CA3), identify CA1 tetrode that is most coherent to get events    
            ...
            % to the cell's tetrode
                ...
                gamma1 = highgammagnd{index(i,1)}{index(i,2)}{index(i,3)};
                coherences = [];
                for kk = 1:length(tetlist)
                    gamma2 = highgammagnd{index(i,1)}{index(i,2)}{tetlist(kk)};
    ...
    if ~isempty(spikes{index(i,1)}{index(i,2)}{index(i,3)}{index(i,4)})
        ...
        filteegtimes=geteegtimes(highgammagnd{index(i,1)}{index(i,2)}{reftet});
        includedspike_indices = lookup(includedspike_times,filteegtimes);
        reftetphases = highgammagnd{index(i,1)}{index(i,2)}{reftet}.data(:,2);
    
        spikephases{i} = reftetphases(includedspike_indices);                            % list of phase times of spikes
        numspikes_gammah(i,:) = length(includedspike_indices);             % # spikes firing during gammah period
...
if exist('spikephases')
    ...
    out.numspikes_gammah = numspikes_gammah;
else
    out.indices_coherence = [];
    out.fields_indices_coherence = '[<cellindex> , <reftet num> , <coherence b/t cell tetrode and reftet> ]';
    out.spikephases = [];
