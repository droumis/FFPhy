function kk_highgammadayprocess(directoryname,fileprefix,days, varargin)
...
daytetlist = [];
f = '';
defaultfilter = (['/home/kkay/matlab/Filters/bashighgammafilter.mat']);
assignphase = 0;
tetfilter = '';
reftognd = 0;
...
%         keyboard
    ...
    for fnum = 1:length(flist)
        ...
        temp = filtfilt(highgammafilter,1,entry.data');
        hdata = hilbert(temp);
        env = abs(hdata);
        phase = angle(hdata);
        ...
        if reftognd 
        highgammagnd{day}{epoch}{tet}.samprate = entry.samprate;
        highgammagnd{day}{epoch}{tet}.starttime = entry.starttime;

        highgammagnd{day}{epoch}{tet}.data(:,1) = int16(temp);
        highgammagnd{day}{epoch}{tet}.data(:,2) = int16(phase*10000);
        highgammagnd{day}{epoch}{tet}.data(:,3) = int16(env);
        highgammagnd{day}{epoch}{tet}.fields = ...
            'filtered_amplitude instantaneous_phase*10000 envelope_magnitude';
        else
        highgamma{day}{epoch}{tet}.samprate = entry.samprate;
        highgamma{day}{epoch}{tet}.starttime = entry.starttime;
        ...
        for i = 1:length(lo)
        	if reftognd && lo(i) > 1 && hi(i) < length(highgammagnd{day}{epoch}{tet}.data)
              	highgammagnd{day}{epoch}{tet}.data(lo(i):hi(i)) = minint;
            elseif lo(i) > 1 && hi(i) < length(highgamma{day}{epoch}{tet}.data)
        ...
        if reftognd
            highgammafile = sprintf('%sEEG/%shighgammagnd%02d-%d-%02d.mat', ...
            	savedirectoryname, fileprefix, day, epoch, tet);
            save(highgammafile, 'highgammagnd');
            clear highgammagnd
        else
