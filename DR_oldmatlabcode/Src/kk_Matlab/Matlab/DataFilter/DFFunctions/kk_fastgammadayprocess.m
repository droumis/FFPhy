function kk_fastgammadayprocess(directoryname,fileprefix,days, varargin)

%% simple modification of lowgammadayprocess

%fastgammaDAYPROCESS(directoryname,fileprefix,days, options)
%
...
%			Default 0
%       'nonreference', 0 or 1
%           specifies whether to use EEG or EEGnonreferencefastgammasextract.m
%           Default 0

daytetlist = [];
f = '';
defaultfilter = (['/home/kkay/matlab/Filters/basfastgammafilter.mat']);
assignphase = 0;
tetfilter = '';
subtractreference = 0;
...
%         keyboard
    ...
    for fnum = 1:length(flist)
        ...
        fastgamma{day}{epoch}{tet}.samprate = eeg{day}{epoch}{tet}.samprate;
        fastgamma{day}{epoch}{tet}.starttime = eeg{day}{epoch}{tet}.starttime;

        fastgamma{day}{epoch}{tet}.data(:,1) = int16(temp);
        fastgamma{day}{epoch}{tet}.data(:,2) = int16(phase*10000);
        fastgamma{day}{epoch}{tet}.data(:,3) = int16(env);
        fastgamma{day}{epoch}{tet}.fields = ...
        ...
        for i = 1:length(lo)
        	if lo(i) > 1 && hi(i) < length(fastgamma{day}{epoch}{tet}.data)
            	fastgamma{day}{epoch}{tet}.data(lo(i):hi(i)) = minint;
            end
        end

        % save the resulting file
        if subtractreference
            fastgammafile = sprintf('%sEEGnonreference/%sfastgamma%02d-%d-%02d.mat', ...
            	savedirectoryname, fileprefix, day, epoch, tet);
        else
            fastgammafile = sprintf('%sEEG/%sfastgamma%02d-%d-%02d.mat', ...
            	savedirectoryname, fileprefix, day, epoch, tet);
        end
        save(fastgammafile, 'fastgamma');
            ...
            if ~isempty(s)
                ...
                for c = 1:length(s)
                    ...
                    if ~isempty(data)
                        ...
                        spikes{day}{epoch}{tet}{c}.fastgammaphase = ...
