function extracthighgamma_derive(directoryname, fileprefix, day, tetrode, min_suprathresh_duration, nstd, varargin)
...
for t = tet
    ...
    if samethreshperday == 1
        ...
        for i = 1:length(tmpflist)
            ...
            temprenv = double(highgamma{d}{e}{t}.data(:,3));
    ...
    for i = 1:length(tmpflist)
        ...
        % convert the ripple envelope field to double
        renv = double(highgamma{d}{e}{t}.data(:,3));
        
        % smooth the envelope:
        samprate = highgamma{d}{e}{t}.samprate;
        ...
        if (thresh > 0) & any(find(renv<baseline))
            tmprip = extractevents2(renv, thresh, baseline, 0, mindur, 0)';
            % Assign the fields
            ...
            rip.starttime = highgamma{d}{e}{t}.starttime + rip.startind / samprate;
            rip.endtime = highgamma{d}{e}{t}.starttime + rip.endind / samprate;
            rip.midtime = highgamma{d}{e}{t}.starttime + rip.midind / samprate;
            rip.peak = tmprip(:,3);
            rip.energy = tmprip(:,7);
        ...
        rip.timerange = [0 length(renv)/samprate] + highgamma{d}{e}{t}.starttime;
        rip.samprate = highgamma{d}{e}{t}.samprate;
...
save(sprintf('%s/%sgammah%02d.mat', directoryname, fileprefix, d), 'gammah');
end
