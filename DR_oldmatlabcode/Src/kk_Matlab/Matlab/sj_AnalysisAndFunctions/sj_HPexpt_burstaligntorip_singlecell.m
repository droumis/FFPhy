function [basefr0] = sj_HPexpt_burstaligntorip_singlecell(prefix, day, epoch, riptet, Hptets, PFCtet, PFCcell, binsize, saveg1, dospeed)
...
% -------------------------------------------------
% Smooth Matrix in Y-direction (along ripples)
% -------------------------------------------------

for i=1:size(rip_spkshist_cellsort_Hp,1)
    ...
    rip_spkshist_cellsort_Hp(i,:) = mean(rip_spkshist_cellsort_Hp(winst:winend,:));
    rip_spkshist_cellsort_PFC(i,:) = mean(rip_spkshist_cellsort_PFC(winst:winend,:));
end
% TRIM ENDS
rip_spkshist_cellsort_Hp = rip_spkshist_cellsort_Hp(:,50/binsize:(pret+postt-50)/binsize);
rip_spkshist_cellsort_PFC = rip_spkshist_cellsort_PFC(:,50/binsize:(pret+postt-50)/binsize);


% For d2 ep4 tet 17 - remove first 300

%rip_spkshist_cellsort_Hp(1:500,:)=[]; 
%rip_spkshist_cellsort_PFC(1:500,:)=[]; 
...
% -------------------------------------------------
    ...
    xaxis = -pret:binsize:postt; yaxis = 1:size(rip_spkshist_cellsort_Hp,1);
    imagesc(xaxis,yaxis,flipud(rip_spkshist_cellsort_Hp));
    ...
    set(gca,'YLim',[0 size(rip_spkshist_cellsort_Hp,1)]);
    %xlabel('Time(ms)','FontSize',20,'Fontweight','normal');
    
    ypts = 0:1:size(rip_spkshist_cellsort_Hp,1);
...
%         'FontSize',14,'Fontweight','normal');
    ...
    title(['PFC CellComb Firing Z aligned to ripples-' num2str(binsize) 'ms bins'],...
        'FontSize',20,'Fontweight','normal');
    subplot(3,1,[1 2]); hold on;
    xaxis = -pret:binsize:postt; yaxis = 1:size(rip_spkshist_cellsort_PFC,1);
    imagesc(xaxis,yaxis,flipud(rip_spkshist_cellsort_PFC));
