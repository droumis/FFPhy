function out = sj_plottrajdatafind_HPa(index)
% From sj_plottrajdatafind3 and sj_plottrajdatafind4egg, which are the same things

% sj_plottrajdatafind_HPa([2 4]);
% 1 EEG Spikes from ordered cells - ordered and speed 

% HPa - Day2, Try Epochs 4 and 2 both


%Shantanu - CHange version 1. These are notes for sj_plottrajdatafind3
...
% subplot dimensions - Subplots were used in Version 2
...
animdirect = '/data25/sjadhav/HPExpt/HPa_direct/';
prefix='HPa';
...
%day epoch tet cell - HPa
cellsi(1,:) = [day epoch 1 3]; % 1) 
cellsi(2,:) = [day epoch 1 1]; % 2) % 3,4,5,6 might be similar
...
% %day epoch tet cell - HPa. SELECT
% cellsi(1,:) = [day epoch 1 3]; % 1) 
% cellsi(2,:) = [day epoch 1 1]; % 2) 
% cellsi(3,:) = [day epoch 4 6]; % 3) 
% %cellsi(3,:) = [day epoch 4 3]; % 4) 
% cellsi(4,:) = [day epoch 4 4]; % 4')
% cellsi(5,:) = [day epoch 1 6]; % 5) % 10, 11 and 12 might be all close in terms of place field. THey have multiple locations.
% %cellsi(5,:) = [day epoch 1 4];
% %cellsi(5,:) = [day epoch 7 1]; 
% %cellsi(6,:) = [day epoch 4 1]; % 6)
% cellsi(6,:) = [day epoch 1 2];% 7)
% usecells = [1:6];



% % THESE WERE FOR REF Day 7 Epoch 4
% %day epoch tet cell
...
figno = 3;
 ...
 clr = {'m','c','g','r','b','k','r','g','c','m','m','c','g','r','b','k','r','g','c','m'};
...
for n=1:ntraj-1
    ...
    if currtraj == 3 || currtraj == 4
        ...
        trajsttime = statematrix.time(currstart)-40; % Add time to both
        trajendtime = statematrix.time(currend)+40;
        ...
        if oritimeintraj > 0.2 % more than 2 secs
