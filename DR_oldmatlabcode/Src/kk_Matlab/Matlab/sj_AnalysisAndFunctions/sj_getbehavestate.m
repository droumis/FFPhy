function [state, lindist] = sj_getbehavestate(linpos, day, epoch, includeStates, varargin)
%[state, lindist] = sj_getbehavestate(linpos, day, epoch, includeStates, options)
%
...
%    have to be in the same direction.
%4:  Include times that occur when the animal was on an ambiguous segment (like the
...
%           moving for the behavior to be counted as valid (default 5cm/sec)
%
...
for option = 1:2:length(varargin)-1
    switch varargin{option}
        ...
        case 'prefix'
            prefix = varargin{option+1};
        case 'animdirect'
            animdirect = varargin{option+1};
        otherwise
...
includeStates = 1:includeStates; % Makes it 1:6
...
%%% Cannot save since no control over epochs
% % Save behavestate file if prefix not empty
% if ~isempty(prefix)
%     if ~isempty(animdirect)
%         statefile = sprintf('%s/%sbehavestate%02d.mat', animdirect, prefix, day);
%     else
%         statefile = sprintf('%sbehavestate%02d.mat', prefix, day);
%     end
