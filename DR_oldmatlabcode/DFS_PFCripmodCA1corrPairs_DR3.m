        ...
        if max(alltraj) > peakthresh; %use cell if 5Hz peak threshold. if not, this whole pfc-ca1 set will be skipped.
            ...
            if ~isempty(pairdata(1,i).CA1posidxs);
                for posi = 1:length(pairdata(1,i).CA1posidxs(:,1));
                    ...
                    CA1matchtraj = find(CA1ShortList(posi) == fldsShortList(:),4); %find last corresponding index from psfdata. .4 bc gideon max runs ep =4
                    ...
                    if max(alltraj) > peakthresh; %use cell if 5Hz peak threshold
                        ...
                        for ru = 1:length(CA1matchtrajEP(:,1));
                            if isempty(trajplot{ru,length(trajplot(ru,:))}); %had to do this bc uneven concatenation of a matrix creates a blank in the rest of the rows
                                eachep = length(trajplot(ru,:)); %this is for all the rest of the epochs
                            else
                                eachep = length(trajplot(ru,:))+1; %this is for the first epoch
                            end
                            trajplot{ru, eachep} = fldsdata.output{CA1matchtrajEP(ru,1)}.trajdata; %store CA1 traj data next to pfc data
                            mapplot{ru,eachep} = mfldsdata.output{CA1matchtrajEP(ru,1)}.mapdata.smoothedspikerate; %store CA1 map data next to pfc data..replaced CA1matchtrajmap with CA1matchtrajEP
                        end
                    end
                ...
                for negi = 1:length(pairdata(1,i).CA1negidxs(:,1));
                    ...
                    if max(alltraj) > peakthresh; %use cell if 5Hz peak threshold
                        ...
                        for ruu = 1:length(CA1matchtrajEPb(:,1));
                            ...
                            trajplot{ruu, eachep} = fldsdata.output{CA1matchtrajEPb(ruu,1)}.trajdata; %store CA1 traj data next to pfc data
                            mapplot{ruu,eachep} = mfldsdata.output{CA1matchtrajEPb(ruu,1)}.mapdata.smoothedspikerate; %store CA1 map data next to pfc data.. replaced map index with traj index
                        end
                    end
            ...
            if length(trajplot(1,:)) >1; %if there are any ca1 cells
                for cu = 2:(length(trajplot(1,:))); %all ca1 cells ~ # of pairs w pfc
                    epochcounter = 0;
                    for pfi = 1:length(trajplot(:,1)); %each epoch of every pair
                        if length(trajplot{pfi,1}) ==4 && length(trajplot{pfi, cu}) ==4; %the pfc cell and ca1 cell must have all trajectories
                            epochcounter = epochcounter +1; %count each pair for storing
                            for tru = 1:4; %for each trajectory
                                clear r p minlen;
                                minlen = min(length(trajplot{pfi,1}{tru}(:,5)), length(trajplot{pfi,cu}{tru}(:,5)));
                                [r, p] = corrcoef(trajplot{pfi, 1}{tru}(1:minlen,5), trajplot{pfi, cu}{tru}(1:minlen,5), 'rows', 'pairwise'); %don't use rows if either has a nan
                                EpochCorrCoefstruct(epochcounter,tru) = r(2,1); %store corr coefs
                            end %each traj
                        end %if cells have all trajs
                    end %each epoch of each pair
                    %take average corrcoef of epochs for each pair
                    paircounter = paircounter +1; %count each pair for storing
                    CorrCoefstruct{paircounter,1}.r = mean(EpochCorrCoefstruct); %XXXXXXXXXthis will mean across epochs.. no don't do this... if there's only 1 epoch it will average the trajs
            ...
            if length(trajplot(1,:)) >1; %if there are any ca1 cells
                ...
                elseif pairdata(1,i).PFCidx(1,1) == 2%HPb
                    for mo = 1:length(HPbCA1struct); %loop over all the HP CA1 cells for the shuffled struct
                        %loop over CA1 cells that are no significantly correlated with the current PFC cell
                        if isempty(find(str2num(sprintf('%d', [2 HPbCA1struct{mo,1}.index([1 3 4])])) == CA1ShortList)); %if this ca1 cell is not on the pos ca1 list
                            if isempty(find(str2num(sprintf('%d', [2 HPbCA1struct{mo,1}.index([1 3 4])])) == CA1ShortListb)) ; %and if this ca1 cell is not on the neg ca1 list
                                mocnt = mocnt +1; %across all anims
                ...
                elseif pairdata(1,i).PFCidx(1,1) == 3%HPc
                    for mo = 1:length(HPcCA1struct); %loop over all the HP CA1 cells for the shuffled struct
                        %loop over CA1 cells that are no significantly correlated with the current PFC cell
                        if isempty(find(str2num(sprintf('%d', [3 HPcCA1struct{mo,1}.index([1 3 4])])) == CA1ShortList)); %if this ca1 cell is not on the pos ca1 list
                            if isempty(find(str2num(sprintf('%d',[3 HPcCA1struct{mo,1}.index([13 4])])) == CA1ShortListb)) ; %and if this ca1 cell is not on the neg ca1 list
                                mocnt = mocnt +1; %across all anims
                ...
                elseif pairdata(1,i).PFCidx(1,1) == 4%Nadal
                    for mo = 1:length(NdlCA1struct); %loop over all the HP CA1 cells for the shuffled struct
                        if isempty(find(str2num(sprintf('%d', [4 NdlCA1struct{mo,1}.index([1 3 4])])) == CA1ShortList));
                            if isempty(find(str2num(sprintf('%d', [4 NdlCA1struct{mo,1}.index([1 3 4])])) == CA1ShortListb)); %loop over CA1 cells that are no significantly correlated with the current PFC cell
                                mocnt = mocnt +1;
                                ndlcnt = ndlcnt +1;
                                for tru = 1:length(trajplot{1}); %for each trajectory
                                    clear r p minlen;
