    ...
    for i = 1:length(PFCShortList);
        ...
        if max(alltraj) > peakthresh; %use cell if 5Hz peak threshold. if not, this whole pfc-ca1 set will be skipped.
            ...
            for trajcell = 1:length(trajplot(1,:)); %for each cell in traj plot
                for epcell = 1:length(trajplot(~cellfun('isempty',trajplot(:,trajcell)),trajcell)); %takes all the epochs that exist for the ca1 cell within those of the pfc cell
...
if plotCorrCoef == 1;
    ...
    for fu = 1:length(CorrCoefstructPOS); %POS
        if CorrCoefstructPOS{fu,1}.pfcindex(1,1) == 1;
            ...
            posRHPa(length(posRHPa)+1, 1) = (CorrCoefstructPOS{fu,1}.r(:))'; %vert stack
...
%             posRHPb(hpb, 1) = mean(CorrCoefstructPOS{fu,1}.r(:));
            posRHPb(length(posRHPb)+1, 1) = (CorrCoefstructPOS{fu,1}.r(:))'; %vert stack
...
%             posRHPc(hpc, 1) = mean(CorrCoefstructPOS{fu,1}.r(:));
            posRHPc(length(posRHPc)+1, 1) = (CorrCoefstructPOS{fu,1}.r(:))'; %vert stack
...
%             posRNdl(ndl, 1) = mean(CorrCoefstructPOS{fu,1}.r(:));
            posRNdl(length(posRNdl)+1, 1) = (CorrCoefstructPOS{fu,1}.r(:))'; %vert stack
        end
    end
    ...
    for wu = 1:length(CorrCoefstructShuffHPa); %collect mean corrcoef for each pair into list
        allRShuffHPa(length(allRShuffHPa)+1,1) = (CorrCoefstructShuffHPa{wu,1}.r(:))'; %vert stack
    end
    
    wu = 0;
    for wu = 1:length(CorrCoefstructShuffHPb); %collect mean corrcoef for each pair into list
        allRShuffHPb(length(allRShuffHPb)+1,1) = (CorrCoefstructShuffHPb{wu,1}.r(:))'; %vert stack
    end
    
    wu = 0;
    for wu = 1:length(CorrCoefstructShuffHPc); %collect mean corrcoef for each pair into list
        allRShuffHPc(length(allRShuffHPc)+1,1) = (CorrCoefstructShuffHPc{wu,1}.r(:))'; %vert stack
    end
    
    wu = 0;
    for wu = 1:length(CorrCoefstructShuffNdl); %collect mean corrcoef for each pair into list
        allRShuffNdl(length(allRShuffNdl)+1,1) = (CorrCoefstructShuffNdl{wu,1}.r(:))'; %vert stack
    end
