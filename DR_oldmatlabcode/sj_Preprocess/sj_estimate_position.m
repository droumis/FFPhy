function position = sj_estimate_position(rawpos,varargin)
...
%   direction of this line segment is filtered to estimate head direction.
%   A
...
%   INTERP1_ANGLE (writtten by smk): which depends on recenter_angle
%
%Written by smk (2009 November 1)
%
% Shantanu - modifying to add descript for backward compatibility

centimeters_per_pixel = 0.45;
...
for i = 1:numel(rawpos)    % always 1 for me as I feed in rawpos{day}{epoch}
    %if ~isempty(rawpos{i})
    if ~isempty(rawpos)  % shantanu   
        
        % Add descript field: shantanu
        position{i}.descript = rawpos.descript;
        descript1 = sprintf('Loess estimated position data from rawpos');
        position{i}.descript{end+1,1} = descript1;
        
        % Determine whether a back marker was tracked
        if (sum(sum(~isnan(rawpos.data(:,4:5)))) == 0) || (sum(sum(rawpos.data(:,4:5))) == 0)
            back_exists = false;
        else
            back_exists = true;
        end
        % Convert timestamps to floating-point seconds
        t = double(rawpos.data(:,1));
        % Promote to double to take advantage of MATLAB acceleration
        xfront = double(rawpos.data(:,2));
        yfront = double(rawpos.data(:,3));
        xback = double(rawpos.data(:,4));
        yback = double(rawpos.data(:,5));
        ...
        % Inherit timestamps
        position{i}.data(:,1) = rawpos.data(:,1);
        if (back_exists)
