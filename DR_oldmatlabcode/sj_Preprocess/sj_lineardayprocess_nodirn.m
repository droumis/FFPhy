function sj_lineardayprocess_nodirn(directoryname,fileprefix,days, varargin)
%LINEARDAYPROCESS(directoryname,fileprefix,days, options)
%
% If only one diode was used, skips "head direction" and "gettraj" parts of
% linpos. So statematrix does not have "traj" and "lindist" fields
%
...
for day = days
   ...
   for i = 1:length(task{day})   
      if ((~isempty(task{day}{i})) && (strcmp(task{day}{i}.type,'run')) )
            ...
            index = [day i];
            [linpos{day}{i}.statematrix,linpos{day}{i}.segmenttable, linpos{day}{i}.trajwells, linpos{day}{i}.wellSegmentInfo, linpos{day}{i}.segmentInfo] = sj_linearizeposition1_nodirn(directoryname,fileprefix, index, varargin);
