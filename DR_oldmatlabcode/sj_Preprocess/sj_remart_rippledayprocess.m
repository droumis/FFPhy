function sj_remart_rippledayprocess(directoryname,fileprefix,days, varargin)
...
%%% Removing artifacts %%%
npoints = 45;  % REc-day5 45, day4,6,7 40, day3 30, day2 30
...
for day = days
    ...
    for fnum = 1:length(flist)
        ...
        %if epoch ==1
            ...
            DIOfilestr = dir(sprintf('%s/*DIO%02d*.mat', directoryname, day));
            DIOfname = DIOfilestr.name;
            load(DIOfname);
            stim = DIO{day}{epoch}{15};
            [etmp] = sj_removeartifacts1(eeg{day}{epoch}{tet}, stim, npoints);
            eeg{day}{epoch}{tet}.data = etmp;
            ...
            ripplefile = sprintf('%s/EEG/%sripplenostim%02d-%d-%02d.mat', ...
            ...
            neweegfile = sprintf('%s/EEG/%seegnostim%02d-%d-%02d.mat', ...
                directoryname, fileprefix, day, epoch, tet);
            save(neweegfile, 'eeg');
            clear eeg
        %end
