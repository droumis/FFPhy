...
if runscript==1
    savefile = [savedir 'HP_ripplemod_all_epoch_',num2str(m),'-',num2str(d),'-',num2str(y)];
else
    savefile = [savedir 'HP_ripplemod_all_epoch_2-12-2014'];
end
...
if runscript == 1
    ...
    modf = createfilter('animal',animals,'epochs',allepochfilter, 'cells',...
        cellfilter, 'iterator', iterator);
    ...
    % Version4: Change in Feb 2014 - Add Speed, and Nripple Condn
    
    %02-12-2014
    % ----------
     modf = setfilterfunction(modf,'DFAsj_getripalignspiking4',{'spikes', 'ripples', 'tetinfo', 'pos'},'dospeed',1,'lowsp_thrs',4,'minrip',2,'minstd',3); %

    %02-11-2014
    % ----------
    % modf = setfilterfunction(modf,'DFAsj_getripalignspiking4',{'spikes', 'ripples', 'tetinfo', 'pos'},'dospeed',1,'lowsp_thrs',5,'minrip',2'minstd',3); %
    % Default stdev is 3
...
% -----------------------------------------------------------------------------------------------------
if gatherdata
    gatherdatafile = [savedir 'HP_ripplemod_all_epoch_gather_',num2str(m),'-',num2str(d),'-',num2str(y)];
else
    gatherdatafile = [savedir 'HP_ripplemod_all_epoch_gather_2-12-2014']
end
...
if gatherdata
    ...
    if 1
        ...
        %currsavedir = '/data15/gideon/ripplemod/';
...
if 1
    ...
    set(gca,'XLim',[-pret postt]);
    title(sprintf('Norm histogram for sig and nonsig ripple mod %s units'));
    xlabel('Time(ms)','FontSize',xfont,'Fontweight','normal');
