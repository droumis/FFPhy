function out = DFAsj_getripalignspiking(index, excludetimes, spikes, ripples, tetinfo, pos, varargin)
...
%     end
    ...
    % Align to random jittered triggers in bck window: (trigtime-500ms)+random point in 1-50ms
    % ------------------------------------------------------------------------------------------------
    cntbck=cntbck+1;
...
rip_starttime=rip_starttime(2:end-1);
ripposindex = lookup(rip_starttime, pos{day}{epoch}.data(:,1)*1000);
ripPos = pos{day}{epoch}.data(ripposindex,2:3);
