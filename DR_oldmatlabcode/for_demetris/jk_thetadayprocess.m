function jk_thetadayprocess(directoryname,fileprefix,days, varargin)
...
for day = days
    ...
    if (isempty(daytetlist)) 
       ...
       for t = tet';
	   tmpflist = dir(sprintf('%s/EEG/*eeg%02d-*-%02d.mat',directoryname, day, t));
      	   nfiles = length(tmpflist); 
	   for i = 1:length(tmpflist)
	       flist{end+1} = sprintf('%s/EEG/%s', directoryname, tmpflist(i).name);
	   end
       end
    end
