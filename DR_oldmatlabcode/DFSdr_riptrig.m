...
savedir = '/data19/droumis/HPExpt/ProcessedDataDR/';
...
figdir = '/data19/droumis/HPExpt/FiguresDR';
gatherdata = 1; savegatherdata = 1;
figname = 'HPall_inhib';
gatherdatafile = [savedir 'HPa_awakePFC_riptrig'];
...
%combine epochs, run over mult animals
for k = 1:length(modf);
    for i = 1:length(modf(k).output{1});
%         if isempty(find(modf(k).output{1}(i).rip_spkshist_cell == nan))
            Allcellriphist(i,:) = nanmean(modf(k).output{1}(i).rip_spkshist_cell,1);
            [peakval(i) peakbin(i)] = max(Allcellriphist(i,:));
            preRip(i) = nanmean(Allcellriphist(i,30:50));
            
%         end
    end
end
Allcellriphist = zscore(Allcellriphist);
% [temp order] = sort(peakbin); %sort by peak
[temp order] = sort(preRip); %sort by pre-rip window
sortAllcellriphist = Allcellriphist(order,:);
% for i = 1:length(Allcellriphist);
 imagesc(sortAllcellriphist(i));
%  hold
% end
 line([50 50], [0 length(sortAllcellriphist)], 'Color', [0 0 0]);
return
% 
% 
% % ----------------------------------
...
%combine epochs, filter for ca1 and pfc cells and separate here
%run glm to see if ca1 cells predict pfcc.. 
for k = 1:length(modf);
    for i = 1:length(modf(k).output{1});
%         if isempty(find(modf(k).output{1}(i).rip_spkshist_cell == nan))
            Allcellriphist(i,:) = zscore(smooth(nanmean(modf(k).output{1}(i).rip_spkshist_cell,1),10));
            [peakval(i) peakbin(i)] = max(Allcellriphist(i,:));
            preRip(i) = nanmean(Allcellriphist(i,30:50));
%         end
    end
...
sortAllcellriphist = Allcellriphist(order,:);
% for i = 1:length(Allcellriphist);
 imagesc(sortAllcellriphist,[-3 3]);
%  hold
% end
 line([50 50], [0 length(sortAllcellriphist)], 'Color', [0 0 0]);
return
