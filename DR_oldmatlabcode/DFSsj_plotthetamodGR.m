...
% Notice
savedir = '/data15/gideon/Ndl/ProcessedData/';
%savefile = [savedir 'HP_thetamod_PFC']; area = 'PFC'; clr = 'b'; % PFC
savefile = [savedir 'Ndl_thetamod_PFC']; area = 'PFC';  clr = 'r';% PFC
savefig1=0;
...
if runscript == 1
    ...
    % Epoch filter
    % -------------
    dayfilter = '8:17'; % Shantanu - I am adding day filter to parse out epoch filter
    ...
    % Cell filter
    % -----------
    %cellfilter = 'strcmp($area, ''AC'')'; % This includes all, including silent cells. Spike criterion later
    ...
    % Gideon: ask shantanu about this, that it relies on descript=reftet in
    % the tetinfo, only for the ref tets.
    ca1reftetfilter = '(isequal($descrip, ''reftet''))';
    ...
    modf = setfilterfunction(modf,'DFAsj_plotthetamod',{'spikes','theta'}); % Corresponding function is sj_plotthetamod
...
%gatherdatafile = [savedir 'HP_thetamod_CA1_gather']; % CA1 cells to Hipp theta
gatherdatafile = [savedir 'Ndl_thetamod_PFC_gather']; % CA1 cells to Hipp theta
...
if (figopt1)
    for i=1:cntcells
        ...
        if curridx(1)==3 && sig_shuf==1  % To control plotting
...
ncells_days=[NaN(7,1); ncells_days];
ncells_days_sig=[NaN(7,1); ncells_days_sig];
