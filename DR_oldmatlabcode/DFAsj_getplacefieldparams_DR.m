...
%DR added mean threash within cell,,traj
...
%DR get mean across trajs for each cell
tmpalltraj = [];
for traj = 1:length(trajdata)
    if ~isempty(trajdata{traj})
        tmpalltraj = [tmpalltraj; trajdata{traj}(:,5)];
        %         if traj ==1;
        %             tmpalltraj = trajdata{traj}(:,5);
        %         else
        %             tmpalltraj = [tmpalltraj; trajdata{traj}(:,5)];
        %         end
