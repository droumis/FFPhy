...
switch indsSetName
    case 'performance'
        ...
        %         corrOutBInd = find(cell2mat(cellfun(@(x) strcmp(x,'corrOut'), iEpTetDataTypeFields, 'UniformOutput', false)));
        %         mistOutBInd = find(cell2mat(cellfun(@(x) strcmp(x,'mistOut'), iEpTetDataTypeFields, 'UniformOutput', false)));
        corrOutBInd = 6;
        mistOutBInd = 7;
        corrOutEventInd = find(eState.eventstate{ian}.state(1:end,corrOutBInd) == 1);
        mistOutEventInd = find(eState.eventstate{ian}.state(1:end,mistOutBInd) == 1);
        outBEventInd = [corrOutEventInd; mistOutEventInd];
        inBEventInd = setdiff([1:length(eState.eventstate{ian}.state(1:end,1))], outBEventInd)';
        iEpTetIndsType = {[1:length(eState.eventstate{ian}.state(1:end,1))], corrOutEventInd, mistOutEventInd, outBEventInd, inBEventInd};
...
%         baselinePhDataIndSet = num2cell(repmat(1, 1, length(DataTypeFields)));  %index into the iEpTetIndsType cell array for which set of indices to use as baseline
    case 'sleepByDay'
        unqDays = unique(eState.eventstate{ian}.state(:,daycol));
        daysInds = arrayfun(@(x) find(eState.eventstate{ian}.state(:,daycol) == x), unqDays, 'un', 0);
...
%         baselinePhDataIndSet = num2cell(unqDays');
    case 'performanceByDay'
        daycol =  find(cell2mat(cellfun(@(x) strcmp(x,'day'), eState.eventstate{ian}.fields, 'UniformOutput', false)));
        corrOutBInd = find(cell2mat(cellfun(@(x) strcmp(x,'correct'), eState.eventstate{1}.fields, 'UniformOutput', false)));
        mistOutBInd = find(cell2mat(cellfun(@(x) strcmp(x,'mistake'), eState.eventstate{1}.fields, 'UniformOutput', false)));
        
        unqDays = unique(eState.eventstate{ian}.state(:,daycol), 'stable');
        daysInds = arrayfun(@(x) find(eState.eventstate{ian}.state(:,daycol) == x), unqDays, 'un', 0);
        corrData = eState.eventstate{ian}.state(1:end,corrOutBInd);
        dayData = eState.eventstate{ian}.state(1:end,daycol);
        mistData = eState.eventstate{ian}.state(1:end,mistOutBInd);
