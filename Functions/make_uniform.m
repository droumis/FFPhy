% Convert phase signal to one with uniform prior.
% Dependencies: Modified ecdf function (ecdf_index - included).

% Author: Nikolas Karalis, 2019

function y = make_uniform(x)

a = min(x);
b = max(x);

y = nan(size(x));
x1 = ~isnan(x); % if wanting to maintain NaN values. Otherwise, comment it out.

[f,~,ind] = ecdf_index(x(x1));
[~,ind] = sort(ind);

y(x1) = (b-a)*f(ind)+a; % if wanting to maintain NaN values. Otherwise, replace with line below.
%y = (b-a)*f(ind)+a;


function [Fout,x,indout,Flo,Fup,D] = ecdf_index(y,varargin)

% Modified by NK to return the "indout" as 3rd argument
