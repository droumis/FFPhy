function kk_clusterlessdecode20(directoryname,fileprefix,dayeps,animalname,varargin)
...
for de = 1:size(dayeps,1)
   ...
   moveperiods = dummy{d}{ep};
        ...
        %movevec  = logical(list2vec(moveperiods,postimevec));
   ...
   elseif EXTYPE == 7                   % Ret
       ...
       %Lind = Lind & (lindist > CP);  % added this 4.17.18 to troubleshoot       
       %Rind = Rind & (lindist > CP);  % added this 4.17.18 to troubleshoot       
  ...
  %%%%% Preliminary "Interposed" Set-up decoding time bins %%%%%% kk 3.29.18 %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  %%%% Doing this first (unlike other decoding functions) for THETA_BINS = 2 %%%%%%%
  %    The 1st half theta cycles are used to encode in the section following %%%%% 
    ...
    Tbins = {};         % (optional) for THETA_BINS > 0
    
    if THETA_BINS == 0          % Moving Window decode
        
        dt = nummsbin/1000;     % decoding bin duration     
    ...
    elseif THETA_BINS > 0      % Theta-bin based decode
        ...
        % 2. Subdivide cycles into 1st & 2nd
            ...
            Tbins               = cell(1,5);  % {1}: 1st, {2}: 2nd, {3}: full, {4}: 1st-2nd, {5}: 1st-2nd filled out
        ...
        % 3. (optional) for THETA_BINS == 3
        if THETA_BINS == 3
            lindist; CP;
            Outerarms_periods = vec2list((lindist > CP),postimevec) ;
        elseif THETA_BINS == 4
            Outerarms_periods = vec2list((lindist > CP-20),postimevec) ;
        end
   ...
   for tet = selected_tets
       ...
       if ismember(THETA_BINS,[2 3 4])      % if 1st half Theta cycle only, filter here
           sinds_1st_half     = logical( isExcluded( SPK(:,1)/10000 , Tbins{1}));  % spikes occurring during 1st half of theta cycles
       else
           sinds_1st_half     = true(size(SPK(:,1)));
       end
       if THETA_BINS == 3 || THETA_BINS == 4
           sinds_outerarms    = logical( isExcluded( SPK(:,1)/10000 , Outerarms_periods));
       else
           sinds_outerarms    = true( size(SPK(:,1)) );
       end
       % (optional) Filter out FS spikes ("type") to get pyramidal neuron spikes
       if exclude_FS
           maxwidths = filedata.params(:,6)/10;
           sinds_nonFS = maxwidths > 1;
       ...
       %%% Encoding spikes %%%%%%%%%%%%%
       sinds_e      = sinds_type  &  sinds_amp  &  sinds_exc  &  sinds_1st_half  &  sinds_outerarms ;   % Final set of encoding spikes                
       %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
   ...
   % Identify the rat's actual LR (during enc periods)
   posinds_enc = [];
   numposbin_enc = nan;     
   ...
   occ       = sum(deltapdf1,2);                                % [ xbins x 1         ] Rat's enc period Dir occupancy from Gauss kerns
        ...
        % SANITY CHECK
        if any(isnan(LR2(posinds_enc)))
            disp('none should be nan, sanity check')
            keyboard
        end
   ...
   for tet3 = selected_tets
       ...
       Lint{tet3} = sum(Xnum{tet3},2) ./ occ ./ totalencdur;            % [ xbins x 1 ]   Adds spikes (each one probability 1) and divides by occ (enc periods) and pos time step               
        ...
        % SANITY CHECK
        if any(isnan(LR2(posind_spk_e{tet3})))
            disp('none should be nan, sanity check')
            keyboard
        end
   ...
   %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    % i. (preliminary) Identify Excursions (i.e. contiguous periods to decode) %%%%%%%%%%%%%%%%
    ...
    for ex = 1:Numexcur
        ...
        for bbb = 1:numdecbin
            ...
            decbin = Win(t,:) + epstart;        % [starttime endtime]
                dt = decbin(2) - decbin(1);     % duration of decoding bin
            midtime = mean(Win(t,:),2);         % [ midtime ]
            
                disp(sprintf('(LR %d, dt: %d) %s d%d ep%d: %s',EXTYPE,round(dt*1000),animalname(1:3),d,ep,num2str(midtime)))
