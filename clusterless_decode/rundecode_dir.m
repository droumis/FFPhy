% Cluster decoding w/ direction x 3 W-track arms -- 6 posteriors
...
if calculate
    %%% select data %%
    animals_tocalc = {'Bond'}; %{'Bond','Corriander'}; %
    cellchoice = 1;
    dayeps = [7 2; 7 4; 7 6; 8 2 ; 8 4; 8 6; 9 2 ; 9 4 ; 9 6; 10 2; 10 4; 10 6];  %[6 2; 6 4; 6 6];  % leave empty if want to do all epochs
    manual_period = [];
    minplaceunits = 15;
    winsize = .015;  % in seconds, decoding bin size
    ...
    modelnum = 4;   % 1: all spikes, 2: trajencode, 3: exclude SWR only
end
...
if calculate
    for aa = 1:length(animals_tocalc)
        ...
        kk_clusterdecode_dir(animalinfo{2},animalinfo{3},dayeps,animalname,...
            ...
            'winsize',winsize,'manual_period',manual_period,'exclude_inters',exclude_inters,'minplaceunits',minplaceunits);
    end
end
...
if plot_continuous
    ...
    xvecms =  P.binvec_c{1} -  P.binvec_c{1}(1);
    armdists = P.armdists;
   ...
   figure('units','normalized','outerposition',[0 0 .8 .8])
    ...
    if plot_riptrace
        plot((riptrace_timevec(ind1:ind2) - epstart), 6 * -riptrace(ind1:ind2)/2 + ylevel + 10,'-','Color',[.8 .75 .75],'linewidth',2,'Parent',ax(1) ); hold on
        % mean line
        plot([riptrace_timevec(ind1) riptrace_timevec(ind2)] - epstart, [(ylevel+10+meanz*6)  (ylevel+10+meanz*6)]  ,'--','Color',[.1 .1 .1],'linewidth',2,'Parent',ax(1) ); hold on
        % +/- 1 SD
        plot([riptrace_timevec(ind1) riptrace_timevec(ind2)] - epstart, [(ylevel+10+(meanz+stdz)*6)  (ylevel+10+(meanz+stdz)*6)]  ,'--','Color',[.4 .4 .4],'linewidth',2,'Parent',ax(1) ); hold on
        plot([riptrace_timevec(ind1) riptrace_timevec(ind2)] - epstart, [(ylevel+10+(meanz-stdz)*6)  (ylevel+10+(meanz-stdz)*6)]  ,'--','Color',[.4 .4 .4],'linewidth',2,'Parent',ax(1) ); hold on
    end
    ...
    for n = 1:2
        ...
        plot([-xvecms(a) xvecms(b)],[centerarmmax(n) centerarmmax(n)],'--','Color',[.8 .8 .8],'linewidth',.5,'Parent',ax(n+1))
