function kk_clusterlessdecode7(directoryname,fileprefix,dayeps,animalname,varargin)
...
for de = size(dayeps,1):-1:1
  ...
  encodeperiods = cell(1,2);    
   ...
   for nn = 1:num_encodeeps
       ...
       % Initialize pospos %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
       outpos{nn} = nan(size(lindist)) ;   % (horizontal vector)  0+ (OUT) +500 if R, 1000+ (IN), +500 if R  lin dist from center well + L vs. R
       outpos2{nn} = nan(size(lindist));
            centerarmmax(nn) =  CPbuff;   % maximum linear distance of center arm    
                CAinds = (lindist < CPbuff)    &  (headdir2 >= 0) ;      % Before CP + head out
                Rinds = ~CAinds & (seg4 | seg5) & (headdir2 >= 0);       % R arm     + head out
                Linds = ~CAinds & (seg2 | seg3) & (headdir2 >= 0);       % L arm     + head out
            outpos{nn}(CAinds)          = lindist(CAinds)                      +   0;   % Center 
            outpos{nn}(Linds)           = lindist(Linds)  - centerarmmax(nn)   +   200; % Left
            outpos{nn}(Rinds)           = lindist(Rinds)  - centerarmmax(nn)   +   400; % Right
                
            %%%%%%%%%%%
            outpos{nn}(isnan(outpos{nn})) = -1;   % head in
            
                outpos{nn} = outpos{nn}(:)   ;
                outpos2{nn} = outpos{nn}(:)' ;            
       
              
       %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
       
       %%%% Construct positional mark space (xbins)  %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
       xbins{nn} = 0 : xdel : (max(outpos{nn}) + xdel);  %  min(trajpos{nn}):xdel:(max(trajpos{nn}) + xdel);        %   
                ...
                d_cut{nn} = lookup(max(outpos{nn}(Linds)),xbins{nn});
                
                e_cut{nn} = lookup(400,xbins{nn});
                f_cut{nn} = lookup(max(outpos{nn}(Rinds)),xbins{nn});
      ...
      %  Construct the transition matrix from animal's behavior %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
       stateM = zeros(num_xbins(nn),num_xbins(nn));
            [ ~ , ind_xbin ]  =  histc(outpos{nn},xbins{nn});
...
%           [dx,dy] = meshgrid( [-sidelength:sidelength] );
...
%           stateM_gaus = conv2(stateM, weight, 'same');                  % Gaussian smoothed
       ...
       for tet = selected_tets
           ...
           [dummy,~] = evaluatetimefilter(animalinfo{2},animalinfo{3},{vel4},[d epenc]);
                ...
                outdir_periods = vec2list((headdir2 >= 0),postimevec_epenc);
           ...
           if modelnum == 2
               ...
               inds_e = inds_epenc & inds_thresh & inds_moving & inds_outdir;
                    encodeperiods{nn} = vel4_periods;  %%%%%%%%%%%%%%%%%%%%%%%%%%  (Noticed this Dec 2017) -- should maybe replace this with outdir + vel4
   ...
   for nnn = 1:num_encodeeps
           ...
           MU1 = ones(length(xbins{nnn}),1)  *  outpos2{nnn}(posinds_enc{nnn});  % actual positions of the animal       
           ...
           for tet3 = selected_tets
               ...
               MU2 = ones(length(xbins{nnn}),1)  *  outpos2{nnn}(posind_spike_e{nnn}{tet3});    % actual (encoding) spike positions
...
%    end
    ...
    for n = 1:num_encodeeps
        ...
        occ{n} = normpdf( xbins{n}'*ones(1,numpossamps_enc(n)),...
                       repmat(outpos2{n}(posinds_enc{n}),num_xbins(n),1),...
        ...
        if 1
            ...
            Xnum_all = normpdf( xbins{n}'*ones(1,num_allspikes_e),...           %  # of encoding spikes this epoch
                repmat(outpos2{n}(posind_allspikes_e),num_xbins(n),1),...    %  # pos inds of all encoding spikes this epoch
                sxker );            
        elseif 0
            Xnum_all = normpdf( xbins{n}'*ones(1,length(spktimes_all{n})),...
                repmat(pospos2{n}(posind_spike_all{n}),num_xbins(n),1),...
                sxker );
        end
            % set non-occupied xbins indices to 0
    ...
    for ww = 1:numevents
        ...
        for nnn = 1:num_encodeeps
                ...
                if decodemode == 1
                    ...
                    elseif t == round(23*num_msbin_steps/100)
                        ...
                        if 0
                            ...
                            %colormap(flipud(hot))
                            set(gca,'ydir','normal')
                            plot(postimevec_epenc(aa:bb) - postimevec_epenc(1),outpos{1}(aa:bb),'b-','linewidth',2);
                            keyboard
                        end                        
            ...
            if 0
                % POSITION
                figure;
                plot(postimevec_epenc - postimevec_epenc(1),outpos{1},'Color',[.8 .8 .8],'linewidth',2);
            end         
            if 0
                % DECODE
                tvec = (1:num_msbin_steps)*nummsbin/1000;
            ...
            savefilename = sprintf('%s_%d_%d_%s_%d_outpos',animalname(1:3),d,ep,'fullepoch',nnn);
            P = struct;
            P.nummsbin = nummsbin;
            ...
            P.posvec = outpos{1};   % actual animal traj / positions
