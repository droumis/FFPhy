function kk_clusterlessdecode4(directoryname,fileprefix,dayeps,animalname,varargin)
   ...
   if TETSET == 5
       ...
       an = find(strcmp(animalname,animals_order));
             ...
             if SKIPSAVED
               ...
               filename = sprintf('%s_%d_%d_fullepoch_1_trajpos.mat',animalname(1:3),d,ep);
  ...
  encodeperiods = cell(1,2);    
   ...
   for nn = 1:num_encodeeps
       ...
       trajpos2{nn} = nan(size(lindist));
            ...
            trajpos{nn}(isnan(trajpos{nn})) = -1;   % everything that isn't specified in trajencode
       ...
       xbins{nn} = 0 : xdel : (max(trajpos{nn}) + xdel);  %  min(trajpos{nn}):xdel:(max(trajpos{nn}) + xdel);        %   
                ...
                %indices for each positional "island" a to b, c to d, e to f, g to h (each trajectory)
                a_cut{nn} = 1;
                b_cut{nn} = lookup(max(trajpos{nn}(traj3)),xbins{nn});
                    CPind = lookup(CPbuff,xbins{nn});  
      ...
      %  Construct the transition matrix from animal's behavior %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
       ...
       elseif 1
           ...
           if 1
               % SFN poster: plot transition matrix
               
               figure('units','normalized','outerposition',[0 .3 .9 .6])
              
               
               % (Left plot) 
               subplot(1,2,1); imagesc(1:size(stateM,1),1:size(stateM,2),stateM > 0,[0 .0001])
               colormap gray;  colormap(flipud(colormap)) ; colorbar ; axis square; hold on
               plot([0 0],[size(stateM,1) size(stateM,1)],'r-','linewidth',1);
               plot([0 200],[600 size(stateM,1)],'r-','linewidth',1);

               
               set(gca,'tickdir','out')
               
               % (Right plot) 
               subplot(1,2,2); imagesc(1:size(M,1),1:size(M,2),M,[0 .001])
               colormap gray;  colormap(flipud(colormap)) ; colorbar
               axis square
               set(gca,'tickdir','out')
...
%                T(~validxbins{nn},:) = [];
%                T(:,~validxbins{nn}) = [];
%                subplot(1,2,2); imagesc(1:size(T,1),1:size(T,2),T,[0 .001])
        ...
        for nnn = 1:num_encodeeps
            ...
            for t = ind_start:num_msbin_steps
                ...
                elseif num_spikesinbin > 0
                    ...
                    L = L./sum(L) ;
                   disp(sprintf('(trajpos) %s d%d ep%d: %s',animalname(1:3),d,ep,num2str(t * nummsbin / 1000)))
                end
