        ...
        for aa = 1:length(animals_tocalc)
            ...
            for de = 1:size(dayeps,1)
                ...
                if ~exist('P','var') || isempty(P) || ~strcmp(P.animalname,animalname) || ...
                        ~all(P.dayep == [d ep])
                    % Decode file %%%%%%%%%%%%%%
                    filename = sprintf('%s_%d_%d_Pos_1.mat',animalpref,d,ep);
                    cd(datadir);
                ...
                % Identify excursions %%%%%%%%%%%%%%%%%%%%%%5
                excurlist = [];              
                if EXCURTYPE == 1
                    inds = ismember(P.excurlist(:,3),[1 3 -11]); % Outbound trajs (1,3) + Center well trackbacks + R>>L and L>>R
                end
                excurlist   = P.excurlist(inds,1:3);
                numexcur    = size(excurlist,1);
                %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  
                % Concatenate across Excurs %%%%%%%%%%%%%%%%%
                
                SIvec_orig      = [];
                Altvec_orig     = [];
                Contigvec       = [];  % 111122221111: contig vector
                
                analysis_totaldur = 0;
                for xx = 1:numexcur
    
                    % Vector times of Excursion
                    excurvec = list2vec(excurlist(xx,[1 2]),postimevec);
                        
                    % Process posterior into image
                    Post    = P.Posts{xx};
                    tvec    = P.Tvecs{xx};
            
                    % Isolate periods to analyze
                    intersectvec = CPprevec & excurvec & movingvec;   % Prior to CP, Excursion, Movement
                    intersectperiods = vec2list(intersectvec,postimevec);                        
                    ...
                    choiceperiodsdur = round(sum(intersectperiods(:,2) - intersectperiods(:,1)));
                    analysis_totaldur = analysis_totaldur + choiceperiodsdur;
                    
                    % Vector times of analysis Periods                    
                    choicevec = logical(list2vec(intersectperiods - postimevec(1),tvec)); 
                    ...
                    LRdensity       = Ldensity + Rdensity;
                        LRdensity_cut = LRdensity(choicevec);
                    ...
                    tph; tvec_tph;
                    tphs = tph(lookup(tvec_cut,tvec_tph - epstart)); % theta phase of each density bin
                    LRthreshinds2 = LRthreshinds(choicevec);
                    ...
                    for bbb = 1:nbins
                        phasehist(bbb) = sum(LRdensity_cut( (I' == bbb) & LRthreshinds2(:) ));
                    end
                    ...
                    [tbins,~,~,~,~,~,tbblocks]  ...
                        ...
                        filterinds = logical(isExcluded(mean(tbins,2),intersectperiods)); % Get bins that occur analysis periods
                    ...
                    SIvec_orig_exc = nan(1,totalnumcyc);
                    Altvec_orig_exc = nan(1,totalnumcyc);
                    ...
                    for tb = 2:size(tbins,1)
                        ...
                        if (Lval + Rval)/(Cval + Rval + Lval) < LR_prop_thresh
                            ...
                            if tbins(tb,1) == tbins(tb-1,2)
                                % Store SI value
                                SIvec_orig_exc(tb) = LRval;
                                if tb ~= 1 && ~isnan(SIvec_orig_exc(tb-1))
                                    % Calculate Alt speed
                                    Altvec_orig_exc(tb) = abs( LRval - SIvec_orig_exc(tb-1) );   % alternation speed
                                end
                            else
                                continue
                            end
                    ...
                    if isempty(Contigvec)
                        FLAGVAL = 1;
                    elseif Contigvec(end) == 1
                        FLAGVAL = 2;
                    else
                        FLAGVAL = 1;
                    end
                    
                    SIvec_orig  = [SIvec_orig ;  SIvec_orig_exc(:)];
                    Altvec_orig = [Altvec_orig ; Altvec_orig_exc(:)];
                    Contigvec   = [Contigvec ; FLAGVAL * ones(totalnumcyc,1) ];
                    
                    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
                    
                end
                
                disp(sprintf('%d sec of excurperiods (%d)',analysis_totaldur,EXCURTYPE))
                
                
                
            % Alternation analysis %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            
            % I. Detecte Alternations throughout epoch %%%%%%%%%%%%%%
