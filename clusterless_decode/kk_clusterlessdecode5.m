function kk_clusterlessdecode5(directoryname,fileprefix,dayeps,animalname,varargin)
...
for de = 1:size(dayeps,1)
  ...
  encodeperiods = cell(1,2);    
   ...
   for nn = 1:num_encodeeps
       ...
       % Initialize pospos %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
       pospos{nn} = nan(size(lindist)) ;   % (horizontal vector)  0+ (OUT) +500 if R, 1000+ (IN), +500 if R  lin dist from center well + L vs. R
       pospos2{nn} = nan(size(lindist));
            ...
            pospos{nn}(CAinds)        = lindist(CAinds)                  +   0;   % Center 
            pospos{nn}(Rinds) = lindist(Rinds)  - centerarmmax(nn)   +   400; % Right
            pospos{nn}(Linds) = lindist(Linds)  - centerarmmax(nn)   +   200; % Left
                pospos{nn} = pospos{nn}(:)   ;
                pospos2{nn} = pospos{nn}(:)' ;            
       
              
       %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
       
       %%%% Construct positional mark space (xbins)  %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
       xbins{nn} = 0 : xdel : (max(pospos{nn}) + xdel);  %  min(trajpos{nn}):xdel:(max(trajpos{nn}) + xdel);        %   
                ...
                d_cut{nn} = lookup(max(pospos{nn}(Linds)),xbins{nn});
                
                e_cut{nn} = lookup(400,xbins{nn});
                f_cut{nn} = lookup(max(pospos{nn}(Rinds)),xbins{nn});
      ...
      %  Construct the transition matrix from animal's behavior %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
       stateM = zeros(num_xbins(nn),num_xbins(nn));
            [ ~ , ind_xbin ]  =  histc(pospos{nn},xbins{nn});
       ...
       for tet = selected_tets
           ...
           inds_trajencode = isExcluded(filedata.params(:,1)/10000,vel4_periods)  ;
   ...
   for nnn = 1:num_encodeeps
           ...
           MU1 = ones(length(xbins{nnn}),1)  *  pospos2{nnn}(posinds_enc{nnn});  % actual positions of the animal       
           ...
           for tet3 = selected_tets
               ...
               MU2 = ones(length(xbins{nnn}),1)  *  pospos2{nnn}(posind_spike_e{nnn}{tet3});    % actual (encoding) spike positions
...
%    end
    ...
    for n = 1:num_encodeeps
        ...
        occ{n} = normpdf( xbins{n}'*ones(1,numpossamps_enc(n)),...
                       repmat(pospos2{n}(posinds_enc{n}),num_xbins(n),1),...
        ...
        if 1
            ...
            Xnum_all = normpdf( xbins{n}'*ones(1,num_allspikes_e),...           %  # of encoding spikes this epoch
                repmat(pospos2{n}(posind_allspikes_e),num_xbins(n),1),...    %  # pos inds of all encoding spikes this epoch
                sxker );            
        elseif 0
        ...
        for nnn = 1:num_encodeeps
            ...
            for t = ind_start:num_msbin_steps
                ...
                elseif num_spikesinbin > 0
                    ...
                    L = L./sum(L) ;
                    disp(sprintf('(pospos) %s d%d ep%d: %s',animalname(1:3),d,ep,num2str(t * nummsbin / 1000)))
                end
                ...
                if decodemode == 1
                    ...
                    elseif t == round(23*num_msbin_steps/100)
                        ...
                        if 0
                            ...
                            %colormap(flipud(hot))
                            set(gca,'ydir','normal')
                            plot(postimevec_epenc(aa:bb) - postimevec_epenc(1),pospos{1}(aa:bb),'b-','linewidth',2);
                            keyboard
                        end                        
                    elseif t == round(50*num_msbin_steps/100)
                        ...
                        savefilename = sprintf('%s_%d_%d_%s_%d_pospos_firsthalf',animalname(1:3),d,ep,'fullepoch',nnn);
                        P = struct;
                        P.nummsbin = nummsbin;
                        ...
                        P.posvec = pospos{1};   % actual animal traj / positions
                        ...
                        if 0
                            figure; 
                             plot(postimevec_epenc - postimevec_epenc(1),pospos{1},'Color',[.8 .8 .8],'linewidth',2);
                        end
            ...
            savefilename = sprintf('%s_%d_%d_%s_%d_pospos',animalname(1:3),d,ep,'fullepoch',nnn);
            P = struct;
            P.nummsbin = nummsbin;
            P.animalname = animalname;
            P.dayep = [d ep];
