   ...
   for nn = 1:num_encodeeps
      ...
      %  Construct the transition matrix from animal's behavior %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
       ...
       elseif 1
           ...
           if 1
               % End nubs (choice point, forward movement assumption)
                M(:,~validxbins{nn}) = 0;    % zero invalid columns
                M(~validxbins{nn},:) = 0;    % zero invalid rows
                M(c_cut{nn},b_cut{nn}) = 1;  % C out to R out
                M(e_cut{nn},b_cut{nn}) = 1;  % C out to L out 
                M(e_cut{nn},i_cut{nn}) = 1;  % L in to R out
                M(h_cut{nn},i_cut{nn}) = 1;  % L in to C in
                M(c_cut{nn},k_cut{nn}) = 1;  % R in to L out
                M(h_cut{nn},k_cut{nn}) = 1;  % R in to C in
           end
           
           if 0
               % Directional switching
               R = 1e-3;
               L =    diag( ones(sidelength-600,1),-600) * R + ...
                      diag( ones(sidelength-600-1,1),-601) * R + ...
                      diag( ones(sidelength-600+1,1),-599) * R ;
               U =    diag( ones(sidelength-600,1),+600) * R + ...
                      diag( ones(sidelength-600+1,1),+599) * R + ...
                      diag( ones(sidelength-600-1,1),+601) * R;
...
%    end
    ...
    for ww = 1:numevents
        ...
        for nnn = 1:num_encodeeps
            ...
            quartervec = linspace(1, num_msbin_steps, 5);
                quartervec = round(quartervec);
                quarterinds = nan(4,2);
                quarterinds(1,:) = [ 1 quartervec(2) ];
                quarterinds(2,:) = [ quartervec(2)+1 quartervec(3) ];
                quarterinds(3,:) = [ quartervec(3)+1 quartervec(4) ];
                quarterinds(4,:) = [ quartervec(4)+1 num_msbin_steps ];
               
                if dec_start == 0
                    Qvec = 1:4;
                else
                    ind_start = dec_start * 1000 / nummsbin + 1;      
                    Qvec = find((ind_start >= quarterinds(:,1)) & ...
                                (ind_start < quarterinds(:,2)) );
                end
                
            for Q = Qvec   % Decode the epoch in quarters
                
                if SKIPSAVED
                    cd(savedir)
                    filename = sprintf('%s_%d_%d_Q%d_dirpos.mat',animalname(1:3),d,ep,Q);
                ...
                if dec_start == 0
                    msbin_a = quarterinds(Q,1);
                    msbin_b = quarterinds(Q,2);
                else
                    msbin_a = ind_start;
                    msbin_b = quarterinds(Q,2);
                end
                
                msbinvec = msbin_a:msbin_b;
                blocklength = length(msbinvec);
                ...
                % save %%%%%%%%%%%%%%%%%%%%%%%%%%%
                cd(savedir)
                disp('done calculating a Quarter of the epoch')
                savefilename = sprintf('%s_%d_%d_Q%d_dirpos',animalname(1:3),d,ep,Q);
                P = struct;
                P.animalname = animalname;
                P.date = date;
                P.nummsbin = nummsbin;
                P.totalnummsbins = length(ms_bin_timevec_ep);
                P.dayep = [d ep];
                P.remoteep = remoteep;
            ...
            if 0
                ...
                aa = lookup(plot_start + msbin_a/1000,postimevec_epenc - postimevec_epenc(1));
                bb = lookup(plot_end + msbin_a/1000 ,postimevec_epenc - postimevec_epenc(1));
                
                % plot
                figure; imagesc(tvec(a:b) + msbin_a/1000,xbins{1},postxM_r(:,a:b),[0 .1]); hold on
                colormap gray
                colormap(flipud(gray))
                set(gca,'ydir','normal')
                
                plot(postimevec_epenc(aa:bb) - postimevec_epenc(1),...
                     dirpos{1}(aa:bb),'r-','linewidth',2);
