function kk_clusterlessdecode11(directoryname,fileprefix,dayeps,animalname,varargin)
  ...
  encodeperiods = cell(1,2);    
   ...
   for nn = 1:num_encodeeps
       ...
       postimevec_epenc = linpos{d}{epenc}.statematrix.time;
            ...
            epstart = postimevec_epenc(1);
            epend = postimevec_epenc(end);
...
%    end
    ...
    for ww = 1:numevents
        ...
        for nnn = 1:num_encodeeps
                ...
                if SKIPSAVED
                    cd(savedir)
                    filename = sprintf('%s_%d_%d_dirpos.mat',animalname(1:3),d,ep);
                ...
                msbinvec = 1:num_msbin_steps;
                
                if dec_start > 0
                    msbin_a = dec_start * 1000 / nummsbin + 1;
                else
                    msbin_a = 1;
                end
                
                % initialize
                postxM_r = zeros(num_xbins(nnn),num_msbin_bins_ep) ;              % Matrix of posteriors
                ...
                for tt = msbin_a:num_msbin_steps
            ...
            if 1
                ...
                decodeper = vec2list(sum(postxM_r,1) > 0, tvec);
                ...
                aa = lookup(plot_start + msbin_a*nummsbin/1000,postimevec_epenc - epstart);
                bb = lookup(plot_end + msbin_a*nummsbin/1000 ,postimevec_epenc - epstart);
                
                % plot
                figure; imagesc(tvec(a:b) + msbin_a*nummsbin/1000,xbins{1},postxM_r(:,a:b),[0 .1]); hold on
                colormap gray
                colormap(flipud(gray))
                set(gca,'ydir','normal')
                
                plot(postimevec_epenc(aa:bb) - epstart,...
                     dirpos{1}(aa:bb),'r-','linewidth',2);
                
                 keyboard
                 
                 figure; imagesc(postxM_r,[0 .1])
                colormap gray
                colormap(flipud(gray))
                set(gca,'ydir','normal')
