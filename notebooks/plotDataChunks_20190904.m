...
if plotfigs
    ...
    for ts = 1:length(timeSplits(:,1))
        ...
        outdir = sprintf('%s/dfa_plotDataChunk/%s/day%d_epoch%d/',pconf.andef{4},animal,day,epoch);
        ...
        if ~any(swrInWinIdx)
            fprintf('no rips in win %1.f : %1.f\n', tstart, tend);
            if skipNoRipChunks
                fprintf('skipping\n');
                continue;
            end
        ...
        if ~isempty(mustack)
            ...
            axis tight; yl = ylim; ylim([yl(1)-2 yl(2)+2]); xticks([]);
            set(gca, 'YGrid', 'off', 'XGrid', 'on');
            set(gca,'TickDir','out');
            ylabel('muSpikes');
        end
        ...
        velinds = vel < maxvelocity; v = vel(velinds); v(v<4) = 1; v(v>=4) = 0; hold on;
        ...
        scatter(linpostimeCh,lindistCh,10,Pp.linposclr,'filled', 'marker', 'd'); %background pos
        xlabel('Time s'); ylabel('Lindist'); hold on; axis tight; xl = xlim;
        ...
        for c = 1:length(IOdioCh) % collect input dios
            ...
            % dios are state changes.. so start with an Up (1) and end with a Down (0) change
        ...
        if ~isempty(diosDF)
            ...
            patch(sf1,a,b,diosDF(:,1),'FaceAlpha',Pp.dioFAlpha,'edgealpha',Pp.dioEAlpha);
            yl = ylim(sf2);
            a = [diosDF(:,2:3)'; diosDF(:,[3 2])'];
            b = repmat([yl(1) yl(1) yl(2) yl(2)]',1, length(diosDF(:,1)));
            patch(sf2,a,b,diosDF(:,1),'FaceAlpha',Pp.dioFAlpha,'edgealpha',Pp.dioEAlpha);
            yl = ylim(sf3);
            a = [diosDF(:,2:3)'; diosDF(:,[3 2])'];
            b = repmat([yl(1) yl(1) yl(2) yl(2)]',1, length(diosDF(:,1)));
            patch(sf3,a,b,diosDF(:,1),'FaceAlpha',Pp.dioFAlpha,'edgealpha',Pp.dioEAlpha);
            yl = ylim(sf4);
            a = [diosDF(:,2:3)'; diosDF(:,[3 2])'];
            b = repmat([yl(1) yl(1) yl(2) yl(2)]',1, length(diosDF(:,1)));
            patch(sf4,a,b,diosDF(:,1),'FaceAlpha',Pp.dioFAlpha, 'edgealpha', Pp.dioEAlpha);
            yl = ylim(sf5);
            a = [diosDF(:,2:3)'; diosDF(:,[3 2])'];
            b = repmat([yl(1) yl(1) yl(2) yl(2)]',1, length(diosDF(:,1)));
            patch(sf5,a,b,diosDF(:,1),'FaceAlpha',Pp.dioFAlpha,'edgealpha',Pp.dioEAlpha);
            yl = ylim(sf6);
            a = [diosDF(:,2:3)'; diosDF(:,[3 2])'];
            b = repmat([yl(1) yl(1) yl(2) yl(2)]',1, length(diosDF(:,1)));
            patch(sf6,a,b,diosDF(:,1),'FaceAlpha',Pp.dioFAlpha,'edgealpha',Pp.dioEAlpha);
            yl = ylim(sf7);
            a = [diosDF(:,2:3)'; diosDF(:,[3 2])'];
            b = repmat([yl(1) yl(1) yl(2) yl(2)]',1, length(diosDF(:,1)));
            patch(sf7,a,b,diosDF(:,1),'FaceAlpha',Pp.dioFAlpha,'edgealpha',Pp.dioEAlpha);
            yl = ylim(sf8);
            a = [diosDF(:,2:3)'; diosDF(:,[3 2])'];
            b = repmat([yl(1) yl(1) yl(2) yl(2)]',1, length(diosDF(:,1)));
            patch(sf8,a,b,diosDF(:,1),'FaceAlpha',Pp.dioFAlpha, 'edgealpha', Pp.dioEAlpha);
            yl = ylim(sf9);
            a = [diosDF(:,2:3)'; diosDF(:,[3 2])'];
            b = repmat([yl(1) yl(1) yl(2) yl(2)]',1, length(diosDF(:,1)));
            patch(sf9,a,b,diosDF(:,1),'FaceAlpha',Pp.dioFAlpha, 'edgealpha', Pp.dioEAlpha);
        end
        hold off
        %% plot ripple patches
        if any(swrInWinIdx)
            ...
            patch(sf5, [xs'; xe'; xe'; xs'],repmat([yl(1) yl(1) yl(2) yl(2)]',1,...
                length(xe)),Pp.ripclr,'FaceAlpha',0, 'edgecolor','b', 'edgealpha', .5);
            yl = ylim(sf6);
