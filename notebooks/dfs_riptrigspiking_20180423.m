...
if runFilterFramework || loadFilterOutput
    ...
    combineEpochs = 0;%runFilterFramework;
    saveCombinedEpochs = 0;%combineEpochs;
...
for iAn = 1;%:length(animals);
    ...
    % get ntrode info for each day tet cell for this animal
    ...
    try
        ...
        error('all ntrodes need to have a suparea, subarea, and area tag')
    end
    icolors = colorPicker(Pp.colorSet, strAreas, strSubAreas);
    % ---------- plot per cell --------------------------------
...
%             set(0,'DefaultFigureWindowStyle','normal')
        ...
        for iepenv = 1:size(iepenvs,2); %for each epoch type
            if isempty(iepenvs{iepenv})
                % hack check skip. if not all epochs for day in data
                continue
            end
            n = n+1;
            ...
            % set per rip raster subaxis
            % get rid of empty eptype
            sfA = n;
            num_nonempty_eptypes = numel(find(~cellfun('isempty',iepenvs)));
            sfC = n + num_nonempty_eptypes*2;
            subaxis(4,num_nonempty_eptypes,[sfA sfC]);
%             , 'Spacing',...
%                 Pp.Spacing, 'SpacingVert', Pp.SpacingVert, 'SpacingHoriz', ...
...
%                         'Units', 'normalized', 'horizontalAlignment', 'center');
            ...
            subaxis(4,num_nonempty_eptypes,sfD);
%             , 'Spacing', Pp.Spacing,...
