...
Fp.params = {'licks', 'lickbouts', 'exemplar_wepochs', 'valid_ntrodes', 'nonMU_cells', Fp.filtfunction};
...
if combineEpochs
    if exist('F', 'var')
        ppF = combine_epochs(F, Fp, Fp.paths);
    else
...
if loadCombinedEpochs
    F = load_data(Fp.paths.filtOutputDirectory, Fp.Label, Fp.animals,...
        'filetail', '_combEps');
end
%% ---------------- calc mod---------------------------------------
if calcmod
    ...
    filtF.params = {'wtrackdays', 'lickbouts', 'ca1SU', 'lickTrigSpikingMod'};
