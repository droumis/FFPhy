function smoothrippledayprocess(directoryname,fileprefix,days, varargin)
%smoothrippledayprocess(directoryname,fileprefix,days, options)
%
...
daytetlist = [];
f = '';
defaultfilter = 'smoothrippledayprocess_filter.mat';
system = 2;
downsample = 75;
...
for day = days
   ...
   if (isempty(daytetlist)) 
      tmpflist = dir(fullfile(directoryname,'EEG',sprintf('*ripple%02d-*.mat', day)));
      ...
      for t = tet;
         tmpflist = dir(fullfile(directoryname,'EEG', ...
           sprintf('*ripple%02d-*-%02d.mat',day,t)));
   ...
   if length(flist) == 0
      warning(sprintf('[smoothrippledayprocess] No files found for day %d!',day));
      keyboard
   end
   ...
   smoothripple{day}{epoch}{tet} = filtereeg2(eeg{day}{epoch}{tet}, ...
         thetafilter, 'int16', 1); 
      % downsample if requested 
   if (downsample ~= 1)
      smoothripple{day}{epoch}{tet}.data = ...
      smoothripple{day}{epoch}{tet}.data(1:downsample:end, :);
      smoothripple{day}{epoch}{tet}.samprate = ...
         smoothripple{day}{epoch}{tet}.samprate / downsample;
   end
   clear eegrec
   % replace the filtered invalid entries with the minimum int16 value of
   % -32768
   for i = 1:length(lo)
      if lo(i) > 1 & hi(i) < length(smoothripple{day}{epoch}{tet}.data)
      smoothripple{day}{epoch}{tet}.data(lo(i):hi(i)) = minint;
      end
   end

   % save the resulting file
   smoothripplefile = sprintf('%s/EEG/%ssmoothripple%02d-%d-%02d.mat', ...
