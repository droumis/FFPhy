...
%	'single', 1	Specifies the data should be in single format.  
...
samprate_ratio = round(samprate_ratio);
...
nan_inds = find(isnan(raweegstruct.data));
raweegstruct.data(nan_inds) = 0;
...
if samprate_ratio==1
    ...
    raweegstruct.data = decimate(raweegstruct.data, samprate_ratio);
    filteredeegstruct.samprate = raweegstruct.samprate / samprate_ratio;
end


if (firfilter) 
   if length(raweegstruct.data) > 3*length(filterstruct.kernel)
      filtdata = filtfilt(filterstruct.kernel, 1 , raweegstruct.data);
   else
      filtdata = zeros(size(raweegstruct.data));
   end
else
...
if (~useint16)
    if (usesingle)
       filteredeegstruct.data = zeros(length(phase),2,'single');
       filteredeegstruct.data(:,1) = single(phase);
       clear phase
       filteredeegstruct.data(:,2) = single(env);
       clear env
    else
       filteredeegstruct.data = zeros(length(phase),2);
       filteredeegstruct.data(:,1) = phase;
       clear phase
       filteredeegstruct.data(:,2) = env;
       clear env
    end
    filteredeegstruct.fields = ...
    'instantaneous_phase envelope_magnitude';
else
    filteredeegstruct.data = zeros(length(phase),3, 'int16');
    filteredeegstruct.data(:,1) = int16(phase*10000);
    clear phase
    filteredeegstruct.data(:,2) = int16(env);
    clear env
    filteredeegstruct.fields = ...
    'instantaneous_phase*10000 envelope_magnitude';
end
