...
%     end
    ...
    for ide = 1:size(dayeps,1) % per epoch
        ...
        epoch = dayeps(ide,2);
        try
            epData = data(ian).output{1,ide}.data{1};
            if isempty(epData)
        ...
        if max(vEvents) ~= numValid
            fprintf('D:%d E:%d excluding bc nan: %d of %d events\n', ...
                day, epoch, max(vEvents)-length(vEvents), length(vEvents));
        end
    ...
    out(ian).day = cell2mat(evDay);
    out(ian).epoch = cell2mat(evEpoch);
    out(ian).ntrodes = unique(cell2mat(cellfun(@(x) x(:,3), {data(ian).output{ide}.index}, ...
        'un', 0)'), 'stable');
    
    out(ian).data = cell2mat(permute(dataTnsr, [1 3 2])); % stack all rips
    out(ian).numEvPerEp = cell2mat(numEvPerEp);
    out(ian).evStartIdx = cell2mat(evStartIdx);
    %         out(ian).evEndIdx{t} = cell2mat(evEndIdx);
    out(ian).evStart = cell2mat(evStart);
