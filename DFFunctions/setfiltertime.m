...
for an = 1:length(f)
    ...
    for i = 1:length(f(an).epochs)
        ...
        for j = 1:size(f(an).epochs{i},1)        % iterate over day-epoch pairs
            combinedexclude = [];
            for k = 1:length(timefilter)
                try
                    filterresults{k}{f(an).epochs{i}(j,1)};
                if ~isempty(filterresults{k}{f(an).epochs{i}(j,1)})      % checks if a day has any exclusions at all, kk 4.11.13
                    if (filtertype(k) == 1)
                    ...
                    string = sprintf('day-epoch bypassed (setfiltertime): %d %d',f(an).epochs{i}(j,1),f(an).epochs{i}(j,2));
                    disp(string)
                end
