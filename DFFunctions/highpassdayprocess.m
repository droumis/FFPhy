function highpassdayprocess(directoryname,fileprefix,days, varargin)
%RIPPLEDAYPROCESS(directoryname,fileprefix,days, options)
%
%Applies a high pass filter to all epochs for each day and saves the data in
...
%			specifies, for each day, the tetrodes for which high pass
%			extraction should be done
%
%		'f', matfilename
%			specifies the name of the mat file containing the
%			high pass filter to use 
%			(default /usr/local/filtering/highpassfilter.mat).  
%			Note that the filter must be called 'highpassfilter'.

daytetlist = [];
f = '';
defaultfilter = 'highpassdayprocess_filter.mat';
system = 2;
...
for day = days
	...
	hpdata{day}{epoch}{tet} = filtereeg2(eeg{day}{epoch}{tet}, ...
			highpassfilter, 'single', 1); 
% 			highpassfilter, 'int16', 1); 
	clear eegrec
	% replace the filtered invalid entries with the minimum int16 value of
	% -32768
	for i = 1:length(lo)
	    if lo(i) > 1 & hi(i) < length(hpdata{day}{epoch}{tet}.data)
		hpdata{day}{epoch}{tet}.data(lo(i):hi(i)) = minint;
	    end
	end

	% save the resulting file
	highpassfile = sprintf('%s/EEG/%shpdata%02d-%d-%02d.mat', ...
			    directoryname, fileprefix, day, epoch, tet);
        save(highpassfile, 'hpdata');
	clear hpdata
    end
end
