function f = kk_multicellanal(f)
% f = kk_multicellanal(f)

%% modified by kk May 2013 to be able to handle loading the epoch's ripple eeg
...
for an = 1:length(f)
    ...
    for g = 1:length(f(an).epochs)
        ...
        for e = 1:size(f(an).epochs{g},1)
            ...
            indices = f(an).data{g}{e};             % indices of individual neuronal cells
